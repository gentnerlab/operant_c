...
const char exp_name[] = "CLASSCHOICE";
int box_id = -1;
int flash = 0;
int nchan = 2;
int interleaved = 0;
 ...
 *******************************************************/
int playstereoclass(char *sfname1, char *sfname2,  float targlength, int targloc, float lessdb, int catch, double period)
{
  SNDFILE *sf1, *sf2;
  ...
  long pad = 0;
  int j= 0, loops = 0, inframes, outsamps, nspeak = 2, err, init;
  ...
  if (lessdb == -100.0){
    ...
    if (catch) {
      for (j=0; j<inframes; j++)
        obuff1[j] = scale1 * obuff1[j];
    }
  }
  /* catch trial edits: pick up here */
  obuff = (short *) malloc(sizeof(int)*(outsamps));
  if(DEBUG){fprintf(stderr,"outframes:%d, nspeak:%d, pad:%i, outsamps:%i, targloc:%i\n", (int)outframes, nspeak, (int) pad, outsamps, targloc);}
  /* combine individual buffers into obuff */
  loops=0;
  for (j=0; j<(outsamps); j=j+nspeak){
    loops++;
    if (targloc == 1){  /*CORRESPONDS TO RIGHT SPEAKER TARGET if rt spk is set to channel 1*/
      obuff[j] = obuff1[j/nspeak];   /* 2nd dist or target */
      obuff[j+1] = obuff2[j/nspeak]; /* real dist */
    }
    else if (targloc == 2){  /*MAKE SURE THIS CORRESPONDS TO LEFT  SPEAKER TARGET*/
      obuff[j] = obuff2[j/nspeak];    /* real dist */
      obuff[j+1] = obuff1[j/nspeak];  /* 2nd dist or target */
    }
    else{
...
void do_usage()
{
  ...
  fprintf(stderr, "        -blk         = manually specify block size for trial type. Default = 100 trials.\n");
  fprintf(stderr, "        -qint        = manually specify block size for trial type. Default = 100 trials.\n");
...
int main(int argc, char *argv[]){
  ...
  if (box_id <= 0){
    ...
    fprintf(stderr, "\tERROR: try 'CLASSCHOICE -help' for available options\n");
    exit(-1);
  }
  ...
  ********************************************/
  ...
  do{                                                                               /* start the main loop */
	...
	played = playstereoclass(fstim, fdist, (float)target_duration, targloc, lessdb, catchtrial,period);
	if (DEBUG){fprintf(stderr, "\nplayed: %d\n", played);}
	if(played != 1){
	...
	target=targloc;
	    ...
	    if (DEBUG){fprintf(stderr,"flag: right response to catch trial, lights out\n");}
          ...
          if  (center){ /* correct, went center on catch trial */
            resp_sel = 0;
	    resp_acc = 3;
	    ++Rstim[stim_number].center;++Tstim[stim_number].center;
	    reinfor=feed(stim_reinf, &f);
	    if (reinfor == 1) { ++fed;}
	    if (DEBUG){fprintf(stderr,"flag: center response to catch trial, hopper up\n");}
	  }
	  else {
	    resp_sel = -1;
	    resp_acc = 2;
	...
	fprintf(datafp, "%d\t%d\t%d\t%d\t%s\t%s\t%1.2f\t%d\t%d\t%d\t%.4f\t%d\t%s\t%s\n", session_num, trial_num, 
		correction, stim_class,stimexm, distexm, target_duration, target, resp_sel, resp_acc, resp_rxt, reinfor, tod, date_out );
	fflush (datafp);
