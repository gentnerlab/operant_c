...
#define SECONDARYRNUM            10           /* secondary reinforcer # flashes */
...
#define DEF_PROBE_REINFORCEMENT          50            /* Default percentage reinforcement of probe trials */

int percent_probe_reinforcement = DEF_PROBE_REINFORCEMENT;
...
int probeRxFR(Failures *f, int *numconsecutivecorrect, int *frcritical)
{
 int outcome=0;
 *numconsecutivecorrect = *numconsecutivecorrect + 1;
 ...
 if (outcome <= percent_probe_reinforcement){
...
/**********************************************************************
 **********************************************************************/
int command_line_parse(int argc, char **argv, int *box_id, int *subjectid, int *starthour, int *stophour, int *startmin, int *stopmin, int *resp_wind, float *timeout_val, char **stimfname, int *frcritical, int *percent_probe_reinf)
{
  int i=0;
  ...
  for (i = 1; i < argc; i++){
	...
	sscanf(argv[++i], "%i", percent_probe_reinf);
      }
	...
	dosecondaryR = 1;
        ...
        fprintf(stderr, "Try '2acgng -help'\n");
      }
    }
    else
      {
...
int main(int argc, char *argv[])
{
	...
	/* Parse the command line */
        command_line_parse(argc, argv, &box_id, &subjectid, &starthour, &stophour, &startmin, &stopmin, &resp_wind, &timeout_val, &stimfname, &frcritical, &percent_probe_reinforcement); 
       	if(DEBUG){
       		fprintf(stderr, "command_line_parse(): box_id=%d, subjectid=%d, startH=%d, stopH=%d, startM=%d, stopM=%d, xresp=%d, cue=%d, resp_wind=%d timeout_val=%f flash=%d stimfile: %s frcritical: %d percent_probe_reinforcement: %d\n",
		box_id, subjectid, starthour, stophour, startmin, stopmin, xresp, cueflag, resp_wind, timeout_val, flash, stimfname, frcritical, percent_probe_reinforcement);
	}
	...
	nstims = nclasses = 0;
	  ...
	  for (i = 0; i < nstims; i++){
	    ...
	    if((stimulus[i].freq==0) || (stimulus[i].reinf==0)|| (stimulus[i].punish==0)){
	      printf("ERROR: insufficnet data or bad format in '.stim' file. Try '2acgng -help'\n");
	      exit(0);} 
	...
	do{                                                                               /* start the main loop */
	  while ((currtime>=starttime) && (currtime<stoptime)){                          /* start main trial loop */
		...
		if ( (left==0) && (right==0) ){ /* no response to probe */
		  ...
		  ++Rstim[stim_number].C;++Tstim[stim_number].C;
		  ++Rclass[stim_class].C; ++Tclass[stim_class].C; 
		  reinfor =  probeRxFR(&f, &numconsecutivecorrect, &frcritical);
		}
		else if (right!=0){
		  resp_sel = 2;
		...
		else{  fprintf(datafp, "DEFAULT SWITCH for bit value: ERROR, CODE REMOVED");
		}
		}
	    else {
		...
		Tclass[i].count = Tclass[i].X + Tclass[i].C;
	      ...
	      /* UPDATE SUMMARY FOR NON-PROBES */
	      if(freopen(dsumfname,"w",dsumfp)!= NULL){
		fprintf (dsumfp, "          SUMMARY DATA FOR st%d, %s\n", subjectid, exp_name); 
		fprintf (dsumfp, "\tPROPORTION CORRECT RESPONSES TO NON-PROBE TRIALS (by stimulus, including correction trials)\n");
		...
		fprintf (dsumfp, "\n\nPROPORTION CORRECT RESPONSES TO NON-PROBE TRIALS (by stim class, including correction trials)\n");
		/*Only the first n-1 classes, nth class reserved for probe*/
		for (i = 1; i< nclasses; ++i){
		...
		/*UPDATE SUMMARY FOR PROBES: C is left, X is right, N is no response*/
		fprintf (dsumfp, "\n\nPROPORTION RESPONSES TO PROBES (by stimulus)");
		fprintf (dsumfp, "\tStim\tLeftPercent\tRightPercent\tNoGoPercent\tTotal     \t\tLeftPercent\tRightPercent\tNoGoPercent\tTotal\n");
		for (i = 0; i<nstims;++i){
			if (stimulus[i].class == 4){
			
			  fprintf (dsumfp, "\t%s\t%1.4f\t%1.4f\t%1.4f\t%d     \t\t%1.4f\t%1.4f\t%1.4f\t%d\n", 
				   stimulus[i].exemplar, 
				   (float)Rstim[i].C/(float)(Rstim[i].count + Rstim[i].N), 
				   (float)Rstim[i].X/(float)(Rstim[i].count + Rstim[i].N), 
				   (float)Rstim[i].N/(float)(Rstim[i].count + Rstim[i].N),
					Rstim[i].count + Rstim[i].N,
				   (float)Tstim[i].C/(float)(Tstim[i].count + Tstim[i].N), 
				   (float)Tstim[i].X/(float)(Tstim[i].count + Tstim[i].N), 
				   (float)Tstim[i].N/(float)(Tstim[i].count + Tstim[i].N),
				Tstim[i].count + Tstim[i].N);
			}
			}
		
	    fprintf (dsumfp, "\n\nPROPORTION RESPONSES OVERALL TO PROBES\n");
		fprintf (dsumfp, "\t%1.4f\t%1.4f\t%1.4f\t%d     \t\t%1.4f\t%1.4f\t%1.4f\t%d\n", 
		   (float)Rclass[4].C/(float)(Rclass[4].count + Rclass[4].N), 
		   (float)Rclass[4].X/(float)(Rclass[4].count + Rclass[4].N), 
		   (float)Rclass[4].N/(float)(Rclass[4].count + Rclass[4].N),
			Rclass[4].count + Rclass[4].N,
		   (float)Tclass[4].C/(float)(Tclass[4].count + Tclass[4].N), 
		   (float)Tclass[4].X/(float)(Tclass[4].count + Tclass[4].N), 
		   (float)Tclass[4].N/(float)(Tclass[4].count + Tclass[4].N),
		Tclass[4].count + Tclass[4].N);
