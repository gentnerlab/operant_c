...
// added options to punish non-responses mb 2/2/10
// add option to not reward correction trials mb/kp 10/27/10
#include <stdio.h>
#include <stdlib.h>
#include <sndfile.h>
#include <math.h>
...
void do_usage()
{
  ...
  fprintf(stderr, "	   -Pun	        = use this flag to punish non-responses, including non-responses for probe trials, \n");
  fprintf(stderr, "			  this also eliminates correction trials. \n");
  fprintf(stderr, "	   -noC         = use this flag to turn off all correction trials.\n");
  fprintf(stderr, "	   -noPun       = use this flag to keep correction trials but not reward them.\n");
...
int command_line_parse(int argc, char **argv, int *box_id, int *subjectid, int *starthour, int *stophour, int *startmin, int *stopmin, int *resp_wind, float *timeout_val, char **stimfname)
{
  ...
  for (i = 1; i < argc; i++){
	...
	xresp = 1;
      else if (strncmp(argv[i], "-Pun", 4) == 0)
	punish_noresp = 1;
      else if (strncmp(argv[i], "-noC", 4) == 0)
	no_correction_flag = 1;
      else if (strncmp(argv[i], "-noPun", 6) == 0)
	noreward_correction = 1;
...
int main(int argc, char *argv[])
{
	...
	if(punish_noresp){fprintf(stderr, "!! WARNING: Punishing non-responses and eliminating correction trials!\n");}
	if(no_correction_flag) {fprintf(stderr, "!! Correction trials turned off\n");}
	...
	do{                                                                               /* start the main loop */
	  while ((currtime>=starttime) && (currtime<stoptime)){                          /* start main trial loop */
		...
		if ( (left==0 ) && (right==0) ){
		  ...
		  if (punish_noresp) {timeout(100); reinfor = 1;}
		  else {reinfor = 0;}
		  ...
		  if (correction == 0 && noreward_correction){
			reinfor = 0;}
 	          else {
		...
		if ( (left==0) && (right==0) ){
		  ...
		  ++Rclass[stim_class].C; ++Tclass[stim_class].C; 
	          if (correction == 0 && noreward_correction){
			reinfor = 0;}
		  else{ reinfor=feed(stim_reinf, &f);
		  if (reinfor == 1) { ++fed;}}
