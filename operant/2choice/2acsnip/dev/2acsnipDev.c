...
void do_usage()
{
	...
	fprintf(stderr, "        		  value:  2 = White noise probes only - Obey motif bouandaries in file /usr/local/stimuli/MotifBoundaries_010509\n");
	fprintf(stderr, "        		  value:  98 = acclimation - add WN on top of song, use -L to specify SNR - Obey motif boundaries as with '-N=2'\n");
	...
	fprintf(stderr, "        -L float      = Optional flag for setting the desired dB level for probe trials if -N is set.  If -N is set to 98,99, specify SNR for mask noise\n");
	fprintf(stderr, "        -P float        = if -N is set to 98,99, this must be provided. specifies the percentage of time that the mask will be played over the song\n");
...
int make_probesnippets(char *sfname, int box_id, int snipoffset, int snipdur, int sniptype, float desired_db)
{
	...
	switch (sniptype){
		...
		case 2: /*do white noise masked probes, where masking obeys motif boundaries*/
			if (DEBUG){printf("\ndonoiseprobe is %d. replacing snippet with white noise - obeying motif bounds\n", sniptype);}
			/*1. load motif file     2. find data for current song    3. pick random motif  4. set snipoffset and snipdur to correct values   5. continue*/
			if (DEBUG){printf("Got to 1\n");}
			if (DEBUG){printf("motifdatafile = %s\n",MOTDATFILE);}

			if ((motdatfp = fopen(MOTDATFILE,"r")) != NULL){

				if (DEBUG){printf("Got to 2\n");}
				char currline[128];
				char *result = NULL;
				int tmpmotex,tmpmotclass,motclass[64];
				char compname[128],stimname[128];
				float tmpmotstart,tmpmotend;
				short numMots = 0;
				long motstart[64],motend[64];
				...
				while(fgets(currline, sizeof currline, motdatfp) != NULL)
				{
					...
					sprintf(compname,"%s%s", STIMPATH, stimname);
					if(DEBUG){printf("compname is: %s\tsfname is: %s\n", compname, sfname);
						printf("%d, %d , %s , %f , %f \n", tmpmotex,tmpmotclass,stimname,tmpmotstart,tmpmotend);
					}
					if(strcmp(compname,sfname)==0){
						motex[numMots]=tmpmotex;
						motclass[numMots]=tmpmotclass;
						motstart[numMots]=(long) ((1000*tmpmotstart)+0.5); /*0.5 necessary for proper float->int conversion*/
						motend[numMots]=(long) ((1000*tmpmotend)+0.5);
						numMots++;
					}
				}
				if(DEBUG){printf("Found %d motifs for stim %s\n", numMots,sfname);}
				fclose(motdatfp);
				if(numMots<=0){printf("numMots = %d\texiting make_probesnippets()\n",numMots);return -1;}
				motind = (int) ((numMots+0.0)*rand()/(RAND_MAX+0.0));

				dursamps = (int) rint((float)sfnew_info.samplerate*(float)(motend[motind]-motstart[motind])/1000.0);
				offsetsamps = (int) rint((float)sfnew_info.samplerate*(float)motstart[motind]/1000.0);

				if(DEBUG){printf("motif exemplar selected is # %d\n", motex[motind]);}
				if(DEBUG){printf("white noise offset  is %lu msec, which is %lu samples\n", motstart[motind], offsetsamps );}
				if(DEBUG){printf("white noise dur  is %lu msec, which is %lu samples\n", motend[motind]-motstart[motind], dursamps);}
				total=0;
				for (i = 0; i<dursamps; i++){;
					tmpobuff[i] = 1000*(((2.0)*rand()/(RAND_MAX+0.0))-1.0);
					total+=tmpobuff[i];
				...
				printf("could not open motif data file: %s",MOTDATFILE);
				break;
			}

		case 98: /*do noise training. play low level white noise over portions of song to acclimate bird to noise-Obey motif boundaries*/
	...
	switch (sniptype){
		case 2:{  
			       return  motex[motind]+20000; /*This will be a code in the data file that a motif was replaced with white noise - this is the exemplar from the 'Motif Boundaries' file*/
		       }
		case 98:{
				return  motex[motind]+90000; /*This will be a code in the data file that a motif was masked with white noise - this is the exemplar from the 'Motif Boundaries' file*/
			}
		default:{
				return 1;
			}
...
int main(int argc, char *argv[])
{
	...
	char fullsfname[128];
	int minoffset=0,maxoffset=-1,mindur=50,maxdur=-1, offset=0, pbdur=-1, donoiseprobe=0,donoiseprobeback=0,dotrain=0,ret=0;
	...
	if((period= setup_pcmdev(pcm_name))<0){
		...
		if (donoiseprobe==1||donoiseprobe==2){
		...
		do{                                                                               /* start the main loop */
			while ((currtime>=starttime) && (currtime<stoptime)){                     /* start main trial loop */
				...
				if (donoiseprobe==99||donoiseprobe==98){
				...
				if((stim_class>2)||(dotrain==1)){
					...
					if (donoiseprobe==0){  /* in here, decode donoiseprobe and call function to make particular stim */
					...
					else { /*going into make_probesnippets which will modify the stimulus in some interesting way*/
						...
						if (DEBUG){
							...
							if (donoiseprobe==2){printf("If returned >20000, indicates successful motif-level white noise replacement\nreturned#-20000 = motif exemplar replaced by noise\n");}
							if (donoiseprobe==98){printf("If returned >90000, indicates successful motif-level white noise masking\nreturned#-90000 = motif exemplar replaced by noise\n");}				
						}
					}
				}
				else{
				...
				do{                                             	/* start correction trial loop */
					...
					if (DEBUG){
						...
						if (ret>20000){donoiseprobeback = donoiseprobe;donoiseprobe = ret;} /*hack donoiseprobe to output which motif was modified by make_probesnippets*/
						...
						if (donoiseprobe==ret){donoiseprobe = donoiseprobeback;ret=0;} /*reset donoiseprobe for next trial*/
