...
int timeout(int rval)
{
	...
	if(do_timeout <= rval){
		fprintf(stderr, "        		  value:  2 = White noise probes only - Obey motif boundaries in file /usr/local/stimuli/MotifBoundaries_010509\n");
		operant_write(box_id, HOUSELT, 0);
		usleep(timeout_duration);
		operant_write(box_id, HOUSELT, 1);
		return (1);
...
void do_usage()
{
	...
	fprintf(stderr, "        		  value:  9           = add WN on top of song at regions defined by -M, use -L to change SNR of noise (default = 2)\n");
...
int main(int argc, char *argv[])
{
	...
	if((period= setup_pcmdev(pcm_name))<0){
		...
		if(DEBUG){printf("Done reading in motif data for stim %s.  Found %d motifs\n",stimulus[i].exemplar, stimmotdat[i].nummots);}
			...
			printf("could not open motif data file: %s.\texiting",MOTDATFILE);
			return -1;
		}
