...
void do_usage()
{
  ...
  fprintf(stderr, "        -wkx         = work off: set min:max for no-work period duration (default is 0:0)\n");
  fprintf(stderr, "        -wko         = work on: set min:max for work period duration (default is 40:60)\n");
...
/**********************************************************************
 **********************************************************************/
int command_line_parse(int argc, char **argv, int *box_id, int *subjectid, int *starthour, int *stophour, int *startmin, int *stopmin, int *wkxmin, int *wkxmax, int *wkomin, int *wkomax, int *resp_wind, float *timeout_val, float *breakdur, float *ptarg,  int *new_trialtarg, float *targ_dB, float *dist_dB, char **stimfname)
{
  int i=0;
  ...
  for (i = 1; i < argc; i++){
	...
	cueflag = 1; 
      ...
      else if (strncmp(argv[i], "-wkx", 4) == 0)
	sscanf(argv[++i], "%i:%i", wkxmin,wkxmax);
      else if (strncmp(argv[i], "-wko", 4) == 0)
	sscanf(argv[++i], "%i:%i", wkomin, wkomax);
...
int main(int argc, char *argv[])
{
  ...
  int nclasses, nstims, stim_class, stim_type, nonmatch_type, stim_reinf, stim_punish, resp_sel, resp_acc, subjectid, period, tot_trial_num, ntargtrials = 1,numtrials,
    played, resp_wind=0,trial_num, session_num, i,j,k, correction, targval,distval, dist_type, loop, stim_number, dist_number, nonmatch_number, cue[3], 
    *playlist=NULL, totnstims=0, mirror=0, dosunrise=0,dosunset=0,starttime,stoptime,currtime, new_trialtarg = 0, nonmatchval, sv, start,
    thisonlen, thisont, thisofflen, thisofft,wkxmin, wkxmax, wkomin, wkomax;
  ...
  /* set up default trial time period probabilities */
  wkxmin = 1;
  wkxmax = 1;
  wkomin = 50;
  wkomax = 70;
  ...
  command_line_parse(argc, argv, &box_id, &subjectid, &starthour, &stophour, &startmin, &stopmin, &wkxmin, &wkxmax, &wkomin, &wkomax, &resp_wind, &timeout_val, &breakdur, &ptarg, &new_trialtarg, &normdB, &distdB, &stimfname); 
  fprintf(stderr, "command_line_parse(): box_id=%d, subjectid=%d, startH=%d, stopH=%d, startM=%d, stopM=%d, xresp=%d, NOxresp=%d, cue=%d, wkxmin = %d, wkxmax = %d, wkomin = %d, wkomax = %d, resp_wind=%d, timeout_val=%f, flash=%d, breakdur=%f, ptarg=%f, trialtarg=%d, targdB= %.4f, distdB = %.4f, fullflag = %d, stimfile: %s\n",box_id, subjectid, starthour, stophour, startmin, stopmin, xresp, NOxresp, cueflag, wkxmin, wkxmax, wkomin,wkomax, resp_wind, timeout_val, flash, breakdur, ptarg, new_trialtarg, normdB, distdB, fullflag, stimfname);
  ...
  sprintf(datafname, "%i.M2S_rDAT", subjectid);
  ...
  ********************************************/
  ...
  thisonlen = rand() % (wkomax-wkomin) + wkomin;
  thisofft = currtime+thisonlen;
  thisofflen = rand() % (wkxmax-wkxmin) + wkxmin;
	...
	while (numtrials < ntargtrials){
	  ...
	  while(currtime < thisofft){
		...
		exit(-1);
	      ...
	      fprintf(datafp, "%d\t%d\t%d\t%s\t%s\t%s\t%d\t%d\t%d\t%.4f\t%.4f\t%d\t%s\t%s\n", session_num, trial_num, 
		      correction, stimexm, nonmatchexm, distexm, stim_class, resp_sel, resp_acc, breakdur, resp_rxt, reinfor, tod, date_out );
	      fflush (datafp);
