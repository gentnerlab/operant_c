...
** 11-03-06 TQG: modified for new probestimulus presentation abilities
*/
...
void do_usage()
{
  ...
  fprintf(stderr, "        -p int         = set the presentation frequency for the probe stimuli");               
  ...
  fprintf(stderr, "                         where each line is: 'Class' 'Sndfile' 'Go_Rf'\n");
...
/**********************************************************************
 **********************************************************************/
int command_line_parse(int argc, char **argv, int *box_id, int *subjectid, int *starthour, int *stophour, int *startmin, int *stopmin, int *prbrate, int *resp_wind, float *timeout_val, char **stimfname)
{
int i=0;
 for (i = 1; i < argc; i++){
   if (*argv[i] == '-'){
...
int main(int argc, char *argv[])
{
  ...
  int dinfd=0, doutfd=0, nclasses, nstims, stim_class, stim_number, stim_reinf, 
    subjectid, loop, period, played, resp_wind,correction,prbrate=0, 
  ...
  struct tm *loctime;
	...
	command_line_parse(argc, argv, &box_id, &subjectid, &starthour, &stophour, &startmin, &stopmin, &prbrate, &resp_wind, &timeout_val, &stimfname);
        if(DEBUG){
          fprintf(stderr, "command_line_parse(): box_id=%d, subjectid=%d, startH=%d, stopH=%d, startM=%d, stopM=%d, proberate=%d, xresp=%d, resp_wind=%d timeout_val=%f flash=%d stimfile: %s\n",
                box_id, subjectid, starthour, stophour, startmin, stopmin, prbrate, xresp, resp_wind, timeout_val, flash, stimfname);
        }
	...
	if(prbrate>100){
	  fprintf(stderr, "FATAL ERROR: probe rate (set with '-p int' must be less than 100");
	  exit (-1);
	  snd_pcm_close(handle);
	}
	...
	nstims = nclasses = nbslstims = nprbstims = 0;
	if ((stimfp = fopen(stimfname, "r")) != NULL){
	  ...
	  for (i = 0; i < nstims; i++){
	    ...
	    stimulus[i].reinf=0;
	    if((ret = sscanf(buf, "%d\%s\%d", &stimulus[i].class, stimulus[i].exemplar, &stimulus[i].reinf)) != 3){
	      printf("ERROR: bad line in '.stim' file. Try 'gng -help'\n");
	      snd_pcm_close(handle);
	      exit(-1);} 
	...
	/* make the baseline and probe stimlists */
	if(DEBUG){printf("flag: making the playlists\n");}
	free(bslplist);
	free(prbplist);
	bslplist = malloc( (nbslstims+1)*sizeof(int) );
	prbplist = malloc( (nprbstims+1)*sizeof(int) );

	i=j=k=0;
	for (i=0;i<nstims; i++){
	  if(stimulus[i].class<=2){
	    bslplist[j]=i;
	    if(DEBUG){printf("value for baseline playlist entry '%d' is '%d'\n", j, i);}
	    j++;
	  }
	  else{
	    prbplist[k]=i;
	    if(DEBUG){printf("value for probe playlist entry '%d' is '%d'\n", k, i);}
	    k++;
	  }
	}
   ...
   ********************************************/
	...
	do{                                                                              /* start the block loop */
	  while ((currtime>=starttime) && (currtime<stoptime)){                          /* start main trial loop */
	    ...
	    /*is this a baseline or probe trial*/
	    do_prb = 1+(int) (100.0*rand()/(RAND_MAX+1.0) ); 
	    if(DEBUG){fprintf(stderr,"d_prb = %d\t rval = %d\n", do_prb, prbrate);}
	    
	    if (do_prb <= prbrate){ /*select a probe stimulus*/

	    }
	    else{                  /*select a baseline stimulus */
	      playval = (int) ((nbslstims+0.0)*rand()/(RAND_MAX+0.0));                     /* select stim exemplar at random */ 
