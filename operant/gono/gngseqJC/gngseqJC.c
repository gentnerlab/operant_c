...
** 11-1-10 JAC: stimulus exemplar is NOT choosen randomly - iterates .stim file
** 11-2-10 JAC: saves last trial to preserve correct running order
** 4-5-11  JAC: ported to vogel from ndege
*/
...
#define MAXSTIM                  10001            /* maximum number of stimulus exemplars */ 
...
#define STIMPATH       "/usr/local/stimuli/grm/"
...
/***********************************************************
* TRIAL CHECKER FUNCTION - PROTECTS POSITION DEPENDENT .STIM
***********************************************************/

int checkTrial(char name[128]) 
{
  FILE *infile;
  infile = fopen(name, "r");
  char str_buf[100];
...
int main(int argc, char *argv[])
{
  ...
  int dinfd=0, doutfd=0, nclasses, nstims, stim_class, stim_number, stim_reinf, 
    ...
    trial_num, session_num, i,j,k, *playlist=NULL, totnstims=0, 
  ...
  struct tm *loctime;
	...
	sprintf(dsumfname, "%i.sumamryDAT", subjectid);

    int playval = checkTrial(dsumfname);
    datafp = fopen(datafname, "a");
        dsumfp = fopen(dsumfname, "w");
	    //trialfp = fopen("lastTrial", "w"); 
   ...
   ********************************************/
	...
	do{                                                                              /* start the block loop */
	  while ((currtime>=starttime) && (currtime<stoptime)){                          /* start main trial loop */
	    ...
	    playval++;                                                     /* iterate to next stim exemplar*/  
