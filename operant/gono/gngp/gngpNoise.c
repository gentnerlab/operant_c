/*****************************************************************************
** gngpNoise.c - code for running go/nogo operant training procedure
******************************************************************************
**
...
void do_usage()
{
  ...
  fprintf(stderr, "                         for gngpNoise, make a new line for each stim you'd like to probe w/white noise and assign it class 3\n");
...
int main(int argc, char *argv[])
{
  ...
  int dinfd=0, doutfd=0, nclasses, nstims, stim_class, stim_number, stim_reinf, 
    ...
    int numpecks=0;
  float resp_rxt=0.0, timeout_val=0.0, dbdesired=60.0; /*default db for white noise snippets*/
	...
	do{                                                                              /* start the block loop */
	  while ((currtime>=starttime) && (currtime<stoptime)){                          /* start main trial loop */
		...
		center = operant_read(box_id, CENTERPECK);   /*get value at center peck position*/		 	       
	      ...
	      if (play_and_peck_wn(fstim, period,stimulus_duration, offset, wn_dur, wn_offset, dbdesired, box_id, trl, ptm, &numpecks)!=1){
