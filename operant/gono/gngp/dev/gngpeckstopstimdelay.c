/*****************************************************************************
** gngp.c - code for running go/nogo operant training procedure
******************************************************************************
**
...
** 10-09-07	DPK: modified gngp.c to allow responding at any time
**
*/
...
#include "/usr/local/src/audioio/dev/audout_peck.c"
...
/*struct timespec stimdelay, *stdel;
 stdel = &stimdelay;*/
...
void do_usage()
{
  fprintf(stderr, "gngp usage:\n");
  fprintf(stderr, "    gngp [-help] [-B int] [-mirrorP] [-t float][-on int:int] [-off int:int] [-x] [-S <subject number>] <filename>\n\n");
  ...
  fprintf(stderr, "        -rf            = flash the center key light while valid start trial time \n");
  ...
  fprintf(stderr, "        -sd int        = set a delay between trial request and stimulus start, max delay is int seconds\n");
  fprintf(stderr, "        -rws float     = set a time after stim onset when bird is allowed to respond\n");
...
/**********************************************************************
 **********************************************************************/
int command_line_parse(int argc, char **argv, int *box_id, int *subjectid, int *starthour, int *stophour, int *startmin, int *stopmin, int *resp_wind, float *timeout_val, float *resp_win_start, int *max_stim_wait, char **stimfname)
{
int i=0;
 for (i = 1; i < argc; i++){
   if (*argv[i] == '-'){
     ...
     else if (strncmp(argv[i], "-rws", 4) == 0)
       sscanf(argv[++i], "%f", resp_win_start);
     else if (strncmp(argv[i], "-sd", 3) == 0){
       stimwait = 1;
       sscanf(argv[++i], "%d", max_stim_wait);}
...
int main(int argc, char *argv[])
{
  ...
  int dinfd=0, doutfd=0, nclasses, nstims, stim_class, stim_number, stim_reinf, 
    subjectid, loop, period=0, resp_wind,correction, 
    ...
    int max_stim_wait = 0;
    int nslperr;
    long int stimdel_sec = 0;
    long int stimdel_nsec = 0;
  float resp_rxt=0.0, timeout_val=0.0,resptime=0.0,resp_win_start=0.0;
  float latitude = 32.82, longitude = 117.14;
  time_t curr_tt, rise_tt, set_tt;
  unsigned long int stimulus_duration, offset;
  struct timeval stimoff, stimonset, poststimresp, resp_window, resp_lag, resp_rt;
  struct timespec stimdelay;/*, *stdel;
			      stdel = &stimdelay;*/
  struct tm *loctime;
  struct PECK trial, *trl;
	...
	command_line_parse(argc, argv, &box_id, &subjectid, &starthour, &stophour, &startmin, &stopmin, &resp_wind, &timeout_val, &resp_win_start, &max_stim_wait, &stimfname);
        if(DEBUG){
          fprintf(stderr, "command_line_parse(): box_id=%d, subjectid=%d, startH=%d, stopH=%d, startM=%d, stopM=%d, xresp=%d, resp_wind=%d timeout_val=%f resp_win_start=%f flash=%d flashpre=%d stimwait=%d, max_stim_wait = %d, stimfile: %s\n",
                box_id, subjectid, starthour, stophour, startmin, stopmin, xresp, resp_wind, timeout_val, resp_win_start, flash, flashpre, stimwait, max_stim_wait,  stimfname);
        }
	...
	if (box_id <= 0){
	  ...
	  fprintf(stderr, "\tERROR: try 'gngpeckstopstim -help' for available options\n");
	  snd_pcm_close(handle);
	  exit(-1);
	}
	...
	if(flashpre){fprintf(stderr, "!!WARNING: Flashing keylights during trial request loop!!\n");}
	...
	if ((stimfp = fopen(stimfname, "r")) != NULL){
	  ...
	  for (i = 0; i < nstims; i++){
	    ...
	    if((stimulus[i].freq==0) || (stimulus[i].reinf==0)){
	      printf("ERROR: insufficnet data or bad format in '.stim' file. Try 'gngp -help'\n");
	      snd_pcm_close(handle);
	      exit(-1);} 
	...
	fprintf (datafp, "Sess#\tTrl#\tTrlTyp\tStimulus\tClass\tRspSL\tRspAC\tReinf\tTOD\tDate\n");
	  ...
	  while ((currtime>=starttime) && (currtime<stoptime)){                          /* start main trial loop */
		...
		center = operant_read(box_id, CENTERPECK);   /*get value at center peck position*/		 	       
		 if((center==0) && flashpre){
	      ...
	      /*wait random delay before starting stimulus*/             
	      if(stimwait==0){
		stimdelay.tv_sec=0;
		stimdelay.tv_nsec=0;
	      } /*no delay*/
	      else{
		stimdel_sec = (long int) ((max_stim_wait+0.0)*rand()/(RAND_MAX+0.0));
		stimdel_nsec = (long int) ((999999999+0.0)*rand()/(RAND_MAX+0.0));
		stimdelay.tv_sec=stimdel_sec;
		stimdelay.tv_nsec=stimdel_nsec;  /*set random stimdelay*/
	      }
	      if (DEBUG){printf("waiting %ld:%ld seconds to play stim. max_stim_wait is: %d\n",stimdel_sec,stimdel_nsec/1000000,max_stim_wait);}
	      if (DEBUG){printf("waiting %ld:%ld seconds to play stim (fromstimdelay struct)\n",stimdelay.tv_sec,stimdelay.tv_nsec/1000000);}
	      nslperr = nanosleep(&stimdelay,NULL);
	      if (DEBUG){printf("nanosleep returned: %d\n",nslperr);}
	      /* Play stimulus file */
	      ...
	      if (play_and_peck_stop(fstim, period,stimulus_duration, offset, box_id, trl, ptm, resp_win_start, &numpecks, &stimonset, &respduringstim)!=1){
		...
		printf("stim_off sec: %d \t usec: %d\n", stimoff_sec, stimoff_usec);}
	      ...
	      /* Wait for center key press*/
	      respafterstim = 0;
	     if (respduringstim == 0) /*if no response during stimulus playback, wait for response*/
	     {
		     if (DEBUG){printf("flag: still waiting for response\n");}
		...
		gettimeofday(&resp_lag, NULL);
		 if((center==0) && flash){
	      ...
	      if(center==1){
		      timersub(&resp_lag, &stimonset, &poststimresp);
		      respafterstim = 1;
		      resptime = (float) poststimresp.tv_sec + ( (float) poststimresp.tv_usec/1000000);
		      /*numpecks=numpecks+1;
		      pecktimes.peckkey[numpecks] = 'C';
		      pecktimes.pecktime[numpecks] = (float) poststimresp.tv_sec + ( (float) poststimresp.tv_usec/1000000);
		      if (DEBUG){printf("numpecks = %d\tpeckkey = %s  \n");}*/
	      }
	    }

	      operant_write (box_id, CTRKEYLT, 0);    /*make sure the key lights are off after resp interval*/
		   
	      /* Calculate response time - Think this can be taken out DK 10/17/07*/
	      curr_tt = time (NULL); 
		...
		if (center==0){ /*no response*/
	      ...
	      fprintf(datafp, "%d\t%d\t%d\t%s\t\t%d\t%d\t%d\t%d\t%s\t%s\n", session_num, trial_num, 
		      correction, stimexm, stim_class, resp_sel, resp_acc, reinfor, tod, date_out );
		      
		if(respduringstim == 1){     
			if(numpecks>2){fprintf(datafp,"\t\t\tnum\tkey\tstimlag\n");}
			for(i=1;i<numpecks-1;i++){
				fprintf(datafp, "\t\tpeck\t%d\t%c\t%0.3f\n",i,pecktimes.peckkey[i],pecktimes.pecktime[i]);
			}
			fprintf(datafp, "\t\tResp\t\t\t%0.3f\n",pecktimes.pecktime[numpecks-1]);
		}
	      
	       else if(respafterstim == 1){
		      if(numpecks>1){fprintf(datafp,"\t\t\tnum\tkey\tstimlag\n");}
			for(i=1;i<numpecks;i++){
				fprintf(datafp, "\t\tpeck\t%d\t%c\t%0.3f\n",i,pecktimes.peckkey[i],pecktimes.pecktime[i]);
			}
		      fprintf(datafp, "\t\tResp\t\t\t%0.3f\n",resptime);
