...
int femxpeck_block_four(int numtrials, int starttime, int stoptime);
...
int main(int ac, char *av[])
{
  ...
  switch(t_proc)
    ...
    case 3:  /* run the femx peck shaping sequence */
      ...
      femxpeck_block_four(100, starttime, stoptime);
      break;
...
int choice_block_one(int starttime, int stoptime)
{
  ...
  do
	...
	} while ( (center == 0) && (right == 0) && (left == 0) && (dur.tv_sec < 5) );  
    ...
    } while ( (center==0) && (left == 0) && (right == 0) && (currtime>=starttime) && (currtime<stoptime) );
...
{ int currtime; 
  ...
  do{
    coin = 1+(int)(30.0*rand()/(RAND_MAX+1.0));
...
int femxpeck_block_four(int numtrials, int starttime, int stoptime)
{
  ...
  feed_duration = 3000000;
  fprintf(datafp,"\nBLOCK 4\n\n");
  fprintf(datafp,"Blk #\tTrl #\tLFT Key\t #RGT KEY\t #Resp Time\n");
  ...
  do{
    do{
      ...
      operant_write (box_id, LFTKEYLT, 0);
      right = 0, left = 0;
      /*make both keys active*/
      do{
        nanosleep(&rsi, NULL);
        target1 = operant_read(box_id, RIGHTPECK);
        target2 = operant_read(box_id, LEFTPECK);
      ...
      /*output trial data - LEFT peck, then right PECK*/
      fprintf(datafp, "%d\t%d\t%d\t%d\t%s\n", block_num, trial_num, target2, target1, ctime(&curr_tt) );
      fflush (datafp);
      sleep(iti);
      ++trial_num;
