  ...
  for (i = 1; i < ac; i++)
      ...
      if (*av[i] == '-')
	{
	  if (strncmp(av[i], "-B", 2) == 0){ 
        ...
        sscanf(av[++i], "%i:%i", startH);}
        else if (strncmp(av[i], "-off", 4) == 0){
        sscanf(av[++i], "%i:%i", stopH);}
...
int perch_block_two(int numtrials)
{
  ...
  time_t perchstart, perchtime, endwait;
  int wait_time;
  iti = 2;
  int perch_wait =2; /*wait 2 seconds on perch before starting trial */
  block_num = 2;
  trial_num = 1;
...
do{
  do{
    perch_check=0;
        ...
        wait_time =(int) rand() % 10 + 1;
        if(DEBUG){printf("Wait time is %i \n", wait_time);}
		do{                                         
			nanosleep(&rsi, NULL);	               	       
		...
		time(&perchstart);
        endwait = perchstart + wait_time;

		do{
			nanosleep(&rsi, NULL);
			perch_check = operant_read(box_id, PERCH);
            time(&perchtime);
            if(DEBUG){printf("perch_check %i, perchtime %i, endwait %i\n", perch_check, perchtime, endwait);}
		}while ((perch_check==1) && (perchtime<=endwait)); 
	...
	if ( loop % 25 == 0 )
      ...
      ++trial_num;
        ...
        if(DEBUG){printf("trial false start...\n");}
        sleep(iti);}
