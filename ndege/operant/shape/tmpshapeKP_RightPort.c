...
**
**
** 05/13/13 TQG modified to run a shaping routine for same-different routine (Meliza, 2011 JASA)
**             Termial Proc: peck CTR while stim plays, peck LTR when stim changes
**             Training sequence:
**                 Block 1:  Hopper comes up on VI (stays up for 5 s) for the first day 
**                           that the animal is in the apparatus. Center key flashes for 5 sec, prior 
**                           to the hopper access. If the center key is pressed while flashing, then 
...
**                           4 sec. Run 100 trials.
**
**                 Block 3:  Center port flashes until pecked, then VR CTR, then the hopper 
**                           comes up for 3 sec. Default is to run 150 trials. 
**      
**                 Block 4:  Center port flashes until pecked, then VR CTR, then left port flashes
**                           until LTR, then food for 2.5 sec. Default is to run 150 trials.
**
** 
**
** 
...
int sd_block_three(int numtrials, int vrmin, int vrmax, int starttime, int stoptime);
int sd_block_four(int numtrials, int vrmin, int vrmax, int starttime, int stoptime);
...
int main(int ac, char *av[])
{
  ...
  int dosunrise=0, dosunset=0,starttime,stoptime,vrmin=2,vrmax=3;
  ...
  for (i = 1; i < ac; i++)
      ...
      if (*av[i] == '-')
	{
	  if (strncmp(av[i], "-B", 2) == 0){ 
	  ...
	  else if (strncmp(av[i], "-VR", 3) == 0)
	    sscanf(av[++i], "%i:%i", &vrmin, &vrmax);
	    ...
	    fprintf(stderr, "                            '-T 5' for same-different\n");
	    ...
	    fprintf(stderr, "     -VR int:int    = set min:max num trials in the variable ratio schedule for SD shape (default is 2:5)\n");
	    exit(-1);
	  }
	  else
	    {
  ...
  if (t_proc == 5){
    fprintf(stderr, "          SAME-DIFF terminal procedure\n\n");}
  ...
  switch(t_proc)
    ...
    case 4:/* run the 3choice shaping sequence */
      ...
      choice_block_four(numtb4, t_proc, starttime, stoptime);
      break;
    case 5:/* run the SD shaping sequence */
      gng_block_one(starttime, stoptime);
      gng_block_two(numtb2, starttime, stoptime);
      sd_block_three(numtb3, vrmin, vrmax, starttime, stoptime);
      sd_block_four(numtb4, vrmin, vrmax, starttime, stoptime);
      break;
    }
...
/* same-diff block 3 */
/* flash CTR, VR min-max, feed*/
int sd_block_three(int numtrials, int vrmin, int vrmax, int starttime, int stoptime)
{
  int target, targetkeylt, targetpeck, targetfeed, currtime, vrcount, vr, vrtmp, vrdiff; 
  char hour[16], min[16];
  ...
  do{
    do{
      ...
      /*flash targetkeylight until its pecked*/
      do{
	    nanosleep(&rsi, NULL);	               	       
	    target = operant_read(box_id, targetpeck);
	    ++loop;
      ...
      target=0;
      operant_write(box_id, targetkeylt, 1);
      /*have target light on while do VR*/
      if(DEBUG){
	printf("target light should be on");}
      /*loop until VR responses detected */
      do{
	...
	do{
	  ...
	  target = operant_read(box_id, targetpeck);
        ...
        vrcount++;
        if(DEBUG){printf("FUCKING VR COUNTER: %d\n" , vrcount);}
      } while (vrcount < vr);
      if(DEBUG){printf("target light should be on");}
      operant_write(box_id, targetkeylt, 0); /*turn off target light when feed*/      
      ...
      /*output trial data */
      fprintf(datafp, "%d\t%d\t%d\t%s\n", block_num, trial_num, vrcount, asctime (loctime) );
      fflush (datafp);
      sleep(iti);
      ++trial_num;
...
int sd_block_four(int numtrials, int vrmin, int vrmax, int starttime, int stoptime)
{
  ...
  do{
    ...
    operant_write(box_id, RGTKEYLT, 1);/*have target light on while do VR*/
   /*loop until VR responses detected */
    if (DEBUG) 
      printf("targetlight should have gone on");
  do{
      do{
         nanosleep(&rsi, NULL);
         target = operant_read(box_id, targetpeck);
  ...
  operant_write(box_id, RGTKEYLT, 0); /*turn off target light when turn on left*/ 
 
      /*flash left key light and wait for response*/
      loop = 0;
      operant_write (box_id, LFTKEYLT, 1);
      target = 0;
