...
**
** 1/30/19 MRB modified to run shaping for change_peck procedure.
**		BLOCK 1: Same as above, hopper stays up for 5s at variable intervals
**		BLOCK 2: Same. Center key flashes until pecked
**		BLOCK 3: Center key flashes, then left key flashes until pecked. Hopper comes up.
**		BLOCK 4: No center key flash, left key flashes. After pecked, right key flashes. Must peck left
**		then right to obtain food reward.
**/
...
int startH = EXP_START_TIME; /* default times unless changed with command line flags */
...
int change_block_three(int numtrials, int tproc);
int choice_block_four(int numtrials, int tproc);
...
int main(int ac, char *av[])
{
      ...
      if (*av[i] == '-')
	{
	  if (strncmp(av[i], "-B", 2) == 0){ 
	    ...
	    fprintf(stderr, "                            '-T 3' for femx_peck, '-T 4' for 3choice\n");
            fprintf(stderr, "                            '-T 5 for change peck procedure\n");
        ...
        fprintf(stderr, " optional: -on x -off x = specify on time and off time in hours (24 hour clock). Use -off 20 to end at 8:00pm");
	    exit(-1);
	  }
	  else
	    {
  ...
  if (t_proc == 5){
    fprintf(stderr, "          CHANGE PECK terminal procedure\n\n");}
  ...
  switch(t_proc)
    ...
    case 4:/* run the 3choice shaping sequence */
      ...
      choice_block_three(numtb3, t_proc);
      choice_block_four(numtb4, t_proc);
    case 5: /* run the change_peck shaping sequence */
      choice_block_one();
      choice_block_two(numtb2);
      change_block_three(numtb3, t_proc);
      change_block_four(numtb4, t_proc);
    }
...
/* change peck block 4 */
int change_block_four(int numtrials, int tproc)
{
  int vi = 1;
  int pausetime =1;
  iti = 2;
...
/*impose a longer variable wait period before switching to right*/
      vi = (rand() % 5) + 2;
      sleep(vi);
