...
int feedVR(int rval, Failures *f, int *numconsecutivecorrect, int *vrcritical, int *vrmean)
{
    *numconsecutivecorrect = *numconsecutivecorrect + 1;
    if(DEBUG){fprintf(stderr,"numconsecutivecorrect = %d\t vrcritical = %d\n", *numconsecutivecorrect, *vrcritical);}
    if (*numconsecutivecorrect >= *vrcritical){

        *numconsecutivecorrect = 0;
        *vrcritical = 1 + rand() / (RAND_MAX / (2 * *vrmean - 1) + 1);
        int feed_me=0;
...
int command_line_parse(int argc, char **argv, int *box_id, int *subjectid, int *starthour, int *stophour, int *startmin, int *stopmin, int *resp_wind, float *timeout_val, 
	char **stimfname, int *seq_length, float *seq_isi_mean, float *seq_isi_stdev, int *vrmean)
{
int i=0;
 for (i = 1; i < argc; i++){
   if (*argv[i] == '-'){
     ...
     else if (strncmp(argv[i], "-VR", 2) == 0)
       sscanf(argv[++i], "%i", vrmean);
 ...
 ********************************************/
	int main(int argc, char *argv[])
	{
		...
		command_line_parse(argc, argv, &box_id, &subjectid, &starthour, &stophour, &startmin, &stopmin, &resp_wind, &timeout_val, &stimfname, &seq_length, &seq_isi_mean, &seq_isi_stdev, &vrmean);
		if(DEBUG){
		  ...
		  if (dosecondaryR == 1){
		    ...
		    for (i=0;i<secondaryRnum;i++){
		      ...
		      usleep(secondaryRusoff);
                  ...
                  reinfor=feedVR(stim_reinf, &f, &numconsecutivecorrect, &vrcritical, &vrmean); 
