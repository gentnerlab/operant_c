...
** 10-07-08 MRB: Adding code to build paired stims on the fly from two
wav files.
*/
...
long unsigned int pause_secs = 11025; /* Time between stimuli in match to sample */
...
void do_usage()
{
  ...
  fprintf(stderr, "        -rep int 	  = set number of repitions prior to sample.\n");
  ...
  fprintf(stderr, "                         where each line is: 'Class' 'Sndfile1' 'Sndfile2' 'Freq' 'Go_Rf'\n");
  fprintf(stderr, "                         'Class'= 1 for S+, 2 for S-, 3 or greater for nondifferential (e.g. probe stimuli) \n");
  fprintf(stderr, "                         'Sndfile1' is the name of the first soundfile (use WAV format 44.1kHz only)\n");
  fprintf(stderr, "                         'Sndfile2' is the name of the second soundfile (use WAV format 44.1kHz only)\n");
  fprintf(stderr, "                         'Freq' is the overall stimulus presentation rate (relative to the other stimuli). \n"); 
...
/**********************************************************************
 **********************************************************************/
int command_line_parse(int argc, char **argv, int *box_id, int *subjectid, int *starthour, int *stophour, int *startmin, int *stopmin, int *resp_wind, float *timeout_val, char **stimfname, int *sample_reps)
{
int i=0;
 for (i = 1; i < argc; i++){
   if (*argv[i] == '-'){
...
if (DEBUG) {fprintf(stderr, "Got to command line parse.\n");}
 return 1;
}
/********************************************
 ...
 ** by a pause.
 ** Changed to repeat sample stimulus several times before probe.
 ********************************************/
...
int main(int argc, char *argv[])
{
  ...
  char datafname[128], hour [16], min[16], month[16], day[16], year[16],  dsumfname[128], stimftemp[128],buf[128], stimexma[128], stimexmb[128], fstima[256], fstimb[256], timebuff[64], tod[256], date_out[256],temphour[16],tempmin[16], buffer[30];
  ...
  struct tm *loctime;
	...
	command_line_parse(argc, argv, &box_id, &subjectid, &starthour, &stophour, &startmin, &stopmin, &resp_wind, &timeout_val, &stimfname, &sample_reps);
        if(DEBUG){
	...
	if ((stimfp = fopen(stimfname, "r")) != NULL){
	  ...
	  for (i = 0; i < nstims; i++){
	    ...
	    sscanf(buf, "%d\%s\%s\%d\%d", &stimulus[i].class, stimulus[i].exemplara, stimulus[i].exemplarb, &stimulus[i].freq, &stimulus[i].reinf);
	...
	fprintf (datafp, "Sess#\tTrl#\tTrlTyp\tStimulusa \tStimulusb\t\t\tClass\tRspSL\tRspAC\tRspRT\tReinf\tTOD\tDate\n");
   ...
   ********************************************/
	...
	return -1;}	
	    ...
	    if(DEBUG){
	      ...
	      printf("stim a name: %s\n", stimexma);
	      printf("full stim a path: %s\n", fstima);
	      printf("full stim b path: %s\n", fstimb);
	      printf("exemplar chosen: %s\tnumber: %d\n", stimulus[stim_number].exemplara, stim_number );
	    }
		...
		fprintf(stderr, "playwav failed on pcm:%s stimfile:%s. Program aborted %s\n", 
			pcm_name, outsfname, asctime(localtime (&curr_tt)) );
