/*****************************************************************************
** gngchanged.c - code for running go/nogo operant training procedure
******************************************************************************
**
...
** 1-19-09 MRB: modifying gngp.c to build repeated background change detection task
*/
 ...
 ** exemplars and builds a temp wav file with repeated background :e
 ** by a pause.
 ********************************************/
...
/*open each motif*/
   ...
   outframes = (long unsigned int)incounta + pause_samps + (long unsigned int)incountb;
	...
	do{                                                                              /* start the block loop */
	  while ((currtime>=starttime) && (currtime<stoptime)){                          /* start main trial loop */
		...
		center = operant_read(box_id, CENTERPECK);   /*get value at center peck position*/		 	       
	      ...
	      if (play_and_peck(fstim, period,stimulus_duration, offset, box_id, trl, ptm, &numpecks)!=1){
