...
int NRxresp = 0;
int XrespON = 1;
int RANdBd = 0; /* if one, randomly assigns distracter volume */
int cueflag = 0;
int fullflag = 0;
...
int feedVR(int rval, Failures *f, int *numconsecutivecorrect, long *vrcritical, float *vr_mean)
{
    *numconsecutivecorrect = *numconsecutivecorrect + 1;
    if(DEBUG){fprintf(stderr,"numconsecutivecorrect = %d\t vrcritical = %ld\n", *numconsecutivecorrect, *vrcritical);}
    if (*numconsecutivecorrect >= *vrcritical){

        *numconsecutivecorrect = 0;
        *vrcritical = (long) (1 + (rand() / (RAND_MAX /(2* *vr_mean-1)+ 1))); /* i.e. if vr_mean is 2 this gives you range 
										    from 1.000 - 3.9999 which is truncated 
										    to 1-3 by (int) x attributes */
	if(DEBUG){fprintf(stderr,"new vrcritical = %ld\n", *vrcritical);}
...
void do_usage()
{
  ...
  fprintf(stderr, "        -t 'x'       = set the timeout duration to 'x' secs (real number, e.g 2.5 )\n");
  fprintf(stderr, "        -w 'x'       = set the response window duration to 'x' secs (integer)\n");
  fprintf(stderr, "        -i 'x'       = set the inter-trial-interval window duration to 'x' secs (real number)\n");
  fprintf(stderr, "        -v 'x'       = set the mean number of correct trials needed for reward (VR)(real number)\n");
  ...
  fprintf(stderr, "        -RdB         = randomly set dB SPL level of distracter stimuli (between 0 and value entered for -dBd)\n"); 
...
/**********************************************************************
 **********************************************************************/
int command_line_parse(int argc, char **argv, int *box_id, int *subjectid, int *starthour, int *stophour, int *startmin, int *stopmin, int *resp_wind, float *timeout_val, float *iti_val, float *vr_mean, float *breakdur, float *ptarg,  int *new_trialtarg, float *targ_dB, float *dist_dB, char **stimfname)
{
  int i=0;
  ...
  for (i = 1; i < argc; i++){
	...
	sscanf(argv[++i], "%f", iti_val);
      }
      else if (strncmp(argv[i], "-v", 2) == 0){     
	sscanf(argv[++i], "%f", vr_mean);
      }
	...
	sscanf(argv[++i], "%f", breakdur);
        ...
        /*fprintf(stderr, "found -pt, set ptarg to %f\t", argv[++i]);*/
	...
	sscanf(argv[++i], "%f", dist_dB);
      else if (strncmp(argv[i], "-RdB", 4) == 0)   
	RANdBd = 1;
...
int main(int argc, char *argv[])
{
  ...
  int nclasses, nstims, stim_class, stim_type, nonmatch_type, stim_reinf, stim_punish, resp_sel, resp_acc, subjectid, period, tot_trial_num, 
    ntargtrials = 1,numtrials,played, resp_wind=0,trial_num, session_num, i,j,k, correction, targval,distval, dist_type, loop, 
    stim_number, dist_number, nonmatch_number, nRANdB=8,cue[3],nreinfloops = 500,distdBval = 1,
  ...
  float timeout_val=0.0, iti_val=0.0, vr_mean=1.0, resp_rxt=0.0, breakdur = 0.5, idealdur = 2.0, normdB = 60.0, 
  ...
  RANdB[0]=0;RANdB[1] = 0; RANdB[2] = 34; RANdB[3] = 41; RANdB[4] = 47; RANdB[5] = 52; RANdB[6] = 56; RANdB[7]= 58.5; RANdB[8] = 60;
  ...
  command_line_parse(argc, argv, &box_id, &subjectid, &starthour, &stophour, &startmin, &stopmin, &resp_wind, &timeout_val, &iti_val, &vr_mean, &breakdur, &ptarg, &new_trialtarg, &normdB, &distdB, &stimfname); 
  fprintf(stderr, "command_line_parse(): box_id=%d, subjectid=%d, startH=%d, stopH=%d, startM=%d, stopM=%d, NRxresp=%d, XrespON=%d, cue=%d, resp_wind=%d, timeout_val=%f, iti_val=%f, vr_mean=%f, flash=%d, breakdur=%f, ptarg=%f, trialtarg=%d, targdB= %.4f, distdB = %.4f, fullflag = %d, stimfile: %s\n",box_id, subjectid, starthour, stophour, startmin, stopmin, NRxresp, XrespON, cueflag, resp_wind, timeout_val, iti_val, vr_mean, flash, breakdur, ptarg, new_trialtarg, normdB, distdB, fullflag, stimfname);
  ...
  if (iti_val > 0.0)
    iti.tv_sec = iti_val-(HOPPER_DROP_MS/1000);
  fprintf(stderr, "inter-trial interval set to %d secs\n", (int) iti.tv_sec);
  if (new_trialtarg > 0)
    ntargtrials = new_trialtarg;
  if (vr_mean < 1){
    fprintf(stderr, "\tmean variable ratio cannot be less than 1 trial! setting value to 1.0\n"); 
    vr_mean = 1.0;}
  if(RANdBd==1){
    fprintf(stderr, "Randomly assigning distracter volume level\n");}
  ...
  if(NRxresp){fprintf(stderr, "!!WARNING: Enabling correction trials for 'No Response' trials!!\n");}
  if(XrespON){fprintf(stderr, "!!WARNING: Enabling correction trials for incorrect responses!!\n");}
  ...
  if ((stimfp = fopen(stimfname, "r")) != NULL){
	...
	exit(0);} 
      ...
      if (vr_mean > 1.0 && stimulus[i].reinf < 100){
	printf("must have 100 reinforcement rate when using variable ratio reinforcement. changing reinf val'\n");
	stimulus[i].reinf = 100;} 
      if(DEBUG){fprintf(stderr, "Punishment rate for %s is set to %d%% for incorrect responses\n", 
  ...
  ********************************************/
  ...
  trial_num = 0;
  tot_trial_num = 0;
  correction = 1; /* your correction trials are coded backwards for some reason (here, correction = 0 means INCORRECT correction trial, correction = 1 means it is NOT a correction trial, correction = 2 means NO RESPONSE correction trial). don't fix them here because you already collected a bunch of data with this variable the way it is. you are correcting for it in matlab code */
  ...
  do{                                                                               /* start the main loop */
    ...
    while ((currtime>=starttime) && (currtime<stoptime)){                          /* start main trial loop */
      ...
      if (DEBUG){printf("main loop: currtime: %d thisonlen: %d thisofft: %d  thisofflen: %d thisont: %d\n",currtime,thisonlen, thisofft, thisofflen, thisont);}
	...
	soundvecout[i] = 0;
      ...
      while ((numtrials < ntargtrials) && (currtime < thisofft) && (currtime >= starttime) && (currtime < stoptime)){
	numtrials ++;
	curr_tt = time(NULL);
	loctime = localtime (&curr_tt);
	strftime (hour, 16, "%H", loctime);
	...
	currtime=(atoi(hour)*60)+atoi(min);
	if (DEBUG){printf("minutes since MIDNIGHT at loop start: %d\t starttime: %d\tstoptime: %d\n",currtime,starttime,stoptime);}
	if (DEBUG){printf("currtime: %d thisonlen: %d thisofft: %d  thisofflen: %d thisont: %d\n",currtime,thisonlen, thisofft, thisofflen, thisont);}
	...
	/*	do{                                              start block loop */
	  ...
	  if (RANdBd == 1){
	    distdBval = (int) ceil(((nRANdB+0.0)*rand()/(RAND_MAX+0.0))); 
	    distdB = RANdB[distdBval];
	  }
	  ...
	  do{                                              /*start correction trial loop */
		...
		if (correction>0){ /*remember, correction==1 means it is NOT a cxn trial, correction = 2 is a NR cxn trial*/
		  reinfor=feedVR(stim_reinf, &f, &numconsecutivecorrect, &vrcritical, &vr_mean);}
		else reinfor=0;/*if cxn trial reset reinfor to zero for log*/
		if (reinfor == 1){ ++fed;} 
		...
		numconsecutivecorrect = 0;
		vrcritical = (long) (1 + (rand() / (RAND_MAX /(2*vr_mean-1)+ 1)));
		if (DEBUG){printf("flag: incorrect response to stimtype 1\n");}
		if (DEBUG){printf("consecutive correct trials back to zero. vcritcal reset randomly to %ld\n", vrcritical);}
	      } 
	      else
		...
		if (correction>0){ /*remember, correction==1 means it is NOT a cxn trial, correction==2 means NR cxn trial*/
		  reinfor=feedVR(stim_reinf, &f, &numconsecutivecorrect, &vrcritical, &vr_mean);}
		else reinfor=0;/*if cxn trial reset reinfor to zero for log*/
		if (reinfor == 1){ ++fed;} 
	    ...
	    fprintf(datafp, "%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%.4f\t%.4f\t%d\t%s\t%s\t%.2f\n", session_num, trial_num, 
		    correction, stim_number, nonmatch_number, dist_number, stim_class, resp_sel, resp_acc, breakdur, resp_rxt, reinfor, tod, date_out,distdB);
	    fflush (datafp);
	    ...
	    if ((resp_acc == 0)&&(XrespON == 1))/*if response is incorrect and cxn trials are turned on*/
	      correction = 0;
	    else
	      correction = 1;                                              /* set correction trial var */
	    if ((NRxresp==1)&&(resp_acc == 2))  /*if no response and noresp cxn is on */
	      correction = 2;                                              /* set correction trial var for no-resp */
	  ...
	  }while ((correction!=1)&&(trial_num<trial_max)&&(currtime>=starttime)&&(currtime<stoptime)); /* correction trial loop */     
	  if(DEBUG){printf("correction trial loop finished, checking time-out period, and time of day");}  
	  /*}while ((trial_num<=trial_max)&&(currtime>=starttime)&&(currtime<stoptime)); */
                         
      } /* time-in loop */
      free(soundvecout);
      if ((currtime >= thisofft)&& (currtime < thisont)){ /*time-out loop */
	if(DEBUG){printf("entering time-out: stopping work period: %d until %d\n ", currtime, thisont);}
	operant_write (box_id, RGTKEYLT, 1);
	...
	if (currtime >= thisont){	  
	  ...
	  thisont = currtime + thisonlen + thisofflen;
	  if (thisont >= stoptime){
	    thisofft = starttime + thisonlen;
	    thisont = starttime + thisonlen + thisofflen;
	    if (DEBUG){printf("setting time-out period for morning\n");}
	  ...
	  if(DEBUG){printf("starting work period: %d until %d\n", currtime, thisofft);}
	}
      } /* end time-out loop */
      curr_tt = time(NULL);
      loctime = localtime (&curr_tt);
