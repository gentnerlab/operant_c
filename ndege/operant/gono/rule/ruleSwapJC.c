...
#define MAXSTIM                  5000            /* maximum number of stimulus exemplars */ 
...
void do_usage()
{
  ...
  fprintf(stderr, "        -w 'x'         = set the block size\n");
...
/**********************************************************************
 **********************************************************************/
int command_line_parse(int argc, char **argv, int *box_id, int *subjectid, int *starthour, int *stophour, int *startmin, int *stopmin, int *blockSize, float *timeout_val, char **stimfname)
{
int i=0;
 for (i = 1; i < argc; i++){
   if (*argv[i] == '-'){
...
int main(int argc, char *argv[])
{
  ...
  struct tm *loctime;
	...
	command_line_parse(argc, argv, &box_id, &subjectid, &starthour, &stophour, &startmin, &stopmin, &blockSize, &timeout_val, &stimfname);
        if(DEBUG){
   ...
   ********************************************/
	...
	do{                                                                              /* start the block loop */
	  while ((currtime>=starttime) && (currtime<stoptime)){                          /* start main trial loop */
        ...
        if (trial_num % blockSize == 0) {
           //cue++;
           cue = (int) ((4+0.0)*rand()/(RAND_MAX+.0));
           cue = cue+1;
        }
      ...
      operant_write(box_id, BLUELT, 0);
        ...
        if (cue == 1) {
             ...
             bottom = 0;
             cueColor = GREENLT;
              operant_write(box_id, cueColor, 1);
        ...
        playval = (int) ((768+0.0)*rand()/(RAND_MAX+0.0));                     /* select stim exemplar at random */ 
        playval = playval + bottom;
		...
		printf("flag: exit value center = %d\n",center);}
	      ...
	      if((stim_class%2) == 1){   /* If S+ stimulus */
		if (center==0){   /*no response*/
	      ...
	      else if((stim_class%2) == 0){  /* If S- stimulus */
		if (center==0){ /*no response*/
		  resp_sel = 0;
		  resp_acc = 1;
		  if (DEBUG){printf("flag: no response to s- stim\n");}
