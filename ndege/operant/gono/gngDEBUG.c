...
#include "/usr/local/src/audioio/audoutDEBUG.c"
...
/* --------- OPERANT VARIABLES ---------- */
#define RESP_INT_SEC             1             /* seconds from simulus end until NORESP is registered  (see below) */
...
#define INTER_TRIAL_INTERVAL     1.0           /* iti in seconds */
	...
	do{                                                                              /* start the block loop */
	  while ((currtime>=starttime) && (currtime<stoptime)){                          /* start main trial loop */
	    ...
	    do{                                             /* start correction trial loop */
	      ...
	      /* commented out to run trials continuously*/
	      //do{                                         
	      //nanosleep(&rsi, NULL);	               	       
		...
		fprintf(stderr, "playwav failed on pcm:%s stimfile:%s returned %d. Program aborted at %s\n", 
			pcm_name, stimexm, played, asctime(localtime (&curr_tt)) );
		fprintf(datafp, "playwav failed on pcm:%s stimfile:%s.returned %d. Program aborted at %s\n", 
			pcm_name, stimexm, played, asctime(localtime (&curr_tt)) );
		
		fclose(datafp);
		fclose(dsumfp);
		free(playlist);
		/*don't quit on playwave error
		  exit(-1);
		*/
	      }
	      fprintf(stderr, "playwav success, returned %d at %s\n",played, asctime(localtime (&curr_tt)) );
