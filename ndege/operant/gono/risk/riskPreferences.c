...
int main(int ac, char *av[])
{
  ...
  sprintf(tonesfNV,"%s1000Hz.wav", STIMPATH);
  strcpy(fstimNV, tonesfNV); 
  srand ( time (0) );	
 
  sprintf(tonesfLV,"%s1500Hz.wav", STIMPATH);
  strcpy(fstimLV, tonesfLV);
  srand ( time (0) ); 
  
  printf(tonesfHV,"%s2000Hz.wav", STIMPATH);
  strcpy(fstimHV, tonesfHV);
  srand ( time (0) ); 
  ...
  block_num = 1;
  foragingRisk(block_num, leftTime, leftProb, rightTime, rightProb, lowTime, lowProb, starttime, stoptime, period, fstimNV, fstimLV, fstimHV);
...
/* foraging risk */
int foragingRisk(int block_num, int leftTime, int leftProb, int rightTime, int rightProb, int lowTime, int lowProb, int starttime, int stoptime, int period, char fstimNV[256], char fstimLV[256], char fstimHV[256]) { 
  int currtime;
  char hour[16], min[16];
  /* update current time */
  curr_tt = time(NULL);
