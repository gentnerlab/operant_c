...
long unsigned make_femxentseq(Seqstim *ps, int stimclass, char *tmpseq, char *outsfname)
{
  ...
  float dohit=0.0;
  char sfname[256], seq[128];//*seq=NULL;;
...
int do_mappings(int state)
{
  ...
  case 1:
    ...
    aud2 = pcm_right;
    xaud = pcm_left;
    if(DEBUG){printf("CURRENT STATE (in case 1) %d\n\n", state);}
    return 2;
    break;
  case 2:
    perch1 = R_PERCH;
    ...
    aud2 = pcm_center;
    xaud = pcm_left;
    if(DEBUG){printf("CURRENT STATE (in case 2) %d\n\n", state);}
    return 1;
    break;
  case 3:
    perch1 = L_PERCH;
  ...
  case 5:
    ...
    aud2 = pcm_center;
    xaud = pcm_right;
    if(DEBUG){printf("CURRENT STATE(in case 5) %d\n\n", state);}
 return 6;
    break;
  case 6:
    perch1 = C_PERCH;
    ...
    aud2 = pcm_left;
    xaud = pcm_right;
    if(DEBUG){printf("CURRENT STATE(in case 6) %d\n\n", state);}
 return 5;
    break;
  default:
  ...
  fprintf(stderr, "aud1:%s, aud2:%s, xaud:%s\n", aud1, aud2, xaud);}
}
...
int main(int argc, char *argv[])
{
  ...
  char *stimfroot, outsf1[256], outsf2[256], tmpExemplar[128];
  const char delimiters[] = " .,;:!-";
  char datafname[128], hour [16], min[16], month[16], year[16], day[16], dsumfname[128], stimftemp[128];
  char  buf[128], stimexm[128],fstim[256],temphour[16],tempmin[16], timebuff[64], tod[256], date_out[256], buffer[30];
