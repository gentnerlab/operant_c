...
int main(int argc, char *argv[])
{
  ...
  char *stimfname = NULL, entseq1[128], entseq2[128], entseq3[128];
  char *stimfroot, outsf1[256], outsf2[256], outsf3[256], tmpExemplar[128], silencesf[1024];
  ...
  unsigned long int tmpDur=0, seq1dur=0, seq2dur=0, seq3dur, stim_dur=0, offset;
  long tot_trial_num;
 ...
 /*check your seqstims struct */
  ...
  strcpy(class[0].name, "Class 3 - cn:");
  strcpy(class[1].name, "CLass 1 - S+:"); 
  strcpy(class[2].name, "Class 2 - S-:");	
  ...
  ********************************************/
  ...
  do{                                                                               /* start the block loop */
	...
	exit(-1);
      ...
      if ((seq3dur = make_femxentseq(pseqstims, 0, entseq3, outsf3)) < 1){
        fprintf(stderr, "ERROR!: problem building probe sequence on box %d trial %d\n", box_id, trial_num+1);
        snd_pcm_close(pcm_left); snd_pcm_close(pcm_center); snd_pcm_close(pcm_right);
        fclose(datafp);fclose(dsumfp);
        exit(-1);
