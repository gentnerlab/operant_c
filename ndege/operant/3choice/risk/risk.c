...
int foragingRisk(int block_num, int leftTime, int leftProb, int rightTime, int rightProb, int lowTime, int lowProb, int starttime, int stoptime);
...
int foragingRisk(int block_num, int leftTime, int leftProb, int rightTime, int rightProb, int lowTime, int lowProb, int starttime, int stoptime) { 
  ...
  fprintf(datafp,"Blk #\tTrl #\tLEFT\tCENTER\tRIGHT\tTIME\tDATE\n");
  ...
  do{
    ...
    condition = 2;
      do {
        ...
        if (atoi(min) % 5 == 0) {
         ...
         nanosleep(&rsi, NULL);
      } while (atoi(hour) != hourMark+2);

    } else if (chance == 3) {
      condition = 3;
      usleep(120*60*feed_duration);
    }
	...
	if ( loop % 25 == 0 ) {
     ...
     if (coin < leftProb) {
      ...
      /*output trial data */
      fprintf(datafp, "%d\t%d\t%d\t%d\t%d\t%s\t%s\n", block_num, trial_num, left, center, right, ctime(&curr_tt), date_out );
      fflush (datafp);
      sleep(iti);
      ++trial_num;
