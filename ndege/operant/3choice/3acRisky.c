...
/**********************************************************************
 **********************************************************************/
int command_line_parse(int argc, char **argv, int *box_id, int *subjectid, int *starthour, int *stophour, int *startmin, int *stopmin, int *resp_wind, float *timeout_val, int *pnoq, int *pgoodq, int * pbadq, int *minlag, int *maxlag, float *minsplT, float *maxsplT, int *initstate, char **stimfname)
{
  int i=0;
...
int main(int argc, char *argv[]){
  ...
  int nclasses, nstims, stim_class, stim_reinf, stim_punish, resp_sel, resp_acc, subjectid, period, tot_trial_num, 
    played, resp_wind=0,trial_num, session_num, i,j,k, correction,playval,stim_number,minlag,maxlag,qcond,cueval,
    *playlist=NULL,totnstims=0,dosunrise=0,dosunset=0,starttime,stoptime,currtime,classtest,doprobes,class_idx,doq,
    wn_spl,wn_dur,UBlag,onsetlag,Qindex, initstate = -1;
  ...
  command_line_parse(argc, argv, &box_id, &subjectid, &starthour, &stophour, &startmin, &stopmin, &resp_wind, &timeout_val, &pnoq, &pgoodq, &pbadq, &minlag, &maxlag, &minsplT, &maxsplT, &initstate, &stimfname); 
  if(DEBUG){
