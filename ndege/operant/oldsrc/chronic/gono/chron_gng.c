...
**               Note that we need the DAC hardware patch on box 5 or 6 (selectabel from the IO panel) 
**               to get the behavioral 
...
int main(int ac, char *av[])
{
	...
	char datafname[128], hour[16], min[16], month[16], stimfname[128], intext[64];
	char day[16], dsumfname[128], stimftemp[128], exmtemp[128], full_stim [128];	
	char buf[128], stimexm[128], input[128],  timebuff[64], tod[256], date_out[256];
	int nstims, stim_class, subjectid, loop, flash, correction, trial_num, i, stimtemp, recycle;
	...
	for (i = 1; i < ac; i++){
	  if (*av[i] == '-'){
	    if (strncmp(av[i], "-B", 2) == 0){ 
	      ...
	      if ( (box_id == 6) || (box_id ==5)){
		fprintf(stderr, "Running in Box '%d'.\n", box_id);
	      }
	      else {
		fprintf(stderr, "ERROR: Box '%d' is not a valid box number\n", box_id);
		fprintf(stderr, "\tchron_gng only runs in box 5 or 6\n");
		...
		fprintf(stderr, "timeout duration set to %d secs\n", (int) timeout_val);
	      ...
	      fprintf(stderr, "        -XX          = use this if you don't want to run correction trials\n");
  ...
  /* Write data file header info */
	...
	printf("Run an epoch? 'Y' or 'Q' to quit\n");
	recycle = 1;
	do{
	  scanf ("%s", intext);
	  if(strncasecmp(intext, "Y", 1) == 0){
	    recycle = 0;
	  }
	  else if(strncasecmp(intext, "Q", 1) == 0){
	    close(client_fd);
	    fclose(datafp);
	    fclose(dsumfp);
	    exit(-1);
	  }
	  else if(strncasecmp(intext, "on", 2) == 0){
	    operant_write (box_id, HOUSELT, 1);    /* turn the houselight on */  
	    recycle = 1;
	  }
	  else if(strncasecmp(intext, "off", 3) == 0){
	    operant_write (box_id, HOUSELT, 0);    /* turn the houselight off */ 
	    recycle = 1;
	  }
	  else{
	    printf("Huh?!\n");
	...
	do{                    /* start of epoch loop */
	  ...
	  operant_write (box_id, HOUSELT, 0);             /* turn the houselight off between epochs */
	  ...
	  sprintf(command_msg ,"reps 1 0 0\n" );            /* make sure saber reps is set to one */
	  if (write(client_fd, command_msg, strlen(command_msg)) == -1)
	    {
	      close(client_fd);
	      client_fd= -1;
	  ...
	  sprintf(command_msg ,"site\n" );            /* reset the pcm seq2 file with 'site' */
	  if (write(client_fd, command_msg, strlen(command_msg)) == -1)
	    {
	      close(client_fd);
	      client_fd= -1;
     ...
     /*  +++++++++++ TRIAL SEQUENCE ++++++++++++++ */
	  ...
	  do{                                                                               /* start the block loop */
	   ...
	   /* Generate some output numbers*/
	    ...
	    stim_number = -1;
	    if(DEBUG){fprintf(stderr,"event num = %d \t epoch_len = %d\n", event_num, epoch_len);}
