...
// DPK 082809:   created 2acdetect from 2acmatch by MB
#include <stdio.h>
#include <stdlib.h>
#include <sndfile.h>
#include <math.h>
...
const char exp_name[] = "2ACdetect";
int box_id = -1;
int flash = 0;
int xresp = 0;
int lrtogether = 0;
int forceunambiguous = 0;
int havegoodstimulus = 0;
int minnum_mots = 1;
int maxnum_mots = 1;
int maxnum_cone = 1;
int maxnum_ctwo = 1;
int maxnum_cthree = 1;
int leftvsright = 0;
int reinf_classone = 100;
int pun_classone = 100;
int reinf_classtwo = 100;
int pun_classtwo = 100;
int reinf_classthree = 100;
int pun_classthree = 100;
int cueflag = 0;
int correction_flag = 0;
char outsfname[256];
long unsigned int pause_samples = 2646; /*Time between exemplars in match to sample corresponds to 60ms at 44.1khz sample rate*/
...
void do_usage()
{
	...
	fprintf(stderr, "        -C           	= use this flag to enable correction trials\n");
	fprintf(stderr, "        -N int:int   	= use this flag to set the range on the number of motifs presented. eg: '-N 1:4' will present b/t 1 and 4 motifs\n");
	fprintf(stderr, "        -A           	= use this flag to allow left and right associated motifs to be presented in the same stimulus\n");
	fprintf(stderr, "        -U           	= use this flag when '-A' is set to force unambiguous trials (there will never be equal number of left, right motifs)\n");
	fprintf(stderr, "                 	the correct reponse will be given by whichever key has more associated motifs presented\n");
	fprintf(stderr, "        -M int:int:int		= use this flag to set the max number of motifs from classes 1:2:3 in a given stimulus\n");
	fprintf(stderr, "                     	eg: '-M 1:1:3' would only allow 1 (or fewer) motifs from classes 1,2 but up to 3 from class 3\n");

	fprintf(stderr, "        -R int:int,int:int,int:int	= use this flag to set the reinforcement and punishment rate for classes 1,2 and 3\n");
	fprintf(stderr, "                      	eg: '-R rnfclass1:punrateclass1;rnfclass2:punrateclass2;rnfclass3:punrateclass3' \n");
	fprintf(stderr, "                   	eg: '-R 80:100;80:100;50:50' NOTE: class3 values will be used whenever the response is ambiguous \n");
	fprintf(stderr, "                  	for example, if an equal number of class1 and class2 motifs are presented, or if there are only class 3 motifs\n");
	...
	fprintf(stderr, "                     	  'Class'= 1 for LEFT-, 2 for RIGHT-key assignment, 3 for irrelevant\n");
...
/**********************************************************************
 **********************************************************************/
int command_line_parse(int argc, char **argv, int *box_id, int *subjectid, int *starthour, int *stophour, int *startmin, int *stopmin, int *resp_wind, int *minnum_mots, int *maxnum_mots, int *maxnum_cone, int *maxnum_ctwo, int *maxnum_cthree, int *reinf_classone, int *pun_classone, int *reinf_classtwo, int *pun_classtwo, int *reinf_classthree, int *pun_classthree, float *timeout_val, char **stimfname)
{
	int i=0;
	...
	for (i = 1; i < argc; i++){
		if (*argv[i] == '-'){
			if (strncmp(argv[i], "-B", 2) == 0)
				...
				sscanf(argv[++i], "%i:%i", minnum_mots, maxnum_mots);
			else if (strncmp(argv[i], "-R", 2) == 0)
				sscanf(argv[++i], "%i:%i,%i:%i,%i:%i", reinf_classone, pun_classone, reinf_classtwo, pun_classtwo, reinf_classthree, pun_classthree);
			else if (strncmp(argv[i], "-M", 2) == 0)
				sscanf(argv[++i], "%i:%i:%i", maxnum_cone, maxnum_ctwo,maxnum_cthree);
...
int buildtempfile(char * stimexma,char * stimexmb)
{
	...
	outframes = (long unsigned int)incounta + pause_samples + (long unsigned int)incountb;
	...
	sprintf(outsfname,"%s2acdetect_tmp_box%d.wav", STIMPATH, box_id);
...
int main(int argc, char *argv[])
{
	...
	char  buf[128], stimexm[128], stimexma[128], stimexmb[128],fstima[256],fstimb[256], timebuff[64], tod[256], date_out[256], buffer[30],temphour[16],tempmin[16];
	int nclasses, nstims, stim_class, stim_reinf, stim_punish, resp_sel, resp_acc, subjectid, period, tot_trial_num,
	played, resp_wind=0,trial_num, session_num, i,j,k, correction, nummots, currnummots, playval, loop, stim_number, cue[3],
	...
	struct stim {
		...
		int num;
	}stimulus[MAXSTIM],currstim[MAXSTIM];
	struct resp{
		int C;
		int X;
		int N;
	...
	command_line_parse(argc, argv, &box_id, &subjectid, &starthour, &stophour, &startmin, &stopmin, &resp_wind, &minnum_mots, &maxnum_mots, &maxnum_cone, &maxnum_ctwo, &maxnum_cthree, &reinf_classone, &pun_classone, &reinf_classtwo, &pun_classtwo, &reinf_classthree, &pun_classthree, &timeout_val, &stimfname);
	if(DEBUG){
		printf("command_line_parse(): box_id=%d, subjectid=%d, startH=%d, stopH=%d, startM=%d, stopM=%d, xresp=%d, cue=%d, corr_flag=%d resp_wind=%d timeout_val=%f flash=%d stimfile: %s minnum_mots:%d maxnum_mots:%d lrtogether:%d maxnum_cone:%d maxnum_ctwo:%d maxnum_cthree:%d reinf_classone:%d pun_classone:%d reinf_classtwo:%d pun_classtwo:%d reinf_classthree:%d pun_classthree: %d\n",box_id, subjectid, starthour, stophour, startmin, stopmin, xresp, cueflag, correction_flag, resp_wind, timeout_val, flash, stimfname, minnum_mots, maxnum_mots, lrtogether, maxnum_cone, maxnum_ctwo, maxnum_cthree, reinf_classone, pun_classone, reinf_classtwo, pun_classtwo, reinf_classthree, pun_classthree);
	}
	...
	if (minnum_mots == 0){
		fprintf(stderr, "\tTERMINAL ERROR: the minimum number of motifs per stimulus must be at least 1\n");
		exit(-1);
	}

	/*make sure you don't get stuck in an endless loop trying to create a stimulus*/
	if (lrtogether == 0){
		if ((maxnum_cone+maxnum_cthree < maxnum_mots) || (maxnum_cone+maxnum_cthree < maxnum_mots)) {
			fprintf(stderr, "\tTERMINAL ERROR: ((maxnum_cone+maxnum_cthree < maxnum_mots) || (maxnum_cone+maxnum_cthree < maxnum_mots)) which makes it possible to get stuck in an infinite loop when trying to create a stimulus\n");
			exit(-1);
		}
	}
	else {
		if (maxnum_cone+maxnum_ctwo+maxnum_cthree < maxnum_mots) {
			fprintf(stderr, "\tTERMINAL ERROR: (maxnum_cone+maxnum_ctwo+maxnum_cthree < maxnum_mots) which makes it possible to get stuck in an infinite loop when trying to create a stimulus\n");
			exit(-1);
		}
	}
...
if(DEBUG){fprintf(stderr, "starthour: %d\tstartmin: %d\tstophour: %d\tstopmin: %d\n", starthour,startmin,stophour,stopmin);}
	...
	/* Initialize box */
	if(DEBUG){
		printf(stderr,"Initializing box %d ...\n", &box_id);
		printf(stderr,"trying to execute setup(%s)\n", pcm_name);
	}
	...
	if ((stimfp = fopen(stimfname, "r")) != NULL){
		...
		for (i = 0; i < nstims; i++){
			...
			sscanf(buf, "%d\%s\%d", &stimulus[i].class, stimulus[i].exemplar, &stimulus[i].freq);
	...
	{
		fprintf(stderr,"Error opening stimulus input file! Try '2ac_detect -help' for proper file formatting.\n");
		snd_pcm_close(handle);
		exit(0);
	}
	fclose(stimfp);
	...
	if (nclasses > 3) {
		fprintf(stderr, "ERROR!: I found more than 3 stimulus classes, what am I supposed to do?!\n");
		snd_pcm_close(handle);
		exit(0);
	}
	/* Don't allow correction trials when probe stimuli are presented */
	/*classes 1,2, and 3 are normal, anything above 3 is a probe to be figured out later*/
	/*if(xresp==1 && nclasses>3){
		fprintf(stderr, "ERROR!: You cannot use correction trials and probe stimuli in the same session.\n  Exiting now\n");
		snd_pcm_close(handle);
		exit(-1);
}
	*/
	if(totnstims < maxnum_mots){
		fprintf(stderr, "ERROR!: We require at least maxnum_mots (%d) number of stimuli. Only %d were found.\n  Exiting now\n",maxnum_mots,totnstims);
		snd_pcm_close(handle);
		exit(-1);
	}
	...
	********************************************/
	...
	do{                                                                               /* start the main loop */
		while ((currtime>=starttime) && (currtime<stoptime)){                          /* start main trial loop */
			...
			/*Create current stimulus*/
			srand(time(0));
			if (DEBUG){printf("minnum_mots: %d",minnum_mots);}
			nummots = minnum_mots + (int) (((maxnum_mots-minnum_mots+1)+0.0)*rand()/(RAND_MAX+0.0)); /*pick a number of motifs between minnum_mots and maxnum_mots*/
			if (DEBUG==2){
				for (i=1;i<=100;i++){
					nummots = minnum_mots + (int) (((maxnum_mots-minnum_mots+1)+0.0)*rand()/(RAND_MAX+0.0));
					printf("nummots: %d\n",nummots);
				}
			}
            
			if (DEBUG) {printf("nummots: %d\n",nummots);}
				havegoodstimulus = 0;
                while (havegoodstimulus == 0) {
                  currnum_cone = 0;
			currnum_ctwo = 0;
			currnum_cthree = 0;
			currnummots = 0;
				while (currnummots < nummots) {
					if (DEBUG){printf("Picking good motifs\ncurrnummots= %d\tnummots: %d\n",currnummots,nummots);}
					playval = (int) ((totnstims+0.0)*rand()/(RAND_MAX+0.0));            /* select stim exemplar at random */

					stim_number = playlist[playval];
					if (DEBUG){printf("playval: %d\tstim_number: %d\tclass: %d\n ", playval,stim_number,stimulus[stim_number].class);}

					if (stimulus[stim_number].class==1){
						if ((currnum_ctwo > 0) && (lrtogether != 1)){if (DEBUG){printf("already have a class 2 motif and lrtogether = %d. skipping stim_number: %d\n", lrtogether,stim_number);}}
						else{
							if (currnum_cone < maxnum_cone){
								currstim[currnummots] = stimulus[stim_number];
								currnum_cone = currnum_cone+1;
								currnummots = currnummots+1;
							}
							else {
								if (DEBUG){printf("already have enough class 1 exemplars (%d, maxnum = %d)\n",currnum_cone,maxnum_cone);}
							}
						}
					}
					else if (stimulus[stim_number].class==2){
						if ((currnum_cone > 0) && (lrtogether != 1)){if (DEBUG){printf("already have a class 1 motif and lrtogether = %d. skipping stim_number: %d\n", lrtogether,stim_number);}}
						else{
							if (currnum_ctwo < maxnum_ctwo){
								currstim[currnummots] = stimulus[stim_number];
								currnum_ctwo = currnum_ctwo+1;
								currnummots = currnummots+1;
							}
							else {
								if (DEBUG){printf("already have enough class 2 exemplars (%d, maxnum = %d)\n",currnum_ctwo,maxnum_ctwo);}
							}
						}
					}
					else if (stimulus[stim_number].class==3){
						if (currnum_cthree < maxnum_cthree){
							currstim[currnummots] = stimulus[stim_number];
							currnum_cthree = currnum_cthree+1;
							currnummots = currnummots+1;
						}
						else {
							if (DEBUG){printf("already have enough class 3 exemplars (%d, maxnum = %d)\n",currnum_cthree,maxnum_cthree);}
						}
					}
					else {
				...
				/*check class here then set havegoodstimulus*/
				currnummots = 0;
				leftvsright = 0;
				if (nummots == 1) {
					stim_class = currstim[0].class;
				}
				else {
					while (currnummots < nummots) { /* do this nummots times*/	
							if (currstim[currnummots].class == 1) {
								leftvsright = leftvsright + 1;
							}
							else if (currstim[currnummots].class == 2) {
								leftvsright = leftvsright - 1;
							}
							else {}
						currnummots = currnummots + 1;
					if (DEBUG) {printf("nummots: %d\tcurrnummots: %d\tlvr: %d\n",nummots,currnummots,leftvsright);}	
                    }
                    
					if (DEBUG) {printf("LVR FINAL = %d\n",leftvsright);}
                    
                    if (leftvsright > 0) {stim_class = 1;}
					else if (leftvsright < 0) {stim_class = 2;}
					else {stim_class = 3;}
				}

				if ((stim_class == 3) && (forceunambiguous == 1)) {
					havegoodstimulus = 0;
				if (DEBUG) {printf("hgs = 0\tforceunambig = %d\tclass = %d\n",forceunambiguous,stim_class);}
                }
				else {
					havegoodstimulus = 1;
				if (DEBUG) {printf("hgs = 1\tforceunambig = %d\tclass = %d\n",forceunambiguous,stim_class);}
                }
			}
			if (DEBUG){printf("done picking motifs, now building stimulus file\n");}
			currnummots = 0;
			if (nummots == 1) { /*don't deal with the concatenation, figuring out class etc if there's only one motif*/
				if (DEBUG){printf("nummots: %d, not doing any concatenation or figuring of class - using those from current exemplar\n",nummots);}
				sprintf(outsfname,"%s%s", STIMPATH, currstim[0].exemplar);                                /* add full path to file name */
				sprintf(stimexm,"%s", currstim[0].exemplar);
			}
			else { /*concat the individual motifs together*/
				while (currnummots < nummots-1) { /* do this nummots-1 times*/
					if (DEBUG){printf("now concatenating motifs together. currnummots= %d\tnummots: %d\n",currnummots,nummots);}

					if (currnummots == 0) {
						strcpy (stimexma, currstim[currnummots].exemplar);                       /* get exemplar filename */
						sprintf(fstima,"%s%s", STIMPATH, stimexma);                               /* add full path to file name */
					}
					else {
						strcpy (stimexma,stimexm);
						strcpy (fstima, outsfname);
					}
					strcpy (stimexmb, currstim[currnummots+1].exemplar);                       /* get exemplar filename */
					sprintf(stimexm,"%s+%s", stimexma,stimexmb);
					sprintf(fstimb,"%s%s", STIMPATH, stimexmb);                                /* add full path to file name */
					if (buildtempfile(fstima, fstimb)!=1) {				           /*concatenate exemplars together*/
						printf("Building stim file failed.\n");
						return -1;
					}
					currnummots = currnummots + 1;
				}
			...
			if (stim_class == 1) {
				stim_reinf = reinf_classone;
				stim_punish = pun_classone;
			}
			else if (stim_class == 2) {
				stim_reinf = reinf_classtwo;
				stim_punish = pun_classtwo;
			}
			else if (stim_class == 3) {
				stim_reinf = reinf_classthree;
				stim_punish = pun_classthree;
			}
			else{
				fprintf(stderr, "Unrecognized stimulus class: Fatal Error");
				fclose(datafp);
			...
			if (DEBUG){printf("playing stimulus: %s\tfrom file: %s",stimexm,outsfname);}
			...
			do{                                             /* start correction trial loop */
				...
				if ((played = playwav(outsfname, period))!=1){
			...
			if (DEBUG){printf("flag: summaries updated\n");}
				...
				if (DEBUG) {printf("correction trial is now set to %d, correction flag=%d\n", correction, correction_flag);}
		...
		/* You'll end up here if its night time or if you skip a day
		   ...
		   and the time its actually played--i.e. if the subject doesn't initiate a cued trial during 
