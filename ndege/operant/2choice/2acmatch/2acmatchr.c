...
// delayed match to sample code mb//
// requires two stimulus exemplars, and builds file on the fly.//
// added option to repeat sample before match stimulus 12/10/08 mb//
#include <stdio.h>
#include <stdlib.h>
#include <sndfile.h>
#include <math.h>
...
int cueflag = 0;
char outsfname[256];
long unsigned int pause_secs = 8820; /*Time between exemplars in samples for match to sample*/
int sample_reps = 1; /* Repetitions of sample before probe trial */
...
void do_usage()
{
  ...
  fprintf(stderr, "        -rep int       = specify the number of repetitions.\n");
...
/**********************************************************************
 **********************************************************************/
int command_line_parse(int argc, char **argv, int *box_id, int
*subjectid, int *starthour, int *stophour, int *startmin, int *stopmin,
int *resp_wind, float *timeout_val, char **stimfname, int *sample_reps)
{
  int i=0;
...
int main(int argc, char *argv[])
{
	...
	/* Parse the command line */
        command_line_parse(argc, argv, &box_id, &subjectid, &starthour,
&stophour, &startmin, &stopmin, &resp_wind, &timeout_val, &stimfname,
&sample_reps); 
       	if(DEBUG){
	  fprintf(stderr, "command_line_parse(): box_id=%d, subjectid=%d, startH=%d, stopH=%d, startM=%d, stopM=%d, xresp=%d, cue=%d, resp_wind=%d timeout_val=%f flash=%d stimfile: %s\n",
		box_id, subjectid, starthour, stophour, startmin, stopmin, xresp, cueflag, resp_wind, timeout_val, flash, stimfname);
	}
