...
// added option -noC to eliminate correction trials mb 1/16/09
#include <stdio.h>
#include <stdlib.h>
#include <sndfile.h>
#include <math.h>
...
fprintf(stderr, "        -noC           = use this flag to disable correction trials\n");
...
/**********************************************************************
 **********************************************************************/
int command_line_parse(int argc, char **argv, int *box_id, int *subjectid, int *starthour, int *stophour, int *startmin, int *stopmin, int *resp_wind, float *timeout_val, int *correction_flag, char **stimfname)
{
  int i=0;
...
/*open each motif*/
   ...
   outframes = (long unsigned int)incounta + pause_secs + (long unsigned int)incountb;
...
int main(int argc, char *argv[])
{
	...
	/* Parse the command line */
        command_line_parse(argc, argv, &box_id, &subjectid, &starthour, &stophour, &startmin, &stopmin, &resp_wind, &timeout_val, &correction_flag, &stimfname); 
       	if(DEBUG){
	  fprintf(stderr, "command_line_parse(): box_id=%d, subjectid=%d, startH=%d, stopH=%d, startM=%d, stopM=%d, xresp=%d, cue=%d, resp_wind=%d timeout_val=%f flash=%d correction=%d stimfile: %s ",
		box_id, subjectid, starthour, stophour, startmin, stopmin, xresp, cueflag, resp_wind, timeout_val, flash, correction_flag, stimfname );
	}
	...
	do{                                                                               /* start the main loop */
	  while ((currtime>=starttime) && (currtime<stoptime)){                          /* start main trial loop */
		...
		fprintf(stderr, "ERROR!: problem re-opening summary output file!: %s\n", dsumfname);
          ...
          if (DEBUG){printf("About to do end of trial chores. correction flag=%d\n.", correction_flag);}
		...
		correction = 0;}/* set correction trial var for no-resp */
          else if(correction_flag == 1) {correction = 1;}
