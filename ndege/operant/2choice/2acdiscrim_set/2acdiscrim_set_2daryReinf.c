...
void do_usage()
{
	...
	fprintf(stderr, "        -SR          = use this flag to enable the secondary reinforcer. will only work in appropriately modified boxes.\n");
...
int main(int argc, char *argv[])
{
	...
	if(DEBUG){
		fprintf(stderr, "command_line_parse(): box_id=%d, subjectid=%d, startH=%d, stopH=%d, startM=%d, stopM=%d, xresp=%d, cue=%d, dosecondaryR=%d, corr_flat=%d resp_wind=%d timeout_val=%3.3f numOTpairs=%d numTpairs=%d numUTpairs=%d freqOT=%d freqT=%d freqUT=%d reinf_OT=%d pun_OT=%d reinf_T=%d pun_T=%d reinf_UT=%d pun_UT=%d usepreviousset=%d flash=%d stimfile: %s ",
		        box_id, subjectid, starthour, stophour, startmin, stopmin, xresp, cueflag, dosecondaryR,correctiontrialsoff, resp_wind, timeout_val, numOTpairs, numTpairs, numUTpairs, freqOT, freqT, freqUT, reinf_OT, pun_OT, reinf_T, pun_T, reinf_UT, pun_UT, usepreviousset, flash, stimfname );
	}
...
if(DEBUG){fprintf(stderr, "starthour: %d\tstartmin: %d\tstophour: %d\tstopmin: %d\n", starthour,startmin,stophour,stopmin);}
	...
	fprintf (datafp, "resp_wind: %d\ttimeout_val: %3.2f\txresp: %d\tcorrectiontrialsoff: %d\tflash: %d\tcueflag: %d\tdosecondaryR: %d\n", resp_wind,timeout_val,xresp,correctiontrialsoff,flash,cueflag,dosecondaryR);
							...
							if (dosecondaryR == 1){
								if (resp_acc == 1) {
									...
									for (i=0;i<secondaryRnum;i++){
										...
										usleep(secondaryRuson*1000);
										operant_write(box_id, cue[2], 0);
										usleep(secondaryRusoff*1000);
									}
								}
								else{
