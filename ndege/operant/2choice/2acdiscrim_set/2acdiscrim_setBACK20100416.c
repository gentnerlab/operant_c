...
#define TRIALSONMINUTES		1400
#define TRIALSOFFMINUTES	1
...
void do_usage()
{
	...
	fprintf(stderr, "        -onM int     = set on minutes for trial scheduling (trials available for onM, then unavailable for offM then available for onM etcetc)\n");
	fprintf(stderr, "        -offM int    = set off minutes for trial scheduling (trials available for onM, then unavailable for offM then available for onM etcetc)\n");
...
/**********************************************************************
 **********************************************************************/
int command_line_parse(int argc, char **argv, int *box_id, int *subjectid, int *starthour, int *stophour, int *startmin, int *stopmin, int *trialsonminutes, int *trialsoffminutes, int *resp_wind, float *timeout_val, int *numOTpairs, int *numTpairs, int *numUTpairs, int *freqOT, int *freqT, int *freqUT, int *reinf_OT, int *pun_OT, int *reinf_T, int *pun_T, int *reinf_UT, int *pun_UT, char **stimfname)
{
	int i=0;
	...
	for (i = 1; i < argc; i++){
		if (*argv[i] == '-'){
			if (strncmp(argv[i], "-B", 2) == 0)
				...
				sscanf(argv[++i], "%i", trialsonminutes);
						else if (strncmp(argv[i], "-offM", 5) == 0)
				sscanf(argv[++i], "%i", trialsoffminutes);
...
int main(int argc, char *argv[])
{
	...
	int addedstim=0;
	int trialson = 1;
	int trialsonminutes = TRIALSONMINUTES;
	int trialsoffminutes = TRIALSOFFMINUTES;
	...
	command_line_parse(argc, argv, &box_id, &subjectid, &starthour, &stophour, &startmin, &stopmin, &trialsonminutes, &trialsoffminutes, &resp_wind, &timeout_val, &numOTpairs, &numTpairs, &numUTpairs, &freqOT, &freqT, &freqUT, &reinf_OT, &pun_OT, &reinf_T, &pun_T, &reinf_UT, &pun_UT, &stimfname);
	if(DEBUG){
		fprintf(stderr, "command_line_parse(): box_id=%d, subjectid=%d, startH=%d, stopH=%d, startM=%d, stopM=%d, trialsonminutes=%d, trialsoffminutes=%d, xresp=%d, cue=%d, dosecondaryR=%d, corr_flat=%d resp_wind=%d timeout_val=%3.3f numOTpairs=%d numTpairs=%d numUTpairs=%d freqOT=%d freqT=%d freqUT=%d reinf_OT=%d pun_OT=%d reinf_T=%d pun_T=%d reinf_UT=%d pun_UT=%d usepreviousset=%d flash=%d stimfile: %s ",
		        box_id, subjectid, starthour, stophour, startmin, stopmin, trialsonminutes, trialsoffminutes, xresp, cueflag, dosecondaryR,correctiontrialsoff, resp_wind, timeout_val, numOTpairs, numTpairs, numUTpairs, freqOT, freqT, freqUT, reinf_OT, pun_OT, reinf_T, pun_T, reinf_UT, pun_UT, usepreviousset, flash, stimfname );
	}
...
if(DEBUG){fprintf(stderr, "starthour: %d\tstartmin: %d\tstophour: %d\tstopmin: %d\n", starthour,startmin,stophour,stopmin);}
	...
	fprintf (datafp, "trialsonminutes: %d\ttrialsoffminutes: %d\n", trialsonminutes, trialsoffminutes);
			...
			do{ /* start loop for one set */
				if (trialson == 1){
					...
					if (nexttrialsofftime < 0){ /*first trial of new trialson session*/
						...
						nexttrialsofftime = currtime + trialsonminutes;
					...
					if(nexttrialsontime<0){
						...
						nexttrialsontime = currtime + trialsoffminutes;
