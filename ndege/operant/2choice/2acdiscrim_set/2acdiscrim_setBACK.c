...
#define CRITERION_CORRECT_OT	0	/* Number correct (out of previous 10 trials) necessary to move to next set */
#define CRITERION_CORRECT_T	0	/* Number correct (out of previous 10 trials) necessary to move to next set */
#define CRITERION_CORRECT_UT	7	/* Number correct (out of previous 10 trials) necessary to move to next set */
#define MAX_TRIALS		5000	/* Max trials before moving on if criterion not reached */
...
/**********************************************************************
 **********************************************************************/
int command_line_parse(int argc, char **argv, int *box_id, int *subjectid, int *starthour, int *stophour, int *startmin, int *stopmin, int *resp_wind, float *timeout_val, int *numOTpairs, int *numTpairs, int *numUTpairs, int *freqOT, int *freqT, int *freqUT, int *reinf_OT, int *pun_OT, int *reinf_T, int *pun_T, int *reinf_UT, int *pun_UT, char **stimfname)
{
	int i=0;
...
int main(int argc, char *argv[])
{
	...
	played, resp_wind=0,trial_num, session_num, sessionTrials=0, i,j,k, correction, playval, loop, stim_number, cue[3], *playlist=NULL, *setplaylist=NULL, totnstims=0, mirror=0, dosunrise=0,dosunset=0,starttime,stoptime,currtime, stimset_plays, eff_class[MAXCLASS], sum_check;
	int last_10_OT[10],last_10_T[10],last_10_UT[10];
	int curr_cycle_position_OT,curr_cycle_position_T,curr_cycle_position_UT;
	int criterion_reached, criterion_reached_OT, criterion_reached_T, criterion_reached_UT;
	...
	command_line_parse(argc, argv, &box_id, &subjectid, &starthour, &stophour, &startmin, &stopmin, &resp_wind, &timeout_val, &numOTpairs, &numTpairs, &numUTpairs, &freqOT, &freqT, &freqUT, &reinf_OT, &pun_OT, &reinf_T, &pun_T, &reinf_UT, &pun_UT, &stimfname);
	if(DEBUG){
		fprintf(stderr, "command_line_parse(): box_id=%d, subjectid=%d, startH=%d, stopH=%d, startM=%d, stopM=%d, xresp=%d, cue=%d, corr_flat=%d resp_wind=%d timeout_val=%3.3f numOTpairs=%d numTpairs=%d numUTpairs=%d freqOT=%d freqT=%d freqUT=%d reinf_OT=%d pun_OT=%d reinf_T=%d pun_T=%d reinf_UT=%d pun_UT=%d flash=%d stimfile: %s ",
		        box_id, subjectid, starthour, stophour, startmin, stopmin, xresp, cueflag, correctiontrialsoff, resp_wind, timeout_val, numOTpairs, numTpairs, numUTpairs, freqOT, freqT, freqUT, reinf_OT, pun_OT, reinf_T, pun_T, reinf_UT, pun_UT, flash, stimfname );
	}
	sprintf(&pcm_name, "dac%i", box_id);
	/*sprintf(pcm_name, "dac%i", box_id); */
/*    strcpy(pcm_name_p,pcm_name);*/
    /*sprintf(pcm_name_p, "dac%i", box_id);*/
...
if(DEBUG){fprintf(stderr, "starthour: %d\tstartmin: %d\tstophour: %d\tstopmin: %d\n", starthour,startmin,stophour,stopmin);}
	...
	if ((stimfp = fopen(stimfname, "r")) != NULL){
		...
		for (i = 0; i < nstims; i++){
			...
			sscanf(buf, "%s\%d\%d\%d\%d\%d\%d", stimulus[i].exemplar, &stimulus[i].class, &stimulus[i].freq, &stimulus[i].ntrials, &stimulus[i].corr, &stimulus[i].incorr, &stimulus[i].noresp);
	...
	fprintf (datafp, "resp_wind: %d\ttimeout_val: %3.2f\txresp: %d\tcorrectiontrialsoff: %d\tflash: %d\tcueflag: %d\n", resp_wind,timeout_val,xresp,correctiontrialsoff,flash,cueflag);
...
fprintf (datafp, "pun_OT: %d\tpun_T: %d\tpun_UT: %d\n",pun_OT,pun_T,pun_UT);	
	...
	********************************************/
	...
	/*TODO:figure out desired criterion*/
	...
	free(setplaylist);
	setplaylist = malloc( (numOTpairs*freqOT+numTpairs*freqT+numUTpairs*freqUT+1)*sizeof(int) );
	...
	do{ /* start the main loop */
		while ((currtime>=starttime) && (currtime<stoptime)){ /* start trials loop */
			...
			while (havegoodset == 0) {
				...
				case -1: { /*never before presented*/
						if (setcontents[5] != setrequirements[5]){
							currset[setstimnum].class = 5; /*Undertrained Left exemplar - will now be 'training'*/
							stimulus[rndstim].class = 3; /*Undertrained Left exemplar - will now be 'training'*/
							...
							if (setcontents[6] != setrequirements[6]){
								...
								stimulus[rndstim].class = 4; /*Undertrained Right exemplar - will now be 'training'*/
			...
			fprintf (datafp, "File name: %s [This is for parsing: ND_readrdat_2ac uses 'Fi.e name' and 'Da.e' as delimiters (minus dots)]\n", datafname);
			...
			stimset_plays = 0;
			criterion_reached = 0;
			criterion_reached_OT = 0;
			criterion_reached_T = 0;
			criterion_reached_UT = 0;
			curr_cycle_position_OT = 0;
			curr_cycle_position_T = 0;
			curr_cycle_position_UT = 0;
			/* reset last_10 arrays */
			for (j=0; j<10; j++){
				last_10_OT[j] = 0;
				last_10_T[j] = 0;
				last_10_UT[j] = 0;
			...
			do{ /* start loop for one set */
				/* choose one of the stimuli at random */
				...
				if (DEBUG){resp_sec = resp_rt.tv_sec;}
					...
					if ((stim_class == 1) || (stim_class == 2)) {
						last_10_OT[curr_cycle_position_OT] = resp_acc; /* keep track for history */
					}
					else if ((stim_class == 3) || (stim_class == 4)) {
						last_10_T[curr_cycle_position_T] = resp_acc; /* keep track for history */
					}
					else if ((stim_class == 5) || (stim_class == 6)) {
						last_10_UT[curr_cycle_position_UT] = resp_acc; /* keep track for history */
					}
				...
				if (DEBUG){printf("flag: summaries updated\n");}
					...
					if ((stimfp = fopen(stimfname, "w")) != NULL){
						for (i=0;i<nstims; i++){
							fprintf (stimfp, "%s\t%d\t%d\t%d\t%d\t%d\t%d\n", stimulus[i].exemplar, stimulus[i].class, stimulus[i].freq, stimulus[i].ntrials, stimulus[i].corr, stimulus[i].incorr, stimulus[i].noresp);
						}
					}
					else
					{
				...
				if ((criterion_reached_OT==1) && (criterion_reached_T==1) && (criterion_reached_UT==1)){
					if(DEBUG){printf("CRITERION REACHED for current set\n");}
					criterion_reached = 1;}

				if (curr_cycle_position_OT == 9) {curr_cycle_position_OT = 0;}
				if (curr_cycle_position_T == 9) {curr_cycle_position_T = 0;}
				if (curr_cycle_position_UT == 9) {curr_cycle_position_UT = 0;}

			}while((criterion_reached == 0) && (stimset_plays < MAX_TRIALS) && ((currtime>=starttime) && (currtime<stoptime))); /*Set/Session Loop*/


		if(DEBUG){fprintf(stderr,"Exiting Current Set Loop - Going to sleep or going to make new set\n");}
