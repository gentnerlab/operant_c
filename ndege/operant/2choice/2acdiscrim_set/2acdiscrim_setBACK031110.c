...
int main(int argc, char *argv[])
{
	...
	played, resp_wind=0,trial_num=1, session_num=1, i,j,k, correction, playval, loop, stim_number, cue[3], *playlist=NULL, *setplaylist=NULL, totnstims=0, mirror=0, dosunrise=0,dosunset=0,starttime,stoptime,currtime, stimset_plays, eff_class[MAXCLASS], sum_check;
	int last_10_OT[10],last_10_T[10],last_10_UT[10];
	int curr_cycle_position_OT,curr_cycle_position_T,curr_cycle_position_UT,setupset=1;
	int criterion_reached, criterion_reached_OT, criterion_reached_T, criterion_reached_UT,someOT=0,someT=0,someUT=0;
	...
	if(DEBUG){
		fprintf(stderr, "command_line_parse(): box_id=%d, subjectid=%d, startH=%d, stopH=%d, startM=%d, stopM=%d, xresp=%d, cue=%d, corr_flat=%d resp_wind=%d timeout_val=%3.3f numOTpairs=%d numTpairs=%d numUTpairs=%d freqOT=%d freqT=%d freqUT=%d reinf_OT=%d pun_OT=%d reinf_T=%d pun_T=%d reinf_UT=%d pun_UT=%d usepreviousset=%d flash=%d stimfile: %s ",
		        box_id, subjectid, starthour, stophour, startmin, stopmin, xresp, cueflag, correctiontrialsoff, resp_wind, timeout_val, numOTpairs, numTpairs, numUTpairs, freqOT, freqT, freqUT, reinf_OT, pun_OT, reinf_T, pun_T, reinf_UT, pun_UT, usepreviousset, flash, stimfname );
	}
		...
		while ((currtime>=starttime) && (currtime<stoptime)){ /* start trials loop */
			...
			if (setupset == 1) {
				if (usepreviousset == 0) {
					...
					if (numUTpairs == 0) {criterion_reached_UT = 1;}
					else {criterion_reached_UT = 0;}
					setcontents[1] = 0;
					setcontents[2] = 0;
					setcontents[3] = 0;
					setcontents[4] = 0;
					...
					if (someUT == 0){criterion_reached_UT=1;}
					else {criterion_reached_UT = 0;}
				}
			...
			do{ /* start loop for one set */
				...
				if (curr_cycle_position_OT == 10) {curr_cycle_position_OT = 0;}
				if (curr_cycle_position_T == 10) {curr_cycle_position_T = 0;}
				if (curr_cycle_position_UT == 10) {curr_cycle_position_UT = 0;}
