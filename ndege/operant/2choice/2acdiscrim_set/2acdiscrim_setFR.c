...
int feedFR(int rval, Failures *f, int *numconsequtivecorrect, int *frcritical)
{
	*numconsequtivecorrect = *numconsequtivecorrect + 1;
			if(DEBUG){fprintf(stderr,"numconsequtivecorrect = %d\t frcritical = %d\n", *numconsequtivecorrect, *frcritical);}
	if (*numconsequtivecorrect >= *frcritical){

			*numconsequtivecorrect = 0;
      int feed_me=0;
...
void do_usage()
{
	...
	fprintf(stderr, "        -FR int      = set the fixed ratio parameters. Bird must get int consequtive trials correct before it is fed (default is 1)\n");
...
int main(int argc, char *argv[])
{
	...
	if(DEBUG){
		fprintf(stderr, "command_line_parse(): box_id=%d, subjectid=%d, startH=%d, stopH=%d, startM=%d, stopM=%d, trialsonminutesmin=%d, trialsonminutesmax=%d, trialsoffminutesmin=%d, trialsoffminutesmax=%d, xresp=%d, cue=%d, dosecondaryR=%d, corr_flat=%d resp_wind=%d timeout_val=%3.3f numOTpairs=%d numTpairs=%d numUTpairs=%d freqOT=%d freqT=%d freqUT=%d reinf_OT=%d pun_OT=%d reinf_T=%d pun_T=%d reinf_UT=%d pun_UT=%d frcritical=%d usepreviousset=%d flash=%d stimfile: %s ",
		        box_id, subjectid, starthour, stophour, startmin, stopmin, trialsonminutesmin, trialsonminutesmax, trialsoffminutesmin, trialsoffminutesmax, xresp, cueflag, dosecondaryR,correctiontrialsoff, resp_wind, timeout_val, numOTpairs, numTpairs, numUTpairs, freqOT, freqT, freqUT, reinf_OT, pun_OT, reinf_T, pun_T, reinf_UT, pun_UT, frcritical, usepreviousset, flash, stimfname );
	}
						...
						if (DEBUG){resp_sec = resp_rt.tv_sec;}
							...
							if (eff_class[stim_class] == 1){                                 /* GO LEFT */
								if ( (left==0 ) && (right==0) ){
									...
									reinfor=feedFR(stim_reinf, &f, &numconsequtivecorrect, &frcritical);
									if (reinfor == 1) { ++fed;}
