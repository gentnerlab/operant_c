...
int main(int argc, char *argv[])
{
	...
	if(DEBUG){
		fprintf(stderr, "command_line_parse(): box_id=%d, subjectid=%d, startH=%d, stopH=%d, startM=%d, stopM=%d, trialsonminutesmin=%d, trialsonminutesmax=%d, trialsoffminutesmin=%d, trialsoffminutesmax=%d, xresp=%d, cue=%d, dosecondaryR=%d, docorrectioncue=%d, corr_flag=%d resp_wind=%d timeout_val=%3.3f numOTpairs=%d numTpairs=%d numUTpairs=%d freqOT=%d freqT=%d freqUT=%d reinf_OT=%d pun_OT=%d reinf_T=%d pun_T=%d reinf_UT=%d pun_UT=%d frcritical=%d usepreviousset=%d flash=%d stimfile: %s ",
		        box_id, subjectid, starthour, stophour, startmin, stopmin, trialsonminutesmin, trialsonminutesmax, trialsoffminutesmin, trialsoffminutesmax, xresp, cueflag, dosecondaryR, docorrectioncue, correctiontrialsoff, resp_wind, timeout_val, numOTpairs, numTpairs, numUTpairs, freqOT, freqT, freqUT, reinf_OT, pun_OT, reinf_T, pun_T, reinf_UT, pun_UT, frcritical, usepreviousset, flash, stimfname );
	}
...
if(DEBUG){fprintf(stderr, "starthour: %d\tstartmin: %d\tstophour: %d\tstopmin: %d\n", starthour,startmin,stophour,stopmin);}
	...
	fprintf (datafp, "resp_wind: %d\ttimeout_val: %3.2f\txresp: %d\tcorrectiontrialsoff: %d\tflash: %d\tcueflag: %d\tdosecondaryR: %d\tdocorrectioncue: %d\n", resp_wind,timeout_val,xresp,correctiontrialsoff,flash,cueflag,dosecondaryR,docorrectioncue);
