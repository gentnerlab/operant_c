...
int main(int argc, char *argv[])
{
  ...
  fprintf(stderr, "command_line_parse(): box_id=%d, subjectid=%d, startH=%d, stopH=%d, startM=%d, stopM=%d, xresp=%d, NOxresp=%d, cue=%d, resp_wind=%d, timeout_val=%f, iti_val=%f, vr_mean=%f, flash=%d, breakdur=%f, ptarg=%f, trialtarg=%d, targdB= %.4f, distdB = %.4f, fullflag = %d, stimfile: %s\n",box_id, subjectid, starthour, stophour, startmin, stopmin, xresp, NOxresp, cueflag, resp_wind, timeout_val, iti_val, vr_mean, flash, breakdur, ptarg, new_trialtarg, normdB, distdB, fullflag, stimfname);
