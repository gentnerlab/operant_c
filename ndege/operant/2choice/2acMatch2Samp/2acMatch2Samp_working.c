...
int main(int argc, char *argv[])
{
  ...
  int nclasses, nstims, stim_class, stim_type, nonmatch_type, stim_reinf, stim_punish, resp_sel, resp_acc, subjectid, period, tot_trial_num, ntargtrials = 1,numtrials,
    played, resp_wind=0,trial_num, session_num, i,j,k, correction, targval,distval, dist_type, loop, stim_number, dist_number, nonmatch_number, cue[3],nreinfloops = 500,
  ...
  float timeout_val=0.0, iti_val=0.0, vr_mean=1.0, resp_rxt=0.0, breakdur = 0.5, idealdur = 2.0, normdB = 65.0, 
  ...
  ********************************************/
  ...
  do{                                                                               /* start the main loop */
	...
	if(DEBUG){printf("correction trial loop finished, checking time-out period, time of day, and number of target trials\n");}                           
      } /* time-in loop */
      free(soundvecout);
