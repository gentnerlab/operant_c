...
***********************************************************************************************/
int make_entseq(Seqstim *ps, int stimclass, char **tmpseq, char *outsfname, int lock)
{
...
int main(int argc, char *argv[])
{
	...
	char *stimfname = NULL;
	char *stimfroot, *pcm_name, *entseq=NULL, *entseq2=NULL, tmpExemplar[128];
	const char delimiters[] = " .,;:!-";
	char datafname[128], hour[16], min[16], month[16],day[16], year[16], dsumfname[128], stimftemp[128],outsf[128], outsf2[128];
	char  buf[128], fstim[256], stimexm[256], stimexm1[128],fstim1[256], stimexm2[256], fstim2[256], timebuff[64], tod[256], date_out[256], buffer[30],temphour[16],tempmin[16];
	int stim_class, targetClass, resp_sel, resp_acc, subjectid, period, foo, bar; 
	...
	fprintf (datafp, "Sess#\tTrl#\tType\tSample\tTarget\tClass\tR_sel\tR_acc\tRT\tReinf\tTOD\tDate\n");
	...
	do{                                                                               /* start the main loop */
	  while ((currtime>=starttime) && (currtime<stoptime)){                          /* start main trial loop */
	    ...
	    if (do_prb <= prbrate) /*its a  probe trial*/
	      ...
	      stim_class = (int) ((2.0)*rand()/(RAND_MAX+1.0) ) + 1;       /*choose the sample class for this trial */ 
	    
	    if(DEBUG){fprintf(stderr,"make a sequence %d motifs long using stim class %d \n", seqstims[stim_class-1].count, stim_class);}
	    if ((foo=make_entseq(pseqstims, stim_class, &entseq, outsf, 0)) < 1){
	    ...
	    strcpy (stimexm1, entseq); 
	    strcpy(fstim1, outsf);
	    if(DEBUG){fprintf(stderr, "%s for stim class %d is %s\n", fstim1, stim_class, stimexm1);}
	   

       targetClass = (int) ((2.0)*rand()/(RAND_MAX+1.0) ) + 1;       /*choose the target class for this trial */
       if(DEBUG){fprintf(stderr,"make a sequence %d motifs long using stim class %d \n", seqstims[stim_class-1].count, targetClass);}
       if ((bar=make_entseq(pseqstims, targetClass, &entseq2, outsf2, 1)) < 1){
       fprintf(stderr, "ERROR!: problem building probe sequence on box %d trial %d\n", box_id, trial_num+1);
       snd_pcm_close(handle);
       fclose(datafp);
       fclose(dsumfp);
       ...
       if(DEBUG){fprintf(stderr, "make_entseq returned '%d'-- %s for stim class %d is %s\n", bar, outsf2, targetClass, entseq2);}
       strcpy (stimexm2, entseq2);
       strcpy(fstim2, outsf2);
       if(DEBUG){fprintf(stderr, "%s for stim class %d is %s\n", fstim2, targetClass, stimexm2);}
		...
		fprintf(stderr,"stim_on sec: %d \t usec: %d\n", (int)stimon.tv_sec, (int)stimon.tv_usec);}
	      if ((played = playwav(fstim1, period))!=1){
		fprintf(stderr, "playwav failed on pcm:%s stimfile:%s. Program aborted %s\n", 
			pcm_name, stimexm1, asctime(localtime (&curr_tt)) );
		...
		fprintf(stderr,"stim_dur sec: %d \t usec: %d\n", (int)stim_dur.tv_sec, (int)stim_dur.tv_usec);
          ...
          targetClass = (int) ((2.0)*rand()/(RAND_MAX+1.0) ) + 1;       /*choose the baseline class for this trial */
         ...
         */if(DEBUG){fprintf(stderr, "%s for stim class %d is %s\n", fstim2, targetClass, stimexm2);}
         /*play the target*/
          ...
          if(DEBUG){fprintf(stderr, "full stim is '%s', period is %d\n", fstim2, period);}
          gettimeofday(&stimon, NULL);
          if ((played = playwav(fstim2, period))!=1){
            fprintf(datafp, "playwav failed on pcm:%s stimfile:%s. Program aborted %s\n",
            pcm_name, stimexm2, asctime(localtime (&curr_tt)) );
            fclose(datafp);
            fclose(dsumfp);
            exit(-1);
          }
		...
		if (DEBUG==3){printf("flag: values at right & left = %d %d\t", right, left);}
	      ...
	      if (DEBUG) {printf("SAMPLE is:  %d   TARGET is:  %d", stim_class, targetClass);} 
	      if ( (left==0 ) && (right==0) ){
		resp_sel = 0;
		resp_acc = 2;
	    ...
	    if (stim_class == targetClass){                                 /* LEFT STIM*/
		  if (left != 0){
		    resp_sel = 1;
		    resp_acc = 1;
	  ...
	  else if (stim_class != targetClass){                           /*  RIGHT STIM*/
		if (left!=0){
		  resp_sel = 1;
		  resp_acc = 0;
		  ++classRses[stim_class].left; ++classRtot[stim_class].left; 
		...
		exit(-1);
	      ...
	      fprintf(datafp, "%d\t%d\t%d\t%s\t%s\t%d\t%d\t%d\t%d\t%.4f\t%d\t%s\t%s\n", 
		      session_num,trial_num,correction,stimexm1,stimexm2,stim_class,targetClass,
		      resp_sel,resp_acc,resp_rxt, reinfor,tod,date_out);
	      fflush (datafp);
	      if (DEBUG){printf("flag: trial data written\n");}
	      //if(DEBUG){fprintf(stderr, "%d\t%d\t%d\t%s\t%s\t%d\t%d\t%d\t%.4f\t%d\t%s\t%s\n", 
		  //  	session_num,trial_num,correction,Sample, Target,stim_class,
		//		resp_sel,resp_acc,resp_rxt, reinfor,tod,date_out);
	      //}
