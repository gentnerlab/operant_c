...
int main(int argc, char *argv[])
{
  ...
  int nclasses, nstims, stim_class, stim_type, change_type, stim_reinf, stim_punish, resp_sel, resp_acc, subjectid, period, tot_trial_num, ntargtrials = 1,numtrials, 
    played, resp_wind=0,trial_num, session_num, i,j,k, correction, targval,distval, dist_type, loop, stim_number, dist_number, change_number, cue[3],nrepeats, 
  ...
  ********************************************/
  ...
  do{                                                                               /* start the main loop */
	...
	change_type = stim_type-1;
	while((change_number == stim_number)||(change_type != stim_type)){
	  changeval = (int) ((totnstims+0.0)*rand()/(RAND_MAX+0.0));           
	  change_number = playlist[changeval];
	  change_type = stimulus[change_number].type;
	}
	...
	for (i = 0; i < nrepeats; i++){
	  if (i<2){
	    ...
	    /*add current distracter*/
	    if(DEBUG){printf("adding distracter %d of %d\t", (i-1), nrepeats-2);}
	    dist_type = stim_type;                           	
	    while(dist_type == stim_type){
	...
	do{                                             /* start correction trial loop */
	  ...
	  fprintf(datafp, "%d\t%d\t%d\t%s\t%d\t%d\t%d\t%.4f\t%.4f\t%.4f\t%d\t%s\t%s\n", session_num, trial_num, 
		  correction, stimexm, stim_class, resp_sel, resp_acc, trialdur, changedur, resp_rxt, reinfor, tod, date_out );
	  fflush (datafp);
