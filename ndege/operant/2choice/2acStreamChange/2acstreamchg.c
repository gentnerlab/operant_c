...
void do_usage()
{
  fprintf(stderr, "2acstreamchg usage:\n");
  ...
  fprintf(stderr, "        -NOx         = use this flag to disable all correction trials (default = correction for X trials, not NR trials)\n");
  ...
  fprintf(stderr, "        -td f:f      = specify minimum and maximum trial duration in seconds(float:float)(default = 6.0-14.0 s)\n");
  fprintf(stderr, "        -cd f        = specify change stimulus duration in seconds (float) (default = 2.0 s)\n");  
...
/**********************************************************************
 **********************************************************************/
int command_line_parse(int argc, char **argv, int *box_id, int *subjectid, int *starthour, int *stophour, int *startmin, int *stopmin, int *resp_wind, float *timeout_val, float *mintdur, float *maxtdur, float *changedur, int *new_trialtarg, float *targ_dB, float *dist_dB, char **stimfname)
{
  int i=0;
  ...
  for (i = 1; i < argc; i++){
	...
	xresp = 1;
      else if (strncmp(argv[i], "-NOx", 4) == 0)
	NOxresp = 1;
...
int main(int argc, char *argv[])
{
  ...
  char  buf[128], stimexm[128], changeexm[128], distexm[128], fstim[256], cstim[256], dstim[256],timebuff[64], tod[256], 
    date_out[256], buffer[30],temphour[16],tempmin[16];
  int nclasses, nstims, stim_class, stim_reinf, stim_punish, resp_sel, resp_acc, subjectid, period, tot_trial_num, ntargtrials = 1,numtrials, 
    played, resp_wind=0,trial_num, session_num, i,j,k, correction, targval,distval, loop, stim_number, dist_number, change_number, cue[3],nrepeats, 
    *playlist=NULL, totnstims=0, mirror=0, dosunrise=0,dosunset=0,starttime,stoptime,currtime, new_trialtarg = 0, changeval, sv, sampcount, start,
    thisonlen, thisont, thisofflen, thisofft,ponmin, ponmax, poffmin, poffmax;
  float timeout_val=0.0, resp_rxt=0.0, changedur = 2.0, trialdur, maxtdur = 14.0, mintdur = 6.0, idealdur = 2.0, normdB = 65.0, 
    dist_rms, targ_rms, foo1, foo2, distdB = 0.0;
  double nidealsamps, fs = 44100, padded, trialsamps, changesamp;
  ...
  short  *soundvecout, stim_spw[MAXFILESZ], change_spw[MAXFILESZ], dist_spw[MAXFILESZ], compoundout[30*44100];
  struct stim{
    char exemplar[128];
    int class;
    int reinf;
  ...
  command_line_parse(argc, argv, &box_id, &subjectid, &starthour, &stophour, &startmin, &stopmin, &resp_wind, &timeout_val, &mintdur, &maxtdur, &changedur, &new_trialtarg, &normdB, &distdB, &stimfname); 
    fprintf(stderr, "command_line_parse(): box_id=%d, subjectid=%d, startH=%d, stopH=%d, startM=%d, stopM=%d, xresp=%d, NOxresp=%d, cue=%d, resp_wind=%d, timeout_val=%f, flash=%d, mintdur=%f, maxtdur=%f, changedur= %f, trialtarg= %d, targdB= %.4f, distdB = %.4f, fullflag = %d, stimfile: %s\n",box_id, subjectid, starthour, stophour, startmin, stopmin, xresp, NOxresp, cueflag, resp_wind, timeout_val, flash, mintdur, maxtdur, changedur, new_trialtarg, normdB, distdB, fullflag, stimfname);
  ...
  if(fullflag == 1)
    distdB = normdB;
  fprintf(stderr, "distracter dB set to %.4f, same level as target", distdB);
  ...
  if(NOxresp){fprintf(stderr, "!!WARNING: Disabling ALL correction trials!!\n");}
	
  nidealsamps = (int)(ceil(idealdur*fs));
  ...
  fprintf (datafp, "Sess#\tTrl#\tType\tStim\t\tDist\t\tT_class\tR_sel\tR_acc\tTdur\tCdur\tRT\tReinf\tTOD\tDate\n");
  ...
  ********************************************/
  ...
  do{                                                                               /* start the main loop */
    while ((currtime>=starttime) && (currtime<stoptime)){                          /* start main trial loop */
      if (DEBUG){printf("currtime %d, thisont %d, thisofft %d\n", currtime, thisont, thisofft);}
      while((currtime < thisofft) || (currtime > thisont)){
	...
	while (numtrials < ntargtrials){
	  ...
	  trialsamps = ceil(trialdur*fs);
	  nrepeats = ceil(trialdur/idealdur);
	  if (DEBUG){printf("trialdur: %.4f, trialsamps: %d, nrepeats: %d\n", trialdur, (int)trialsamps, nrepeats);}

	  /* select change stim exemplar at random */ 
	  change_number = stim_number;
	  while(change_number == stim_number){
	    changeval = (int) ((totnstims+0.0)*rand()/(RAND_MAX+0.0));           
	    change_number = playlist[changeval];
	  }
	  if (DEBUG){printf("change number:  %d\t", changeval);}
	  strcpy (changeexm, stimulus[change_number].exemplar);                       /* get exemplar filename */
	  sprintf(cstim,"%s%s", STIMPATH, changeexm);                                /* add full path to file name */
	  if ((sv = getsoundvec(cstim, nidealsamps, soundvecout, normdB, period))!=1){ 
	    fprintf(stderr, "could not convert stimfile %s to sound pressure wav. Program aborted %s\n",
		    changeexm, asctime(localtime (&curr_tt)) );
	    fprintf(datafp, "could not convert stimfile %s to sound pressure wav. Program aborted %s\n",
		    changeexm, asctime(localtime (&curr_tt)) );
	    fclose(datafp);
	    fclose(dsumfp);
	  ...
	  for (i=0; i<nidealsamps; i++){
	    change_spw[i] = soundvecout[i];
	    soundvecout[i] = 0;	
	  }     
	    
	  /* determine trial class*/
	  stim_class = (int) (2.0*(rand()/(RAND_MAX + 0.0)))+ 1;
	  if (DEBUG){printf("trial class:  %d\n", stim_class);}
	  if (stim_class == 1){
	    changesamp = trialsamps;}
	  else if (stim_class == 2){
	    changesamp = trialsamps - ceil(changedur*fs);}
	  else {fprintf(datafp,"incorrect stim class %d, program aborted", stim_class);
	  fclose(datafp);
	  fclose(dsumfp);
	  exit(-1);
	  }
	  ...
	  for (i = 0; i < nrepeats; i++){
	    if (i<2){
	      /*fill this section of compound spw */
	      start = i*nidealsamps;
	      for (j=0; j<nidealsamps; j++){
		compoundout[start+j] = stim_spw[j]; 
		sampcount++;
	      }
	    }
	    else{
	      /*add current distrcater*/
	      if(DEBUG){printf("adding distractor %d of %d\t", (i-1), nrepeats-2);}
		...
		if (sampcount > (changesamp-nidealsamps)){
		  while(dist_number == change_number){
		    distval = (int) ((totnstims+0.0)*rand()/(RAND_MAX+0.0));           
		    dist_number = playlist[distval];
		  }
		}
	      ...
	      if(DEBUG){printf("dist number: %d, sampcount: %d, changesamp-nidealsamps: %d\t", distval, (int)  sampcount, (int)(changesamp-nidealsamps));}
		...
		soundvecout[j] = 0;
	      ...
	      start = i*nidealsamps;
	      for (j=0; j<nidealsamps; j++){
		if ((stim_class == 1)||(sampcount < changesamp)){compoundout[start+j] = stim_spw[j]+dist_spw[j];} 
		else {compoundout[start+j] = change_spw[j]+dist_spw[j];}	
		sampcount++; 
	      }
	    }
	  }
	  ...
	  do{                                             /* start correction trial loop */
	    ...
	    if ((played = playsoundvec(compoundout, trialsamps, period))!=1){
	    ...
	    /* pause for response */
	    if (DEBUG){printf("flag: allowing pause before response so no unintended responses - fix this so you can set a known amount of time\n");}
	    loop = 0;
	    do{
	      nanosleep(&rsi, NULL);
	      ++loop;
	    ...
	    fprintf(datafp, "%d\t%d\t%d\t%s\t%s\t%d\t%d\t%d\t%.4f\t%.4f\t%.4f\t%d\t%s\t%s\n", session_num, trial_num, 
		    correction, stimexm, distexm, stim_class, resp_sel, resp_acc, trialdur, changedur, resp_rxt, reinfor, tod, date_out );
	    fflush (datafp);
      ...
      thisofflen = rand() % (poffmax-poffmin) + poffmin;
      thisont = currtime + thisonlen + thisofflen;
                                              /* reset the stim number for correct trial*/
    }                                                                  /* trial loop */
