...
int main(int argc, char *argv[])
{
	...
	int nclasses, nstims, stim_class, stim_reinf, stim_punish, resp_sel, resp_acc, subjectid, period, tot_trial_num, 
	  played, resp_wind=0,trial_num, session_num, sessionTrials=0, i,j,k, correction, playval, loop, stim_number, cue[3], 
	  *playlist=NULL, totnstims=0, mirror=0, dosunrise=0,dosunset=0,starttime,stoptime,currtime, criterion_reached, stimset_plays, last_10[10], eff_class, curr_cycle_position, choose_stim, sum_check;
	...
	/* Parse the command line */
        command_line_parse(argc, argv, &box_id, &subjectid, &starthour, &stophour, &startmin, &stopmin, &resp_wind, &timeout_val, &stimfname); 
       	if(DEBUG){
	  fprintf(stderr, "command_line_parse(): box_id=%d, subjectid=%d, startH=%d, stopH=%d, startM=%d, stopM=%d, xresp=%d, cue=%d, corr_flat=%d resp_wind=%d timeout_val=%f flash=%d stimfile: %s ",
		box_id, subjectid, starthour, stophour, startmin, stopmin, xresp, cueflag, correction_flag, resp_wind, timeout_val, flash, stimfname );
	}
	...
	nstims = nclasses = 0;
	  ...
	  for (i = 0; i < nstims; i++){
	    ...
	    sscanf(buf, "%s\%s\%d\%d\%d", stimulus[i].exemplara, stimulus[i].exemplarb, &stimulus[i].freq, &stimulus[i].reinf, &stimulus[i].punish);
	...
	do{                                                                               /* start the main loop */
	  while ((currtime>=starttime) && (currtime<stoptime)){                          /* start main trial loop */
	    ...
	    stim_number = playlist[playval];
            stim_class = eff_class;                               /* set stimtype variable */
	 ...
	 stimset_plays = 0;				/* reset variables for discrimination learning */
	    ...
	    do{                                             /* start correction trial loop */
	      ...
	      /* choose one of the two stimuli at random */
		...
		if ( (left==0) && (right==0) ){
	      ...
	      else if (eff_class >= 2){                           /* PROBE STIMULUS */
		...
		exit(-1);
	      ...
	      fprintf(datafp, "%d\t%d\t%s\t%s\t%d\t%d\t%d\t%d\t%.4f\t%d\t%s\t%s\n", session_num, trial_num, 
		      stimexma, stimexmb, stimset_plays, eff_class, resp_sel, resp_acc, resp_rxt, reinfor, tod, date_out );
	      fflush (datafp);
