...
/* --------- STATE VARIABLES ------------*/
#define LOW_TIME    2000000
#define MID_TIME    10000000
#define HIGH_TIME   20000000
#define LOW_PROB    20
#define MID_PROB    40
#define HIGH_PROB   80
...
/**********************************************************************
 **********************************************************************/
int command_line_parse(int argc, char **argv, int *box_id, int *subjectid, int *initstate, int *starthour, int *stophour, int *startmin, int *stopmin, int *resp_wind, float *timeout_val, char **stimfname)
{
  int i=0;
...
int main(int argc, char *argv[])
{
	...
	int nclasses, nstims, stim_class, stim_reinf, stim_punish, resp_sel, resp_acc, subjectid, period, tot_trial_num, 
	  ...
	  totnstims=0, mirror=0, dosunrise=0,dosunset=0,starttime,stoptime,currtime;
    ...
    int rightFeedTime = 0; 
    int rightFeedProb = 0;
    int centerFeedTime = 0; 
    int centerFeedProb = 0;
	...
	/* Parse the command line */
        command_line_parse(argc, argv, &box_id, &subjectid, &initstate, &starthour, &stophour, &startmin, &stopmin, &resp_wind, &timeout_val, &stimfname); 
       	if(DEBUG){
    ...
    switch(initstate) {
    case 1: 
        leftFeedTime = LOW_TIME; 
        leftFeedProb = HIGH_PROB;
        rightFeedTime = MID_TIME;
        rightFeedProb = MID_PROB;
        centerFeedTime = HIGH_TIME;
        centerFeedProb = LOW_PROB;                     
        break;
    case 2:
        leftFeedTime = MID_TIME; 
        leftFeedProb = MID_PROB;
        rightFeedTime = HIGH_TIME;
        rightFeedProb = LOW_PROB;
        centerFeedTime = LOW_TIME;
        centerFeedProb = HIGH_PROB;
        break;
    case 3:
        leftFeedTime = HIGH_TIME;
        leftFeedProb = LOW_PROB;
        rightFeedTime = LOW_TIME;
        rightFeedProb = HIGH_PROB;
	    centerFeedTime = MID_TIME;
	    centerFeedProb = MID_PROB;
	break;
    case 4:
        leftFeedTime = MID_TIME;
        leftFeedProb = MID_PROB;
