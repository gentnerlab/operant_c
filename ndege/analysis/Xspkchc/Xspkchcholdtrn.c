...
int main (int ac, char *av[])
{
  ...
  int resp_sel, resp_acc, resp_tod, resp_date, bystim=0, nstims, cx_src=0, stim_group, cue, tone;
  ...
  for (i = 1; i < ac; i++){
	...
	fprintf(stderr, "Xspkchcholdtrn usage:\n");
	fprintf(stderr, "    Xspkchcholdtrn [-hpcxvm] [-b int] [-S int] <filename>\n\n");
...
/*   } */
  ...
  if(DEBUG==2){printf("%d\n", subjectid);}
  sprintf (infilename, "%d_spkchcholdtrn.rDAT", subjectid);
  if(DEBUG==2){printf("%s\n", infilename);}
...
/*   } */
  ...
  /* setup output files */
  if (bystim){
    sprintf(outfname, "%i_bystim.spkchcholdtrn_XDAT", subjectid);
    for  (i=0; i<nstims; i++)
      fprintf(stderr, "stimulus file '%d': %s\n", i+1, stimlist[i].exem);
  }
  else
    sprintf(outfname, "%i.spkchcholdtrn_XDAT", subjectid);
  ...
  if (plot_out == 1){
    if (bystim)
      sprintf(plotfname, "%i_spkchcholdtrn_bystim.plot_out", subjectid);
    else
      sprintf(plotfname, "%i_spkchcholdtrn.plot_out", subjectid);
    if ((plotfp = fopen(plotfname, "w")) == NULL){
      fprintf(stderr, "ERROR!: problem opening data output file!: %s\n", outfname);
      fclose(plotfp);
      exit (-1);
    }
    fprintf(plotfp, "#gnuplot output file for %d spkchcholdtrn\n", subjectid);
  }
  ...
  while (fgets (line, sizeof(line), infp) != NULL ){                /* read in a line from infile */
    ...
    if (sscanf (line, "%d%d%d%d%d%d%d%d%f%d%d%d", &sess_num, &trl_num, &trl_type, 
		&stim_class, &cue, &tone, &resp_sel, &resp_acc, &resp_rt, &reinf_val, &resp_tod, &resp_date) == 12){
