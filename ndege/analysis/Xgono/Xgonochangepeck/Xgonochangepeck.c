...
int main (int ac, char *av[])
{
  ...
  int resp_sel, resp_acc, resp_tod, resp_date, freq, rater, ratep, total;
  ...
  char stimfa[128], stimfb[128], stimfline[256];
...
/* Parse the command line */
	...
	//printf("%d\n%s\n", subjectid, stimfname);
	sprintf (infilename, "%d_%s.gng_rDAT", subjectid, stimfroot);
	//printf("%s\n", infilename);
	fprintf(stderr, "\n\nAnalyzing data in '%s'\n", infilename);
...
/* determine number of stimulus classes */
	if ((stimfp = fopen(stimfname, "r")) == NULL)
	...
	else
	  {
	    ...
	    while (fgets (stimfline, sizeof(line), stimfp) != NULL )
	      {
		sscanf(stimfline, "%d%s%s%d%d%d", &class, stimfa, stimfb, &freq, &rater, &ratep);
 ...
 /*set ngroups for the subsequent analysis */
	...
	if(DEBUG){
	  for (i = 0; i < nstims; i++){
	    fprintf(stderr, "stimulus '%d' is '%s %s'\n", i, stimlist[i].exema, stimlist[i].exemb);
	  }
	}
/* setup output files */
 ...
 while (fgets (line, sizeof(line), infp) != NULL )                /* read in a line from infile */
   {
	 ...
	 if(bystim == 1){
	   j=0;
	   while((strcmp(stimlist[j].exema, exemplara) != 0 ) && (j<=ngroups) )
	     j++; 
