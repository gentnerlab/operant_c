/****************************************************************************
**    Xgonopeck.c -- use to analyse data from gngpeck
*****************************************************************************
**
** 10-04-07 DPK: Adapted from most recent version of Xgono.c
** 
**
*/
...
{
  int sess_num, trl_num, trl_type, stim_class, block_num;
  ...
  float wn_offset, wn_dur, resp_rt;
	...
	for (i = 1; i < ac; i++)
	  {
		...
		if (strncmp(av[i], "-S", 2) == 0)
		  ...
		  {
		    fprintf(stderr, "Xgonopeck usage:\n");
		    fprintf(stderr, "    Xgonopeck [-hpcZXvs] [-b <value>] [-S <value>] <stimfile>\n\n");
...
/* setup output files */
	...
	if (bystim){
	  sprintf(outfname, "%i_%s_bystim.GONOpeck_XDAT", subjectid, stimfroot);
	  if(DEBUG){
	    for  (i=0; i<nstims; i++)
	  ...
	  sprintf(outfname, "%i_%s.GONOpeck_XDAT", subjectid, stimfroot);
	}
 ...
 while (fgets (line, sizeof(line), infp) != NULL )                /* read in a line from infile */
   {
     sess_num = trl_num = trl_type = stim_class = wn_offset = wn_dur =  0;
     ...
     if (sscanf (line, "%d%d%d%s%d%f%f%d%d%f%d%d%d", &sess_num, &trl_num, &trl_type, exemplar, 
		 &stim_class, &wn_offset, &wn_dur, &resp_sel, &resp_acc, &resp_rt, &reinf_val, &resp_tod, &resp_date) == 13)
       {
	 ...
	 if (trial_rep == blocksize)
       ...
       if (resp_tod == resp_date == 0)
       {
	      ...
	      /*resp_tod_last = resp_date_last = 0;*/
       }
   }/* EOF loop */
