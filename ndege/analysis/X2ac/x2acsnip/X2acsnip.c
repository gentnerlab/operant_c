/****************************************************************************
**    X2acdur_b.c -- use to analyse data from 2acdur routine
*****************************************************************************
**
...
{
	float pC[NMAXSTIMS];
	int sess_num, trl_num, trl_type, ptype, stim_class, block_num, freq, Rrate, Prate;
	...
	int resp_tod_last, resp_date_last;
	int altext=0;
	int printstimcount=0;
	float WNoffset,WNdur,resp_rt;
	...
	for (i = 1; i < ac; i++){
		if (*av[i] == '-'){
			if (strncmp(av[i], "-S", 2) == 0)
				...
				fprintf(stderr, "X2acnoise usage:\n");
				fprintf(stderr, "    X2acnoise [-hpcxvm] [-b int] [-S int] <filename>\n\n");
				...
				fprintf(stderr, "     -z int      = use traditional .2ac_rdat extension (1) or 2acWN_rdat (0,default) \n\n");
	...
	if(DEBUG==2){printf("%d\n%s\n", subjectid, stimfname);}
	if (altext==0){
	  sprintf (infilename, "%d_%s.2acWN_rDAT", subjectid, stimfroot);}
	else{
	...
	/* setup output files */
	if (bystim){
		sprintf(outfname, "%i_%s_bystim.2ACWN_XDAT", subjectid, stimfroot);
		for  (i=0; i<nstims; i++)
			fprintf(stderr, "stimulus file '%d': %s\n", i+1, stimlist[i].exem);
	}
	else
		sprintf(outfname, "%i_%s.2ACWN_XDAT", subjectid, stimfroot);
	...
	while (fgets (line, sizeof(line), infp) != NULL ){                /* read in a line from infile */
		sess_num = trl_num = trl_type = ptype = stim_class =  0;
		WNoffset = WNdur = resp_sel = resp_acc = resp_rt = resp_tod = resp_date = 0;
		if (sscanf (line, "%d%d%d%d%f%f%s%d%d%d%f%d%d%d", &sess_num, &trl_num, &trl_type, &ptype, &WNoffset, &WNdur, exemplar,
	...
	if(DEBUG){fprintf(stderr, "finished the file\n");}
