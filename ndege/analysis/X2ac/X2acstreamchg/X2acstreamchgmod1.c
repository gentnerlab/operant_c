...
int main (int ac, char *av[])
{
  ...
  for (i = 1; i < ac; i++){
	...
	fprintf(stderr, "X2acstreamchg usage:\n");
	fprintf(stderr, "    X2acstreamchg [-hpcxvm] [-b int] [-S int] <filename>\n\n");
  ...
  if(DEBUG==2){printf("%d\n%s\n", subjectid, stimfname);}
  sprintf (infilename, "%d_%s.SC_rDAT", subjectid, stimfroot);
  if(DEBUG==2){printf("%s\n", infilename);}
  ...
  while (fgets (line, sizeof(line), infp) != NULL ){                /* read in a line from infile */
    ...
    if (sscanf (line, "%d%d%d%s%d%d%d%f%f%f%d%d%d", &sess_num, &trl_num, &trl_type, exemplar,
		&stim_class, &resp_sel, &resp_acc, &Tdur, &Ddur, &resp_rt, &reinf_val, &resp_tod, &resp_date) == 13){
