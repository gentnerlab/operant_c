...
{
  float pC[NMAXSTIMS];
  int sess_num, trl_num, trl_type, stim_class, block_num, freq, Rrate, Prate, stim, nonmatch, dist;
  ...
  for (i = 1; i < ac; i++){
	...
	fprintf(stderr, "X2acmatch2samp usage:\n");
	fprintf(stderr, "    X2acmatch2samp [-hpcxvm] [-b int] [-S int] <filename>\n\n");
  ...
  sprintf (infilename, "%d.SOD_rDAT", subjectid);
  ...
  if ((stimfp = fopen(stimfname, "r")) == NULL){
    ...
    while (fgets (stimfline, sizeof(line), stimfp) != NULL ){
      sscanf(stimfline, "%s%d%d%d%d", stimf, &class, &freq, &Rrate, &Prate);
  ...
  /* setup output files */
  if (bystim){
    sprintf(outfname, "%i_%s_bystim.SC_XDAT", subjectid, stimfroot);
    for  (i=0; i<nstims; i++)
      fprintf(stderr, "stimulus file '%d': %s\n", i+1, stimlist[i].exem);
  }
  else
    sprintf(outfname, "%i_%s.SC_XDAT", subjectid, stimfroot);
  ...
  while (fgets (line, sizeof(line), infp) != NULL ){                /* read in a line from infile */
    ...
    if (sscanf (line, "%d%d%d%d%d%d%d%d%d%f%f%d%d%d", &sess_num, &trl_num, &trl_type, &stim, &nonmatch, &dist,
		&stim_class, &resp_sel, &resp_acc, &Tdur, &resp_rt, &reinf_val, &resp_tod, &resp_date) == 14){
	...
	if(DEBUG)
	  if(trial_cnt%20==0){fprintf(stderr, "exem: %s\tstimlist: %s\t j:%i\tstim_group:%i\n", exemplar, stimlist[stim_group].exem, j,stim_group);}
      }
      else
