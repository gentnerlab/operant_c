...
{
  float pC[NMAXSTIMS];
  int sess_num, trl_num, trl_type, stim_class, dist_class, trial_id, block_num, freq, Rrate, Prate;
  int resp_sel, resp_acc, resp_tod, resp_date, bystim=0, nstims, cx_src=0, stim_group;
  int trial_cnt, reinf_val, i, j, class, n_stimclass, ngroups, cnt_cx=1, resp_tod_last, resp_date_last;
  int printstimcount=0;
  float resp_rt, dist_db;
  FILE *outfp = NULL, *infp = NULL, *plotfp = NULL, *stimfp = NULL;
  char *stimfname = NULL;
  char infilename[128], outfname[128], plotfname[128], line[256], exemplar[64], distractor[64];
  ...
  for (i = 1; i < ac; i++){
	...
	fprintf(stderr, "X2acQLT usage:\n");
	fprintf(stderr, "    X2acQLT [-hpcxvm] [-b int] [-S int] <filename>\n\n");
  ...
  if(DEBUG==2){printf("%d\n%s\n", subjectid, stimfname);}
  sprintf (infilename, "%d_%s.2acQLT_rDAT2", subjectid, stimfroot);
  if(DEBUG==2){printf("%s\n", infilename);}
  ...
  while (fgets (line, sizeof(line), infp) != NULL ){                /* read in a line from infile */
    ...
    if (sscanf (line, "%d%d%d%s%s%d%d%d%f%d%d%f%d%d%d", &sess_num, &trl_num, &trl_type, exemplar, distractor,
		&stim_class, &dist_class, &trial_id, &dist_db,  &resp_sel, &resp_acc, &resp_rt, &reinf_val, &resp_tod, &resp_date) == 15){
