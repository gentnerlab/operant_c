/****************************************************************************
**    X2acnoise.c -- use to analyse data from any 2ac operant routine 
*****************************************************************************
**
...
int main (int ac, char *av[])
{
	...
	while (fgets (line, sizeof(line), infp) != NULL ){                /* read in a line from infile */
		...
		if (sscanf (line, "%d%d%d%s%d%f%f%d%d%f%d%d%d", &sess_num, &trl_num, &trl_type, exemplar,
		                &stim_class, &WNoffset, &WNdur, &resp_sel, &resp_acc, &resp_rt, &reinf_val, &resp_tod, &resp_date) == 13)
		{
