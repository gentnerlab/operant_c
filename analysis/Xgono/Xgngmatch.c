...
** 10-12-08 MRB: Editing for new gngmatch match-to-sample procedure
**
*/
...
int main (int ac, char *av[])
{
  ...
  int resp_sel, resp_acc, resp_tod, resp_date, freq, rate1, rate2, total;
  ...
  char exemplara[64], exemplarb[64];	
  char stimfa[128], stimfb[128], stimf[128], stimfline[256];
	...
	if ((stimfp = fopen(stimfname, "r")) == NULL)
	...
	else
	  {
	    ...
	    while (fgets (stimfline, sizeof(line), stimfp) != NULL )
	      {
		sscanf(stimfline, "%d%s%s%d%d%d", &class, stimfa, stimfb, &freq, &rate1, &rate2);
...
/* setup output files */
	...
	if (bystim){
	  ...
	  if(DEBUG){printf("Output file name is: %s\n", outfname);}
	}
 ...
 while (fgets (line, sizeof(line), infp) != NULL )                /* read in a line from infile */
   {
     ...
     if (sscanf (line, "%d%d%d%s%s%d%d%d%f%d%d%d", &sess_num, &trl_num, &trl_type, exemplara, exemplarb,  &stim_class, &resp_sel, &resp_acc, &resp_rt, &reinf_val, &resp_tod, &resp_date) == 12)
       {
