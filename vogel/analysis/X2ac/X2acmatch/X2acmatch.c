...
** 10-16-08 MRB Basic changes for 2ac match to sample analysis.
*/
...
int main (int ac, char *av[])
{
  ...
  char *stimfname = NULL;
  char infilename[128], outfname[128], plotfname[128], line[256], exemplara[64], exemplarb[64];
  char stimfline[256], stimfa[64], stimfb[64], stimfroot [128];
  ...
  for (i = 1; i < ac; i++){
	...
	fprintf(stderr, "X2acmatch usage:\n");
	fprintf(stderr, "    X2acmatch  [-hpcxvm] [-b int] [-S int] <filename>\n\n");
  ...
  if(DEBUG==1){printf(stderr,"rdat file is: %s\n", &infilename);}
 ...
 if(DEBUG){ fprintf(stderr, "Successfully opened stim file.\n");}
  

    class = n_stimclass = nstims = 0;
    while (fgets (stimfline, sizeof(line), stimfp) != NULL ){
if(DEBUG){fprintf(stderr, "Value of stimfline is: %d.\n", stimfline);}
      sscanf(stimfline, "%d%s%s%d%d%d", &class, &stimfa, &stimfb, &freq, &Rrate, &Prate);
      stimlist[nstims].class = class;
      strcpy(stimlist[nstims].exema,  stimfa);
      strcpy(stimlist[nstims].exemb,  stimfb);
      nstims++;
 ...
 if(DEBUG){
	 fprintf(stderr, "Number of stims detected=%d \n", nstims);
	 fprintf(stderr, "got to ourput file setup. \n");
 }

  /* setup output files */
  if (bystim){
    sprintf(outfname, "%i_%s_bystim.2AC_XDAT", subjectid, stimfroot);
    for  (i=0; i<nstims; i++)
      fprintf(stderr, "stimulus file '%d': %s\n", i+1, stimlist[i].exema);
  }
  else
  ...
  if (printstimcount==1){
    ...
    if(bystim){
      ...
      for (i=0; i<ngroups; i++)  
	fprintf(stderr, "\t'%s'\tCorr:%d\tIncorr:%d\t No-R:%d\t Total:%d\n", stimlist[i].exema, Rtot[i].C, Rtot[i].X, Rtot[i].N, Rtot[i].C+Rtot[i].X);
      fprintf(stderr,"CORRECION TRIALS\n");
      for (i=0; i<ngroups; i++)  
	fprintf(stderr, "\t'%s'\tCorr:%d\tIncorr:%d\t No-R:%d\t Total:%d\n", stimlist[i].exema, Rtot[i].cC, Rtot[i].cX, Rtot[i].cN, Rtot[i].cC+Rtot[i].cX);
     fprintf(stderr,"ALL TRIALS\n");
     for (i=0; i<ngroups; i++)  
       fprintf(stderr, "\t'%s'\tCorr:%d\tIncorr:%d\t No-R:%d\t Total:%d\n", 
	       stimlist[i].exema, Rtot[i].C+Rtot[i].cC, Rtot[i].X+Rtot[i].cX, Rtot[i].N+Rtot[i].cN, Rtot[i].C+Rtot[i].X+Rtot[i].cC+Rtot[i].cX);
    }
    else{
       fprintf(stderr,"NON_CORRECION TRIALS\n");
      for (i=0; i<ngroups; i++)  
