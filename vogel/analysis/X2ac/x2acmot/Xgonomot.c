/****************************************************************************
**    Xgonomot.c -- use to analyse data from gngmot procedure
*****************************************************************************
**
**
*/
...
{
  int sess_num, trl_num, nummots, stim_class, block_num, maxmots;
  ...
  for (i = 1; i < ac; i++){
	...
	fprintf(stderr, "Xgonomot usage:\n");
	fprintf(stderr, "    Xgonomot [-hpcZXvs] [-b <value>] [-S <value>] <stimfile>\n\n");
  ...
  while (fgets (line, sizeof(line), infp) != NULL ){               /* read in a line from infile */
    ...
    if (sscanf (line, "%d%d%d%s%d%d%d%f%d%d%d", &sess_num, &trl_num, &nummots, exemplar, 
