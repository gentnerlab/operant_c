...
int main (int ac, char *av[])
{
  ...
  for (i = 1; i < ac; i++){
	...
	fprintf(stderr, "X2ac usage:\n");
	fprintf(stderr, "    X2ac [-hpcxvm] [-b int] [-S int] <filename>\n\n");
  ...
  while (fgets (line, sizeof(line), infp) != NULL ){                /* read in a line from infile */
	...
	if (cx_src==2){                   /*correction trial generated from no-resp on previous trial, so responses are valid */ 
	  if (resp_sel == 0)                                              /* no resp to correction trial*/
	    ...
	    fprintf(stderr, "ERROR!: I found a probe stimulus on a correction trial!! session: %d trial: %d\n",sess_num, trl_num );
	    exit(-1);
	  }
	}
	else if(cx_src==1){               /*correction trial generated from incorrect resp on previous trial,responses are invalid */ 
	  if (resp_sel == 0)                                              /* no resp to correction trial*/
	    Rblock[stim_group].cN++;
	  else if (resp_acc == 1){
