/****************************************************************************
**    Xgonopeckstop.c -- use to analyse data from gngpeck
*****************************************************************************
**
** 10-18-07 DPK: Adapted from most recent version of Xgonopeck.c
** 
**
*/
	...
	for (i = 1; i < ac; i++)
	  {
		...
		if (strncmp(av[i], "-S", 2) == 0)
		  ...
		  {
		    fprintf(stderr, "Xgonopeckstop usage:\n");
		    fprintf(stderr, "    Xgonopeckstop [-hpcZXvs] [-b <value>] [-S <value>] <stimfile>\n\n");
...
/* setup output files */
	...
	if (bystim){
	  sprintf(outfname, "%i_%s_bystim.GONOpeckstop_XDAT", subjectid, stimfroot);
	  if(DEBUG){
	    for  (i=0; i<nstims; i++)
	  ...
	  sprintf(outfname, "%i_%s.GONOpeckstop_XDAT", subjectid, stimfroot);
	}
 ...
 while (fgets (line, sizeof(line), infp) != NULL )                /* read in a line from infile */
   {
     ...
     if (sscanf (line, "%d%d%d%s%d%d%d%d%d%d", &sess_num, &trl_num, &trl_type, exemplar, 
		 &stim_class, &resp_sel, &resp_acc, &reinf_val, &resp_tod, &resp_date) == 10) /*&resp_rt, &reinf_val, &resp_tod, &resp_date) == 11)*/
       { 
