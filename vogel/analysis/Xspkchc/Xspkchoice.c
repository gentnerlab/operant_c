/****************************************************************************
**    Xspkchoice.c -- use to analyse data from spkchoice operant routine 
*****************************************************************************
**
...
{
  float pC[NMAXSTIMS];
  int sess_num, trl_num, trl_type, stim_class, block_num, freq, Rrate, Prate, resp_tod_last, resp_date_last;
  int resp_sel, resp_acc, resp_tod, resp_date, bystim=0, nstims, cx_src=0, stim_group, miss, cr, rf1, rf2, rf3, lf1, lf2, lf3;
  int trial_cnt, reinf_val, i, j, class, n_stimclass, ngroups, cnt_cx=1;
  int printstimcount=0;
  float resp_rt,rt1, rt2, rt3, lt1, lt2, lt3;
  ...
  for (i = 1; i < ac; i++){
	...
	fprintf(stderr, "Xspkchoice usage:\n");
	fprintf(stderr, "    Xspkchoice [-hpcxvm] [-b int] [-S int] <filename>\n\n");
...
/*   } */
  ...
  if(DEBUG==2){printf("%d\n", subjectid);}
  sprintf (infilename, "%d_spkchoice.rDAT", subjectid);
  if(DEBUG==2){printf("%s\n", infilename);}
...
/*       sscanf(stimfline, "%d%s%d%d%d", &class, stimf, &freq, &Rrate, &Prate); */
...
/*   } */
  ...
  /* setup output files */
  if (bystim){
    sprintf(outfname, "%i_bystim.spkchoice_XDAT", subjectid);
    for  (i=0; i<nstims; i++)
      fprintf(stderr, "stimulus file '%d': %s\n", i+1, stimlist[i].exem);
  }
  else
    sprintf(outfname, "%i.spkchoice_XDAT", subjectid);
  ...
  if (plot_out == 1){
    if (bystim)
      sprintf(plotfname, "%i_bystimspkchoice.plot_out", subjectid);
    else
      sprintf(plotfname, "%i_spkchoice.plot_out", subjectid);
    if ((plotfp = fopen(plotfname, "w")) == NULL){
      fprintf(stderr, "ERROR!: problem opening data output file!: %s\n", outfname);
      fclose(plotfp);
      exit (-1);
    }
    fprintf(plotfp, "#gnuplot output file for %d spkchoice\n", subjectid);
  }
  ...
  while (fgets (line, sizeof(line), infp) != NULL ){                /* read in a line from infile */
    ...
    if (sscanf (line, "%d%d%d%d%d%d%f%d%d%d%f%d%f%d%f%d%f%d%f%d%f%d%d%d", &sess_num, &trl_num, &trl_type, 
		&stim_class, &resp_sel, &resp_acc, &resp_rt, &reinf_val, &miss, &cr, &rt1, &rf1, &rt2, &rf2, &rt3, &rf3, &lt1, &lf1, &lt2, &lf2, &lt3, &lf3, &resp_tod, &resp_date) == 24){

      resp_tod_last = resp_tod;
      resp_date_last = resp_date;
    ...
    if (resp_tod == resp_date == 0){
      resp_tod = resp_tod_last;
      resp_date = resp_date_last;
    }
  }                                                     /* EOF loop */
