...
#include "/usr/local/src/audioio/dev/audout_stimstop.c"
...
int main(int argc, char *argv[])
{
  ...
  int dinfd=0, doutfd=0, nclasses, nstims, stim_class, stim_number, stim_reinf, 
    ...
    int numpecks=0;
    int respafterstim = 0;
  float resp_rxt=0.0, timeout_val=0.0,resptime=0.0;
  ...
  struct PECK trial, *trl;
	...
	command_line_parse(argc, argv, &box_id, &subjectid, &starthour, &stophour, &startmin, &stopmin, &resp_wind, &timeout_val, &stimfname);
        if(DEBUG){
          fprintf(stderr, "command_line_parse(): box_id=%d, subjectid=%d, startH=%d, stopH=%d, startM=%d, stopM=%d, xresp=%d, resp_wind=%d timeout_val=%f flash=%d flashpre=%d stimfile: %s\n",
                box_id, subjectid, starthour, stophour, startmin, stopmin, xresp, resp_wind, timeout_val, flash, flashpre, stimfname);
        }
	  ...
	  while ((currtime>=starttime) && (currtime<stoptime)){                          /* start main trial loop */
		...
		center = operant_read(box_id, CENTERPECK);   /*get value at center peck position*/		 	       
		 if((center==0) && flashpre){
	      ...
	      if (play_and_peck_stop(fstim, period,stimulus_duration, offset, box_id, trl, ptm, &numpecks, &stimonset, &respduringstim)!=1){
...
/* Wait for center key press - Modify this within play_and_peck */
respafterstim = 0;
