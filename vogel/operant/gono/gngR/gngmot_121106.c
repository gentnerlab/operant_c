...
/*****************************************************************************
** gngmot.c - code for running go/nogo operant training procedure
******************************************************************************
**
...
void do_usage()
{
  ...
  fprintf(stderr, "       -mmax int        = set the max number of motifs in a sequence (min is 1)\n");      
...
/**********************************************************************
 **********************************************************************/
int command_line_parse(int argc, char **argv, int *box_id, int *subjectid, int *starthour, int *stophour, int *startmin, int *stopmin, int *resp_wind, float *timeout_val, char **stimfname, int *prbrate, int *goRf, int *noRf, int *prbRf, int *motmax)
{
int i=0;
 ...
 for (i = 1; i < argc; i++){
   if (*argv[i] == '-'){
     ...
     else if (strncmp(argv[i], "-mmax", 5) == 0)
       sscanf(argv[++i], "%i", motmax); 
     else{
...
/****************************************************************
 ****************************************************************/
int make_motseq(Stim *ps, int pmots, int nmots, char **tmpseq, char *outsfname)
{
  ...
  *tmpseq = NULL;
  ...
  /*choose nmots (out of pmots) from motlist */
  ...
  for (i=1; i<nmots; i++){
    ...
    for (j=0; j<i; j++){
      while(mot[i] == mot[j]) //if nmots is nearly equal to pmots this could take a while!
	mot[i] = (int) ((pmots+0.0)*rand()/(RAND_MAX+1.0) );
    }
  ...
  *tmpseq = seq;
  ...
  if(DEBUG){
    for (i=0; i<nmots; i++){
      motnum = mot[i];
      sprintf(sfname,"%s%s", STIMPATH, ps[motnum].exemplar);
...
int main(int argc, char *argv[])
{
  ...
  //const char *prbtmp = "seqprb_tmp.wav";
  ...
  int dinfd=0, doutfd=0, stim_class, stim_number, stim_reinf, foo,
    ...
    trial_num, session_num, i, 
    sessionTrials, dosunrise=0,dosunset=0,starttime,stoptime,currtime;
  float resp_rxt=0.0, timeout_val=0.0, sframp=0.0;
  ...
  int nstims=0, nclasses=0, nbslstims=0, nprb3stims=0, nprb4stims=0;
  int ret, nmots, prbx, pmots, do_prb;
  int motmax =1, prbrate=1, goRf=0, noRf=0, prbRf = 0;

  Stim tmp, bslstim[MAXSTIM], prb3stim[MAXSTIM], prb4stim[MAXSTIM], *pstim;
  ...
  struct response {
    ...
    int no;
    float ratio;
  } nmot3Rses[32], nmot3Rtot[32], nmot4Rses[32], nmot4Rtot[32], classRses[MAXCLASS], classRtot[MAXCLASS];
 ...
 /* set up termination handler*/
  ...
  command_line_parse(argc, argv, &box_id, &subjectid, &starthour, &stophour, &startmin, &stopmin, &resp_wind, &timeout_val, &stimfname, &prbrate, &goRf, &noRf, &prbRf, &motmax);
  if(DEBUG){fprintf(stderr, "command_line: box_id=%d subjectid=%d startH=%d stopH=%d startM=%d stopM=%d resp_wind=%d timeout_val=%f flash=%d prbrate=%d goRf=%d noRf=%d prbRf=%d motmax=%d stimfile: %s\n", box_id, subjectid, starthour, stophour, startmin, stopmin, resp_wind, timeout_val, flash, prbrate, goRf, noRf, prbRf, motmax, stimfname);}
 ...
 sprintf(pcm_name, "dac%i", box_id);
  ...
  if ((stimfp = fopen(stimfname, "r")) != NULL){
	...
	nbslstims++;
	sframp=0.00;
	if (DEBUG){printf("adding class 1 soundfile to stimlist entry %d\n", nbslstims-1);}
	...
	//	bslstim[nbslstims-1].reinf = tmp.reinf;
	...
	nbslstims++;
	sframp=0.00;
	if (DEBUG){printf("adding class 2 soundfile to stimlist entry %d\n", nbslstims-1);}
	bslstim[nbslstims-1].class = tmp.class;
	strcpy (bslstim[nbslstims-1].exemplar, tmp.exemplar);
	bslstim[nbslstims-1].dur = tmp.dur;
	bslstim[nbslstims-1].num = i;
	...
	nprb3stims++;
	sframp=0.01;
	if (DEBUG){printf("adding class 3 soundfile to stimlist as entry %d\n", nprb3stims-1);}
	prb3stim[nprb3stims-1].class = tmp.class;
	strcpy (prb3stim[nprb3stims-1].exemplar, tmp.exemplar);
	prb3stim[nprb3stims-1].dur = tmp.dur;
	prb3stim[nprb3stims-1].num = i;
	//prb3stim[nprb3stims-1].reinf = tmp.reinf;
	if((tmp.dur = verify_soundfile(fstim))<1){
	  fprintf(stderr, "Unable to verify %s!\n",fstim );  
	  snd_pcm_close(handle);
	  exit(0); 
	...
	nprb4stims++;
	sframp=0.01;
	if (DEBUG){printf("adding class 4 soundfile to stimlist as entry %d\n", nprb4stims-1);}
	prb4stim[nprb4stims-1].class = tmp.class;
	strcpy (prb4stim[nprb4stims-1].exemplar, tmp.exemplar);
	prb4stim[nprb4stims-1].dur = tmp.dur;
	prb4stim[nprb4stims-1].num = i;
	//prb4stim[nprb4stims-1].reinf = tmp.reinf;
	if((tmp.dur = verify_soundfile(fstim))<1){
	  fprintf(stderr, "Unable to verify %s!\n",fstim );  
	  snd_pcm_close(handle);
	  exit(0); 
	...
	break;
      default:    
	fprintf(stderr,"UNKNOWN STIMULUS CLASS '%d' ENCOUNTERED check the '.stim' file\n", tmp.class);
  ...
  fclose(stimfp);
  if(DEBUG){printf("\n\nDone reading in stims; %d stims in %d classes found\n", nstims, nclasses);}
  ...
  ********************************************/
  ...
  for(i = 0; i<motmax;++i){   /*zero out the response tallies */
    nmot3Rses[i].go = nmot3Rses[i].no = nmot3Rtot[i].go = nmot3Rtot[i].no = 0;
    nmot3Rses[i].ratio = nmot3Rtot[i].ratio = 0.0;
    nmot4Rses[i].go = nmot4Rses[i].no = nmot4Rtot[i].go = nmot4Rtot[i].no = 0;
    nmot4Rses[i].ratio = nmot4Rtot[i].ratio = 0.0;}
  ...
  do{                                                                              /* start the block loop */
	...
	//sprintf(fstim,"%s%s", STIMPATH, prbtmp);
	//if(DEBUG){fprintf(stderr,"fstim is '%s'\n", fstim);}
	nmots = 1+(int) ((motmax+0.0)*rand()/(RAND_MAX+1.0) );
	prbx = 1+(int) (100.0*rand()/(RAND_MAX+1.0) );
	if(prbx <= 50){
	  if(DEBUG){fprintf(stderr,"running class 3 probe trial\n");}
	  pmots = nprb3stims;
	  pstim = prb3stim;
	  stim_class = 3;
	}
	else{
	  if(DEBUG){fprintf(stderr,"running class 4 probe trial\n");}
	  pmots = nprb4stims;
	  pstim = prb4stim;
	  stim_class = 4;
	}
	if(DEBUG){fprintf(stderr,"OK, now choose %d of %d motifs (probe sequence is set to %s)\n", nmots, pmots, prbseq);}
	if ((foo=make_motseq(pstim, pmots, nmots, &prbseq, outsf)) < 1){
	...
	switch(stim_class){
	case 1:   /* If S+ baseline stimulus */
	  ++nmot3Rses[nmots].count, ++nmot3Rtot[nmots].count; stim_reinf=goRf;
	  if (center==0){   /*no response*/
	    resp_sel = 0; resp_acc = 0;reinfor = 0;
	    ++nmot3Rses[nmots].no; ++nmot3Rtot[nmots].no; 
	    ...
	    ++nmot3Rses[nmots].go; ++nmot3Rtot[nmots].go; 
	...
	case 2:  /* If S- stimulus */
	  ++nmot4Rses[nmots].count; ++nmot4Rtot[nmots].count; stim_reinf=noRf;
	  if (center==0){ /*no response*/
	    resp_sel = 0; resp_acc = 1; reinfor = 0;
	    if (DEBUG){printf("flag: no response to s- stim\n");}
	    ++nmot4Rses[nmots].no; ++nmot4Rtot[nmots].no; 
	    ++classRses[stim_class].no; ++classRtot[stim_class].no;
	    break;
	  }
	  else{ /*go response*/
	    resp_sel = 1; resp_acc = 0;
	    ++nmot4Rses[nmots].go; ++nmot4Rtot[nmots].go; 
	    ++classRses[stim_class].go; ++classRtot[stim_class].go;
	...
	case 3:	        /* probe3 trial */
	  ++nmot3Rses[nmots].count; ++nmot3Rtot[nmots].count; stim_reinf=prbRf;
	  if (center==0){ /*no response*/
	...
	case 4:	        /* probe4 trial */
	  ++nmot4Rses[nmots].count, ++nmot4Rtot[nmots].count; stim_reinf=prbRf;
	  if (center==0){ /*no response*/
	    resp_sel = 0; resp_acc = 3; reinfor=0;
	    ++nmot4Rses[nmots].no; ++nmot4Rtot[nmots].no; 
	    ++classRses[stim_class].no; ++classRtot[stim_class].no;
	...
	for (i = 0; i<=motmax;++i){
	  nmot3Rses[i].ratio = (float)(nmot3Rses[i].go) /(float)(nmot3Rses[i].count);
	  nmot3Rtot[i].ratio = (float)(nmot3Rtot[i].go) /(float)(nmot3Rtot[i].count);	
	  nmot4Rses[i].ratio = (float)(nmot4Rses[i].go) /(float)(nmot4Rses[i].count);
	  nmot4Rtot[i].ratio = (float)(nmot4Rtot[i].go) /(float)(nmot4Rtot[i].count);	
	}
	sessionTrials=0;
	...
	if(freopen(dsumfname,"w",dsumfp)!= NULL){
	  ...
	  fprintf (dsumfp, "\tNUM MOTS     \t\tToday     \t\tTotal\n");
	 
	  fprintf (dsumfp, "\tBSL\t\t%1.4f(%d)\t\t%1.4f(%d)\n", nmot3Rses[0].ratio, nmot3Rses[0].count, nmot3Rtot[0].ratio, nmot3Rtot[0].count);
	  for (i = 1; i<=motmax;++i)
	    fprintf (dsumfp, "\t%d\t\t%1.4f(%d)\t\t%1.4f(%d)\n", i, nmot3Rses[i].ratio, nmot3Rses[i].count, nmot3Rtot[i].ratio, nmot3Rtot[i].count);

	  fprintf (dsumfp, "\n\tBSL\t\t%1.4f(%d)\t\t%1.4f(%d)\n", nmot4Rses[0].ratio, nmot4Rses[0].count, nmot4Rtot[0].ratio, nmot4Rtot[0].count);
	  for (i = 1; i<=motmax;++i)
	    fprintf (dsumfp, "\t%d\t\t%1.4f(%d)\t\t%1.4f(%d)\n", i, nmot4Rses[i].ratio, nmot4Rses[i].count, nmot4Rtot[i].ratio, nmot4Rtot[i].count);
	
	  fprintf (dsumfp, "\n\n RESPONSE DATA (by class)\n");
    ...
    for (i = 0; i<motmax;++i){
      nmot3Rses[i].go = 0;
      nmot3Rses[i].no = 0;
      nmot3Rses[i].ratio = 0.0;
      nmot3Rses[i].count = 0;
      nmot4Rses[i].go = 0;
      nmot4Rses[i].no = 0;
      nmot4Rses[i].ratio = 0.0;
      nmot4Rses[i].count = 0; 
    }
