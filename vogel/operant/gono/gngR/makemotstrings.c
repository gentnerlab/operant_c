...
** 12-21-06: TQG: gngmot2 will repeat motifs either 2 or three times in each sequence
*/
...
void do_usage()
{
  fprintf(stderr, "gngmot2 usage:\n");
  fprintf(stderr, "    gngmot2 [-help] [-B int] [-mirrorP] [-t float][-on int:int] [-off int:int] [-x] [-S <subject number>] <filename>\n\n");
...
int make_motseq(Prbstim *ps, int pmots, int nmots, int prbclass, char **tmpseq, char *outsfname)
{
  ...
  *tmpseq = NULL;
  ...
  /*choose nmots (out of pmots) from motlist forcing n sequential repeats*/
    ...
    for (i=nrepeats; i<nmots*nrepeats; i++){
	...
	if(DEBUG){fprintf(stderr, "mot %d is set to %d\n",i+j, mot[i+j]);}
      ...
      i=i+nrepeats-1;
      if(DEBUG){fprintf(stderr, "i is %d\n",i);}
...
int main(int argc, char *argv[])
{
  ...
  fprintf (datafp, "Sess#\tTrl#\tNumMots\tNumReps\tStimulus\t\t\t\t\t\t\tClass\tRspSL\tRspAC\tRspRT\tReinf\tTOD\tDate\n");
  ...
  ********************************************/
  ...
  do{                                                                              /* start the block loop */
	...
	nrepeats = 1 + (int)((100.0)*rand()/(RAND_MAX+1.0) );   /*choose the number of repeats for this trial*/
	if(nrepeats<=50){nrepeats=2;}else{ nrepeats=3;}
	prbx = (int) ((nprbclasses+0.0)*rand()/(RAND_MAX+1.0) );       /*choose the probe class for this trial */ 
	pmots = prbstims[prbx].count;
	if(DEBUG){fprintf(stderr, "nmots: %d \tprbx:%d \tpmots:%d\tnrepeats:%d\n", nmots, prbx, pmots, nrepeats);}
	//pprbstims = prbstims;
	stim_class = prbx+3;
		
	if ((stim_class==19)||(stim_class==20)){ /*run probe catch trial*/
	 stim_number = (int) ((pmots+0.0)*rand()/(RAND_MAX+0.0));       /* select stim exemplar at random */ 
	 strcpy (stimexm, prbstims[prbx].exemplar[stim_number]);                       /* get exemplar filename */
	 sprintf(fstim,"%s%s", STIMPATH, stimexm);
	 if(DEBUG){fprintf(stderr,"Tyring to run prb catch with: class:%d entry:%d exem:%s full:%s\n", stim_class, stim_number, stimexm, fstim);}
	}
	else{ /*run a seq probe trial*/
	  pprbstims = prbstims;
