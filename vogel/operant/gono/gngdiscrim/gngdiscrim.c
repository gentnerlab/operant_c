...
int main(int argc, char *argv[])
{
  ...
  int dinfd=0, doutfd=0, nclasses, nstims, stim_class, stim_number, stim_reinf, 
    ...
    sessionTrials, dosunrise=0,dosunset=0,starttime,stoptime,currtime, criterion_reached, stimset_plays, last_10[10], eff_class, curr_cycle_position, choose_stim, sum_check;
	  ...
	  while ((currtime>=starttime) && (currtime<stoptime)){                          /* start main trial loop */
		...
		last_10[curr_cycle_position] = resp_acc; /* keep track for history */
	      ...
	      fprintf(datafp, "%d\t%d\t%d\t%s\t%s\t\t%d\t%d\t%d\t%.4f\t%d\t%s\t%s\n", session_num, trial_num, 
		      correction, stimexma, stimexmb, stimset_plays, resp_sel, resp_acc, resp_rxt, reinfor, tod, date_out );
	      fflush (datafp);
