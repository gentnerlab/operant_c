 ...
 *******************************************************/
int playstereo(char *sfname1, char *sfname2, char *sfname3, double breakpoint, float targlength, int targloc, int cueinterval, float lessdb, int catch, double period)
{
  SNDFILE *sf1, *sf2, *sf3;
  SF_INFO *sfinfo1, *sfinfo2,*sfinfo3,*sfout_info;
  short *obuff, *obuff1, *obuff2, *obuff3;
  sf_count_t incount1, incount2, incount3;
  double padded, LLrms2, LLrms3;
  long pad = 0;
  ...
  float foo, dist_db;
  int nbits, bw;
  signed int total3, total2;
  float dcoff2,dcoff3, rms2,rms3, db2,db3, newrms, scale2,scale3;
  
  /* memory for SF_INFO structures */
  sfinfo1 = (SF_INFO *) malloc(sizeof(SF_INFO));
  sfinfo2 = (SF_INFO *) malloc(sizeof(SF_INFO));
  sfinfo3 = (SF_INFO *) malloc(sizeof(SF_INFO));
  ...
  if(!(sf3 = sf_open(sfname3,SFM_READ,sfinfo3))){
    fprintf(stderr,"error opening input file %s\n",sfname3);
    free(sfinfo1);
    free(sfinfo2);
    free(sfinfo3);
  ...
  obuff3 = (short *) malloc(sizeof(int)*inframes);
  ...
  if(DEBUG){fprintf(stderr,"got %d frames from sf_readf_short()\n",(int)incount2);}
  if(DEBUG){fprintf(stderr,"file three: trying to sf_readf %d frames,\t",(int)inframes);}
  incount3 = sf_readf_short(sf2, obuff2, inframes);
  if(DEBUG){fprintf(stderr,"got %d frames from sf_readf_short()\n",(int)incount3);}
  ...
  total3 = 0;
  total2 = 0;
  for (j=cueframes; j < inframes; j++){
    total3 += obuff3[j];
    total2 += obuff2[j];
  }
  dcoff3 = (float) (total3 * 1.0 /(inframes - cueframes));
  dcoff2 = (float) (total2 * 1.0 /(inframes - cueframes));
  ...
  LLrms2 = 0.0;
  for (j=cueframes; j<inframes; j++){
    LLrms3 += SQR(obuff3[j] - dcoff3);
    LLrms2 += SQR(obuff2[j] - dcoff2);
  }
  rms3 = sqrt(LLrms3 / (double)(inframes - cueframes)) / (double) pow(2,15);
  rms2 = sqrt(LLrms2 / (double)(inframes - cueframes)) / (double) pow(2,15);
  db3 = bw + (20.0 * log10(rms3) );
  db2 = bw + (20.0 * log10(rms2) );
  if(DEBUG){printf("target/distractor2 rms is %f dB SPL  ", db3);}
  ...
  if (lessdb == -100.0){
    ...
    scale2 = newrms/rms2;
    scale3 = newrms/rms3;
    if(DEBUG){fprintf(stderr,"distractor db:%f newrms:%g,scaledist1 = %g; scaledist2: %g\n", dist_db, newrms, scale2, scale3);}
    // if(DEBUG){fprintf(stderr,"new difference in sound levels (target - distractor(s)): %f, desired difference: %f\n", (db3-dist_db), lessdb);}
    for (j=0; j<inframes; j++)
      obuff2[j] = scale2 * obuff2[j];
    if (catch) {
      for (j=0; j<inframes; j++)
        obuff3[j] = scale3 * obuff3[j];
    }
  }
  ...
  for (j=0; j<(outsamps); j=j+nspeak){
    ...
    if (j<(nspeak*cueframes)){
      ...
      obuff[j] = obuff3[j/nspeak-pauseframes];   /* 2nd dist or target */
      obuff[j+1] = obuff2[j/nspeak-pauseframes]; /* real dist */
    }
      ...
      obuff[j] = obuff2[j/nspeak-pauseframes];    /* real dist */
      obuff[j+1] = obuff3[j/nspeak-pauseframes];  /* 2nd dist or target */
    }
...
void do_usage()
{
  ...
  fprintf(stderr, "                       where each line is: 'Class' 'Wavfile' 'Present_freq' 'Rnf_rate 'Pun_rate''0.00 Breakpoint1...Breakpointn'\n");
...
/**********************************************************************
 **********************************************************************/
int command_line_parse(int argc, char **argv, int *box_id, int *subjectid, int *starthour, int *stophour, int *startmin, int *stopmin, int *resp_wind, float *timeout_val, float *targlength, float *cueint, float *hide_dist, float *catchpercent_man, char **stimfname)
{
  int i=0;
...
int main(int argc, char *argv[]){
  ...
  char  buf[256],distexm[128],dist2exm[128],targetloc[128],stimexm[128],fstim[256], fdist [256], fdist2[256],timebuff[64],buffer[30],*bplist= NULL,
    temphour[16],tempmin[16],tod[256], date_out[256];
  int nstims, stim_reinf, stim_punish, stim_nbps, resp_sel, resp_acc, subjectid, period, tot_trial_num, catchtrial,
    played, resp_wind=0,trial_num, session_num, i,j,k, correction, playval, distval, loop,stim_number,targloc, nbps,minlength,
    *playlist=NULL, totnstims=0,dosunrise=0,dosunset=0,starttime,stoptime,currtime,dist_number,dist_number2,distval2,breakval,
    todaycount, totalcount, todaycorrect, totalcorrect, nclasses,stim_class,dist_class,target, ncatchR, ncatchT, xcatchR, xcatchT;
  float timeout_val=0.0, resp_rxt=0.0, breakpoint = 0.0, cueint = 0.0, targlength = 0.0, lessdb = 0.0, hide_dist, maxbp, minsec, 
    catchpercent = 0.15, catchpercent_man =0.0, catchrand;
  ...
  command_line_parse(argc, argv, &box_id, &subjectid, &starthour, &stophour, &startmin, &stopmin, &resp_wind, &timeout_val, &targlength, &cueint, &hide_dist, &catchpercent_man, &stimfname); 
  if(DEBUG){
    fprintf(stderr, "command_line_parse(): box_id=%d, subjectid=%d, startH=%d, stopH=%d, startM=%d, stopM=%d, resp_wind=%d, timeout_val=%f, targlength=%f, cueint=%f, hide_dist=%f, catchpercent_man=%f, stimfile:%s",    box_id, subjectid, starthour, stophour, startmin, stopmin, resp_wind, timeout_val, targlength, cueint, hide_dist, catchpercent_man, stimfname);
  }
  ...
  fprintf (datafp, "Sess#\tTrl#\tCrxn\tClass\tTargstim\tDiststim\tBreakPt\tTarglen\tCueSrc\tR_sel\tR_acc\tRT\tReinf\tTOD\tDate\n");
  ...
  do{                                                                               /* start the main loop */
	...
	dist_class = stimulus[dist_number].class;                            
      ...
      if (catchrand < catchpercent){
        dist_class = stim_class;
	while ((dist_class == stim_class)||(distval2 == distval)){
	  distval2 = (int) ((totnstims+0.0)*rand()/(RAND_MAX+0.0));
          dist_number2 = playlist[distval2];
	  dist_class = stimulus[dist_number2].class;                            
	}
	       strcpy(dist2exm, stimulus[dist_number2].exemplar);
	       sprintf(fdist2,"%s%s", STIMPATH, dist2exm);                                /* add full path to file name */
	       catchtrial = 1;
	       }
	else{
	  strcpy(fdist2,fstim);
	...
	if (DEBUG){fprintf(stderr, "targloc:%d, catch:%d, crxn:%d ", targloc, catchtrial, correction);} 
	...
	if (DEBUG){fprintf(stderr, "trying to playstereo with target: %s, distractor1 : %s, distractor2 : %s, cue length: %1.2f, target location:'%s' target duration :'%1.2f'\npause length %1.2f, altering distractor by %1.2f dB SPL", stimexm, distexm, fdist2, breakpoint, targetloc, (float)target_duration, cue_stim_interval, lessdb); }
	played = playstereo(fstim, fdist, fdist2, breakpoint, (float)target_duration, targloc,cue_stim_interval, lessdb, catchtrial,period);
	if (DEBUG){fprintf(stderr, "\nplayed: %d\n", played);}
	if(played != 1){
	...
	}while ( (left==0) && (center==0) && (right==0) && (timercmp(&resp_lag, &resp_window, <)) );
	...
	target=targloc;
	    ...
	    if (DEBUG){fprintf(stderr,"flag: right response to catch trial, lights out\n");}
          ...
          if  (center){ /* okay, went center on catch trial */
            resp_sel = 0;
	    resp_acc = 3;
	...
	else if((targloc == 1)& (center==0)){ /* go right */
	  if  ((left==0 ) && (right==1)){ /* correct, went right */
	    ...
	    if (reinfor == 1) { ++fed;}
	    if (DEBUG){fprintf(stderr,"flag: correct response to stimtype 2\n");}
	  }
	    ...
	    ++Rstim[stim_number].left;++Tstim[stim_number].left;
	    reinfor =  timeout(stim_punish);
	    if (DEBUG){fprintf(stderr,"flag: left response to right target, lights out\n");}
	  }
	  else {
	    resp_sel = -1;
	    resp_acc = 2;
	    ...
	    ++Rstim[stim_number].nobyBP[breakval]; ++Tstim[stim_number].nobyBP[breakval];
	    reinfor = 0;
	    if (DEBUG){fprintf(stderr,"flag: no response to right target\n");}
	  }
	}
	else if ((targloc == 2)&&(center ==0)){
	  if  ((left==1 ) && (right== 0)) { /* correct, went left */
	    ...
	    reinfor=feed(stim_reinf, &f);
	    if (reinfor == 1) { ++fed;}
	    if (DEBUG){fprintf(stderr,"flag: correct response to left target\n");}
	  }
	    ...
	    ++Rstim[stim_number].right;++Tstim[stim_number].right;
	    reinfor =  timeout(stim_punish);
	    if (DEBUG){fprintf(stderr,"flag: right response to left target, lights out\n");}
	  }
	  else{
	    resp_sel = -1;
	    resp_acc = 2;
	    ...
	    ++Rstim[stim_number].nobyBP[breakval]; ++Tstim[stim_number].nobyBP[breakval];
	    reinfor = 0;
	    if (DEBUG){fprintf(stderr,"flag: no response to left target\n");}
	  }
	}
	else if (center == 1){ /* doesn't think there is a target - went center*/
	  resp_sel = 0;
	  resp_acc = 3;
	  ++Rstim[stim_number].X;++Tstim[stim_number].X;
	  ++Rstim[stim_number].XbyBP[breakval]; ++Tstim[stim_number].XbyBP[breakval];
	  ++Rstim[stim_number].center;++Tstim[stim_number].center;
	  reinfor = timeout(stim_punish);
	  if (DEBUG){printf("flag: center response to target trial, no reinforcement\n");}
	...
	fprintf(datafp, "%d\t%d\t%d\t%d\t%s\t%s\t%1.2f\t%1.2f\t%d\t%d\t%d\t%.4f\t%d\t%s\t%s\n", session_num, trial_num, 
		correction, stim_class,stimexm, distexm, breakpoint, target_duration, target, resp_sel, resp_acc, resp_rxt, reinfor, tod, date_out );
	fflush (datafp);
	...
	for (i = 0; i<nstims; i++){
	  ...
	  for (j = 0; j<stimulus[j].nbps; j++){
	    Rstim[i].countbyBP[j] = Rstim[i].XbyBP[j] + Rstim[i].CbyBP[j];
	    Tstim[i].countbyBP[j] = Tstim[i].XbyBP[j] + Tstim[i].CbyBP[j];
	  }
	}
	...
	sigprocmask (SIG_UNBLOCK, &trial_mask, NULL);                  /* unblock termination signals */
        correction = 1; /* 10-18-07 took out correction trials bc bird is just guessing to first trial, then doing opp. during crxn trial */
