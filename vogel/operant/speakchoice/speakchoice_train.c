/*********************************************************************************
TRAINING PROTOCOL FOR SPKCHOICE ROUTINE - TO BE USED AFTER 2AC SHAPING - EC 10/07
**********************************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <sndfile.h>
...
int playstereo2_trn(int playside, int fval, float tone_db, float noise_db, int period, int *respside)
{
  ...
  float foo, rampdenom, tonelength = 0.100, stimlength = 1.0;
  ...
  startframe = rand()/(RAND_MAX+0.0)*(stimlength-tonelength)*outsamprate;

  /* determine length inframes*/
  totalframes = (int) (stimlength*outsamprate);
  ...
  if(peak_db>90){
    fprintf(stderr, "PLAYSTEREO2_TRN: SPL in the combined soundfile exceeds 90 dB, SETTING THE MAX TO 90 dB \n");
    scale = 90/peakrms;
    for(i=0; i<outsamps;i++)
      obuff[j] = scale * obuff[j];
  }
  ...
  while (outframes > 0) {
    ...
    if (right == 1){
      *respside = 1;
      ...
      free(ufds);
      ufds=NULL;
      if (outframes > (totalframes-(startframe+tonelength))) /* break if response before target comes on */
	return 0;
      else
	return 1;
    }
...
void do_usage()
{
  fprintf(stderr, "spkchoice_trn usage:\n");
  ...
  fprintf(stderr, "        -w float     = set the response window duration to 'x' secs. default = 0.5 seconds\n");
  ...
  fprintf(stderr, "                       Level 1: cue on, tone on cued side\n");
...
/**********************************************************************
 **********************************************************************/
int command_line_parse(int argc, char **argv, int *box_id, int *subjectid, int *starthour, int *stophour, int *startmin, int *stopmin, float *resp_wind, float *timeout_val, float *tonelength_man, float *tonedb_man, float *brnoise_man, float *rightprob_man, int *reinf_val, int *punish_val, int *trainlevel)
{
  int i=0;
...
int main(int argc, char *argv[])
{
  ...
  int resp_sel, resp_acc, subjectid, period, tot_trial_num, respside,nrespframes,toneframes,trialtype,loop=0,
    played, trial_num, session_num, i, correction, punish_val = 0, reinf_val = 0, earlyerrR = 0, earlyerrT = 0,
    dosunrise=0,dosunset=0,starttime,stoptime,currtime;
  float timeout_val=0.0, resp_rxt=0.0, tonelength = 0.100, tonedb = 75.0, brnoise = 60.0, rightprob = 0.5;
  float  tonelength_man=0.0, tonedb_man = 0.0, respoff_wind = 0.5, resp_wind = 0.0, brnoise_man = 0.0, rightprob_man = 0.0;
  float cueonly_p = 0.0, stimonly_p = 0.0, stimc_p = 1.0, stimx_p = 0.0, stimrand; /* training condition variables set to train level 1 */
  ...
  int leftcue = REDLT, rightcue = GREENLT;
  ...
  command_line_parse(argc, argv, &box_id, &subjectid, &starthour, &stophour, &startmin, &stopmin, &resp_wind, &timeout_val, &tonelength_man, &tonedb_man, &brnoise_man, &rightprob_man, &reinf_val, &punish_val, &trainlevel); 
  if(DEBUG){
    fprintf(stderr, "command_line_parse(): box_id=%d, subjectid=%d, startH=%d, stopH=%d, startM=%d, stopM=%d, xresp=%d, resp_wind=%f timeout_val=%f flash=%d tonelength_man=%f, tonedb_man=%f, brnoise_man=%f, rightprob_man=%f, reinf_val=%d, punish_val=%d, trainlevel=%d\n",
  ...
  if (box_id <= 0){
    ...
    fprintf(stderr, "\tERROR: try 'spkchoicetrn -help' for available options\n");
    exit(-1);
  }
  if (trainlevel <= 0){
 ...
 if(rightprob_man > 0.0)
  ...
  sprintf(datafname, "%i_spkchoicetrn.rDAT", subjectid);
  sprintf(dsumfname, "%i.SPKTRNsummaryDAT", subjectid);
  ...
  ********************************************/
  ...
  for(i = 0; i<=3; ++i){
    R[i].C = R[i].X = R[i].R = R[i].L = R[i].cue = R[i].tone = R[i].N = R[i].count = 0;
    T[i].C = T[i].X = T[i].R = T[i].L = T[i].cue = T[i].tone = T[i].N = T[i].count = 0;
  }
  earlyerrR = earlyerrT = 0;
  if (trainlevel == 2){
    cueonly_p = 0.10; stimonly_p = 0.10; stimc_p = .80; stimx_p = 0.0;}
  if (trainlevel == 3){
    cueonly_p = 0.05; stimonly_p = 0.05; stimc_p = .60; stimx_p = 0.30;}
  ...
  do{                                                                               /* start the main loop */
	...
	/* play the stimulus*/
	if (playside > 0){
	  if ((played = playstereo2_trn(playside, fval, tonedb, brnoise, period, &respside))== -1){
	    fprintf(stderr, "playstereo2_trn failed on pcm:%s. Program aborted %s\n",
        ...
        if (DEBUG){fprintf(stderr,"respside = %d\n",respside);}

	/* get response (if no response during stimulus) */
	if (respside == 0){
	  ...
	  }while ((right==0) && (left== 0) && (timercmp(&resp_lag, &resp_window, <)) ); /* & within response interval */	     
	  if (left == 1) 
	    respside = 2;
	  else if (right == 1)
	    respside = 1;
	...
	else if((trialtype == 0) & (respside > 0)){
	  if ((cueside == 1)&(respside == 1)){
	    resp_acc=1;
	...
	fprintf(datafp, "%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%.4f\t%d\t%s\t%s\n", session_num, trial_num,
		correction, trialtype, cueside, playside, resp_sel, resp_acc, resp_rxt, reinfor, tod, date_out );
	fflush (datafp);
	...
	if(freopen(dsumfname,"w",dsumfp)!= NULL){
	  ...
	  fprintf (dsumfp, "\tResponses Today: R %d  L %d \tTotal: R %d  L %d\n",
		   R[0].R, R[0].L, T[0].R,T[0].L);
