...
long cue_interval = CUE_INTERVAL;
...
/* struct timespec ci = { 0,CUE_INTERVAL}; */
...
int main(int argc, char *argv[]){
  ...
  char  buf[128],cuexm[128],cuelight[128],stimexm[128],fstim[256],timebuff[64],tod[256],date_out[256],buffer[30],temphour[16],tempmin[16], cstim[128];
  int nclasses, nstims, stim_class, stim_reinf, stim_punish, resp_sel, resp_acc, subjectid, period, tot_trial_num, 
    ...
    *playlist=NULL,totnstims=0,dosunrise=0,dosunset=0,starttime,stoptime,currtime,classtest,doprobes,class_idx,doq,
    wn_spl,wn_dur,UBlag,onsetlag,Qindex, CUELT;
  ...
  if ((stimfp = fopen(stimfname, "r")) != NULL){
	...
	printf("ERROR: insufficnet data or bad format in '.stim' file. Try '3ac2 -help'\n");
	exit(0);} 
  ...
  sprintf(datafname, "%i_%s.3ac2_rDAT", subjectid, stimfroot);
  ...
  ********************************************/
  ...
  do{                                                                               /* start the main loop */
	...
	if(DEBUG){fprintf(stderr, "using '%s' as the cue stimulus for this trial\n", cuexm);}
      ...
      /* determine light color corresponding to cue*/
      if (strncasecmp(cuexm,"l",1)==0){
	CUELT=BLUELT;
	sprintf(cuelight, "blue");
      }    
      else if (strncasecmp(cuexm,"m",1)==0){
	CUELT=GREENLT;
	sprintf(cuelight,"green");
      }
      else if (strncasecmp(cuexm,"h",1)==0){
	CUELT=REDLT;
	sprintf(cuelight,"red");
      }
      else{
	CUELT=0;
	sprintf(cuelight,"none");
      }

      sprintf(cstim,"%s%s", STIMPATH, cuexm); 
      if(DEBUG){fprintf(stderr, "using '%s' as the cue light for this trial (IO='%d')\n", cuelight,CUELT);}	
	...
	if(doq){
	  /* 	  operant_write (box_id, CUELT, 1); */   /* uncomment if using visual cue */
	  /* operant_write (box_id, HOUSELT, 0); */      /* uncomment if houselight out during cue  - note: houselight out is also used as negative reinforcement */
	  played = playwav(cstim, period);               /* uncomment if using auditory cue */
	  /* usleep(cue_interval); */                    /* uncomment if using visual cue */
	  /* 	  operant_write (box_id, CUELT, 0); */   /* uncomment if using visual cue */
	  /* operant_write (box_id, HOUSELT, 1); */      /* uncomment if houselight out during cue */


	  if (played != 1){                              /* uncomment if using auditory cue */
	...
	fprintf(stderr, "spl_target=%f, probeCO=%f, correction=%i\n", spl_target, probeCO, correction);
