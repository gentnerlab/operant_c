...
void do_usage()
{
	...
	fprintf(stderr, "        -R int:int    = set the lower:upper bounds (in msec) on the range of stimulus durations for probe trials\n");
...
/**********************************************************************
 **********************************************************************/
int command_line_parse(int argc, char **argv, int *box_id, int *subjectid, int *starthour, int *stophour, int *startmin, int *stopmin, int *mindur, int *maxdur, int *minoffset, int *maxoffset, int *donoiseprobe, int *resp_wind, float *timeout_val, float *dbdesired, char **stimfname)
{
	int i=0;
...
int main(int argc, char *argv[])
{
	...
	char fullsfname[128];
	int minoffset=0,maxoffset=0,mindur=0,maxdur=0, offset=0, pbdur=-1, longest_dur=0, donoiseprobe=0,ret;
	...
	command_line_parse(argc, argv, &box_id, &subjectid, &starthour, &stophour, &startmin, &stopmin, &mindur, &maxdur, &minoffset, &maxoffset, &donoiseprobe, &resp_wind, &timeout_val, &dbdesired, &stimfname);
	if(DEBUG){
		fprintf(stderr, "command_line_parse(): box_id=%d, subjectid=%d, startH=%d, stopH=%d, startM=%d, stopM=%d, xresp=%d, mindur=%d, maxdur=%d, minoffset=%d, maxoffset=%d,  donoiseprobe=%d, dbdesired=%2.2f, resp_wind=%d timeout_val=%f flash=%d stimfile: %s\n",
		        box_id, subjectid, starthour, stophour, startmin, stopmin, xresp, mindur, maxdur, minoffset, maxoffset, donoiseprobe, dbdesired, resp_wind, timeout_val, flash, stimfname);
	}
	...
	if((maxdur-mindur)<=0.0){
		fprintf(stderr, "\tFATAL ERROR: You must give upper and lower bounds for the stimulus duration.\n");
	...
	********************************************/
	...
	do{                                                                               /* start the main loop */
		while ((currtime>=starttime) && (currtime<stoptime)){                     /* start main trial loop */
			...
			if((stim_class>2)||(donoiseprobe==99)){
				if(DEBUG){fprintf (stderr, "PROBE/noise training TRIAL CUED with '%s'\n", fstim);}
				/* randomly select offset between minoffset and maxoffset*/
				offset=minoffset+(int) (((maxoffset-minoffset)+0.0)*rand()/(RAND_MAX+1.0));
				/*figure out your longest duration*/
				if ((stimulus[stim_number].dur-offset)>maxdur)
					longest_dur=maxdur;
				else
					longest_dur=stimulus[stim_number].dur-offset;
				/* randomly select duration between mindur and longest_dur */
				pbdur=mindur+(int) (((longest_dur-mindur)+0.0)*rand()/(RAND_MAX+1.0));
			...
			if (DEBUG){printf("START '%s'\n", fstim);}
				...
				if (((donoiseprobe!=0) && (stim_class > 2))||(donoiseprobe==99)){ /* probes with modified snippets */
