...
void do_usage()
{
  ...
  fprintf(stderr, "      -p int          = set the presentation rate for the probe stimuli(e.g. 10=10%%)\n");
  fprintf(stderr, "      -Rf int:int:int = set the reinforcement rate (%%) for C:X:probe responses\n");
  ...
  fprintf(stderr, "                        'Presfreq' is the presentation rate relative to all stimuli. \n");
  fprintf(stderr, "                         rate for each stimulus is prefreq divided by the sum for all stims.\n");
...
int command_line_parse(int argc, char **argv, int *box_id, int *subjectid, int *starthour, int *stophour, int *startmin, int *stopmin, int *resp_wind, float *timeout_val, char **stimfname)
{
  ...
  for (i = 1; i < argc; i++){
	...
	sscanf(argv[++i], "%i:%i:%i", corrRf, incorrRf, prbRf);
...
/****************************************************************
 ****************************************************************/
int make_motseq(Prbstim *ps, int pmots, int nmots, int prbclass, char **tmpseq, char *o
utsfname)
{
