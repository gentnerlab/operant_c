...
// 12-3-08 mb: added code to enable stop during peck.//
#include <stdio.h>
#include <stdlib.h>
#include <sndfile.h>
#include <math.h>
...
#include "/usr/local/src/audioio/dev/audout_2acpeck.c"
#include <sunrise.h>
...
int cueflag = 0;
char outsfname[256];
long unsigned int pause_secs = 8820; /*Time between exemplars in match to sample*/
...
int main(int argc, char *argv[])
{
	...
	/* structures for pecktimes */
	struct PECK trial, *trl;
		trl = &trial;
		trl->left=trl->center=trl->right=0;
	struct PECKTIME pecktimes, *ptm;
	ptm = &pecktimes;
	int *numpecks, *respduringstim;
	struct timeval *stimonset;
	struct resp{
	  int C;
	  int X;
	  int N;
	...
	do{                                                                               /* start the main loop */
	  while ((currtime>=starttime) && (currtime<stoptime)){                          /* start main trial loop */
		...
		operant_write(box_id, cue[stim_class], 0);
	      ...
	      if (( play_and_peck_stop(outsfname, 0, -1, 1, box_id, trl, ptm, 200, numpecks, stimonset, respduringstim))!=1){
		...
		printf("stim_off sec: %d \t usec: %d\n", stimoff_sec, stimoff_usec);}
	      ...
	      /* Wait for response */
if(respduringstim == 0) /* If no response during playback */
{
