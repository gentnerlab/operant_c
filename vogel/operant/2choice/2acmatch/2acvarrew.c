...
// changed to go block by block, first choosing stimulus 1 and then choosing stimulus 2 //
// changed to encourage speeded response by introducing variable reward option flag //
#include <stdio.h>
#include <stdlib.h>
#include <sndfile.h>
#include <math.h>
...
#define MAXBLOCKLENGTH		 128 		/* max number of stims in a single block */
...
const char exp_name[] = "2ACmatch";
int block_trials = 10; /* default to 10 trials per block */
int box_id = -1;
int flash = 0;
int xresp = 0;
int sample_reps = 1;
int cueflag = 0, var_reward_flag = 0;
...
int feed(int rval, Failures *f, float resprt)
{
  int feed_me=0;
...
void do_usage()
{
  ...
  fprintf(stderr, "        -vr           = use this flag to enable variable reward determined by response latency\n");
  fprintf(stderr, "        -rep int           = number of repetitions before change (default 1)\n");
  ...
  fprintf(stderr, "	   -Length int  = specify the block length (in trials) before continuing. Default=10 \n");
...
/**********************************************************************
 **********************************************************************/
int command_line_parse(int argc, char **argv, int *box_id, int *subjectid, int *block_trials, int *starthour, int *stophour, int *startmin, int *stopmin, int *resp_wind, float *timeout_val, int *sample_reps, char **stimfname)
{
  int i=0;
  ...
  for (i = 1; i < argc; i++){
	...
	cueflag = 1;
      else if (strncmp(argv[i], "-rep", 4) == 0)
	sscanf(argv[++i], "%i", sample_reps);
...
int main(int argc, char *argv[])
{
	...
	int nclasses, nstims, stim_class, nblockids, stim_reinf, stim_punish, resp_sel, resp_acc, subjectid, period, tot_trial_num, 
	  played, resp_wind=0,trial_num, session_num, i,j,k, correction, playval, loop, stim_number, cue[3], 
	  *playlist=NULL, totnstims=0, mirror=0, dosunrise=0,dosunset=0,starttime,stoptime,currtime, blockplay, currblocktrial;
	...
	int reinfor_sum = 0, reinfor = 0, stopflag=0;
	struct stim {
	  char exemplara[128];
	  char exemplarb[128];
	  int blockid;
	...
	/* Parse the command line */
        command_line_parse(argc, argv, &box_id, &subjectid, &block_trials, &starthour, &stophour, &startmin, &stopmin, &resp_wind, &timeout_val, &sample_reps, &stimfname); 
       	if(DEBUG){
	  fprintf(stderr, "command_line_parse(): box_id=%d, subjectid=%d, block_trials=%d, startH=%d, stopH=%d, startM=%d, stopM=%d, xresp=%d, cue=%d, corr_flat=%d resp_wind=%d timeout_val=%f flash=%d stimfile: %s ",
		box_id, subjectid, block_trials, starthour, stophour, startmin, stopmin, xresp, cueflag, correction_flag, resp_wind, timeout_val, flash, stimfname );
	}
	...
	nstims = nclasses = nblockids = 0;
	  ...
	  for (i = 0; i < nstims; i++){
	    ...
	    sscanf(buf, "%d\%d\%s\%s\%d\%d\%d", &stimulus[i].class,
		&stimulus[i].blockid, stimulus[i].exemplara, stimulus[i].exemplarb, &stimulus[i].freq, &stimulus[i].reinf, &stimulus[i].punish);
   ...
   ********************************************/
	...
	do{                                                                               /* start the main loop */
	  while ((currtime>=starttime) && (currtime<stoptime)){                          /* start main trial loop */
	    ...
	    srand(time(0));
	    blockplay = (int) ((double) rand()/RAND_MAX * (double) nblockids) + 1; /* select block to start */
	   currblocktrial=1; 
	    for (currblocktrial=0; currblocktrial<block_trials ; currblocktrial++) { /* iterate rand mly through exemplars from block up to MAXBLOCKTRIALS times */

		   if(DEBUG){printf(" currently on trial %d of %d block trials\n", currblocktrial, block_trials);} 
            playval = (int) ((totnstims+0.0)*rand()/(RAND_MAX+0.0));            /* select stim exemplar at random */ 
	    while (stimulus[playval].blockid != blockplay){			/* keep choosing until you find one with the right blockid */
		playval = (int) ((totnstims+0.0)*rand()/(RAND_MAX+0.0));
	    }
	    
	    

	    if (DEBUG){printf(" found stimulus playval=%d with blockid=%d\n", playval, stimulus[playval].blockid);}
		...
		if (DEBUG){printf("building stimulus file.\n");}
		if (buildtempfile(fstima, fstimb)!=1) {
		...
		if ( (left==0 ) && (right==0) ){
		  ...
		  reinfor=feed(stim_reinf, &f, resp_rxt);
		  if (reinfor == 1) { ++fed;}
		...
		for (i = 0; i<nstims;++i){
		  fprintf (dsumfp, "\t%s\t%s\t%d\t%1.4f     \t\t%d\t%1.4f\n", 
			   stimulus[i].exemplara, stimulus[i].exemplarb, Rstim[i].count, (float)Rstim[i].C/(float)Rstim[i].count, Tstim[i].count, (float)Tstim[i].C/(float)Tstim[i].count );
		}
	...
	if (correction_flag == 0){
        ...
        /* increment block trial and set stopflag as appropriate */
      	  }
	    
	    stim_number = -1;                                                /* reset the stim number for correct trial*/

      if(DEBUG){printf("Reached end of trial loop, choosing new block.\n");}
	  }                                                                  /* trial loop */
