...
int xresp = 0;
int cueflag = 0;
int correction_flag = 0, norflag = 0;
char outsfname[256];
...
void do_usage()
{
  ...
  fprintf(stderr, "        -norT        = use this flag to enable timeouts for no response trials.\n");
...
int command_line_parse(int argc, char **argv, int *box_id, int *subjectid, int *starthour, int *stophour, int *startmin, int *stopmin, int *resp_wind, float *timeout_val, char **stimfname)
{
  ...
  for (i = 1; i < argc; i++){
	...
	sscanf(argv[++i], "%f", timeout_val);
      ...
      else if (strncmp(argv[i], "-norT", 5) == 0){
      norflag = 1;
      }
	...
	do{                                                                               /* start the main loop */
	  while ((currtime>=starttime) && (currtime<stoptime)){                          /* start main trial loop */
		...
		if ( (left==0 ) && (right==0) ){
          ...
          if (norflag) {reinfor = timeout(stim_punish);}
		}
		else if (left != 0){
		  resp_sel = 1;
		  resp_acc = 1;	
