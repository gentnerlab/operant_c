...
/**********************************************************************
 **********************************************************************/
int command_line_parse(int argc, char **argv, int *box_id, int *subjectid, int *starthour, int *stophour, int *startmin, int *stopmin, int *mindur, int *maxdur, int *minoffset, int *maxoffset, int *resp_wind, float *timeout_val, char **stimfname)
{
  int i=0;
...
int main(int argc, char *argv[])
{
	...
	char fullsfname[128];
	int minoffset=0,maxoffset=0,mindur=0,maxdur=0, offset=0, pbdur=-1, longest_dur=0;
	...
	/* Parse the command line */
        command_line_parse(argc, argv, &box_id, &subjectid, &starthour, &stophour, &startmin, &stopmin, &mindur, &maxdur, &minoffset, &maxoffset, &resp_wind, &timeout_val, &stimfname); 
       	if(DEBUG){
	...
	fprintf (datafp, "Sess#\tTrl#\tType\tOffset\tDuration\tStimulus\tClass\tR_sel\tR_acc\tRT\tReinf\tTOD\tDate\n");
	...
	do{                                                                               /* start the main loop */
	  while ((currtime>=starttime) && (currtime<stoptime)){                          /* start main trial loop */
		...
		longest_dur=stimulus[stim_number].dur-offset;
	      ...
	      //pbdur = (rand()%(longest_dur-mindur)) + mindur;
	    }
	    else{
	    ...
	    fprintf(datafp, "%d\t%d\t%d\t%d\t%d\t%s\t\t%d\t%d\t%d\t%.4f\t%d\t%s\t%s\n", session_num, trial_num, 
		    correction, offset, pbdur, stimexm, stim_class, resp_sel, resp_acc, resp_rxt, reinfor, tod, date_out );
	    fflush (datafp);
