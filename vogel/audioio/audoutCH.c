...
#include "audoutCH.h"  /* contains parameters of PECK struct, including  max pecks */
...
int setup_pcmdev(char *pcm_name, int nchans)
{
  ...
  if (snd_pcm_hw_params_set_channels(handle, params, nchans) < 0) {
 ...
 *******************************************************/
int playwav(char *sfname, double period, int nchans)
{
  ...
  if (sfinfo->samplerate != 44100){
    fprintf(stderr, "Sample rate (%d) for %s is not equal to 44.1 KHz!\n",sfinfo->samplerate, sfname);
    sf_close(sf);
    return -1;
  }
  printf("nchans: %d\n", nchans);
  if (sfinfo->channels != nchans){
    fprintf(stderr, "Sound file %s does not have %d chans!\n", sfname, nchans);
    sf_close(sf);
    return -1;
  }
  ...
  if((sfinfo->format & SF_FORMAT_TYPEMASK) == SF_FORMAT_WAV){
    ...
    fprintf(stderr, "Sound file %s is correct format %d!\n", sfname,SF_FORMAT_WAV );
    ...
    obuff = (short *) malloc(sizeof(int)*padded*nchans);
...
int verify_soundfile(char *sfname)
{
 ...
 /*print out some info about the file you just openend */
  ...
  free(sfinfo);
  {printf("duration from soundfile verify:%lu\n",duration);}
  return (duration);
...
/***************************************************************
 *  int play_and_count (char * sound_file_name, double period) *
 *                                                             *
 * returns: 1 on successful play                               *
 *          -1 when soundfile does not play                    *
 *          0 when soundfile plays with under.over run         *
 **************************************************************/
int play_and_count(char *sfname, double period, int nchans, struct PECK *pkpt )
{
...
struct pollfd *ufds;
  ...
  while (outframes > 0){
    ...
    outframes -= err;
     if (outframes == 0){
      {printf("outframes is zero so I'll break\n");}
      break;
    }
