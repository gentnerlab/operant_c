...
snd_pcm_hw_params_get_period_time (params, &pertime, &dir);
  ...
  */
  ...
  /*
  minper=512;maxper=512;
  if(snd_pcm_hw_params_set_periods_max(handle, params, &maxper, &dir)<0){
	  fprintf(stderr, "Couldn't set the max period size to %d\n", (int)maxper);
  }
  fprintf(stderr,"acutal max per: %d\n", maxper);
  
  if(snd_pcm_hw_params_set_periods_min(handle, params, &minper, &dir),0){
	  fprintf(stderr, "Couldn't set the min period size to %d \n", (int)minper);
  }
  fprintf(stderr, "actual min per: %d\n", minper);*/
  
  /*
persize = 512; dir=0;
  if ( (err= snd_pcm_hw_params_set_periods(handle, params, &persize, &dir))< 0) {
    fprintf(stderr, "Error %d setting period size to %d. error %s\n", dir, (int) persize, snd_strerror(err));
  }
  */
  ...
  fprintf(stderr,"period size: %d wtf:%s \n", (int)period_size, strerror(err));
  */

/* snd_pcm_hw_params_get_period_size(params, &persize2, &dir);
  snd_pcm_hw_params_get_period_size_max(params, &perSmax, &dir);   
  snd_pcm_hw_params_get_period_size_min(params, &perSmin, &dir);   
  if(DEBUG){ 
    printf("period = %d frames\n", persize2);
    printf("max pers =%d frames\n",perSmax);
    printf("min pers =%d frames\n", perSmin);
  }
 
  minper=64; maxper=512;
  int dir2;
  snd_pcm_hw_params_set_periods_min(handle, params, &minper, &dir);
  err=snd_pcm_hw_params_set_periods_max(handle, params, &maxper, &dir);
  fprintf(stderr, "actual min per: %d\n", minper);
  fprintf(stderr,"acutal max per: %d wtf:%s \n", maxper, strerror(err));
  snd_pcm_hw_params_set_period_size_first(handle,params, &minper, &dir);
 snd_pcm_hw_params_set_periods(handle, params, 128, &dir); 
*/
...
*/
  ...
  fprintf(stderr, "set period time to %i \n", period_time);
  */
...
int play_and_peck(char *sfname, double period, int stimdur, int stimoffset, int box_id ,struct PECK * trl, struct PECKTIME * ptimes, int *numpecks)
{
  ...
  int i, err, count, init;
  int ramp_dur=0;
  int currpeck,stimlag_sec,stimlag_usec;
